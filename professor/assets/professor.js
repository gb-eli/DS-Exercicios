(()=>{
  ...
  function unifiedLogin(){const u=new URL('../auth/',location.href);u.searchParams.set('returnTo','professor/');location.replace(u.href)}
  ...
  async function boot(){
    loadSession();if(!state.session){unifiedLogin();return}
    try{await getUser();security('auth.session_restored',{source:'professor'});showApp(true);await loadStaff()}catch(err){if(err?.status===401)clearSession();unifiedLogin();setLoginMessage(err?.status===401?'Sua sessão expirou. Entre novamente.':'Sua sessão está ativa, mas este perfil não possui acesso ao Console Professor.',true)}
  }
  $('login-form')?.addEventListener('submit',e=>{e.preventDefault();unifiedLogin()});
  $('logout').addEventListener('click',async()=>{clearInterval(state.liveTimer);closeGuided();await auth.signOut();state.session=null;state.overview=null;state.students=[];state.activities=[];unifiedLogin()});
  $('open-hub')?.addEventListener('click',()=>{location.href='../';});$('open-lobby')?.addEventListener('click',()=>{location.href='../lobby/?v=14.10.8.65';});
