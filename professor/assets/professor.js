(()=>{
  ...
  function fillReleaseForm(){const cid=$('release-class').value,eid=$('release-exercise').value,sid=selectedReleaseStudent(),r=releaseFor(eid,cid,sid),base=referenceBaseDefaults(eid);$('release-enabled').checked=r?r.enabled!==false:true;$('release-at').value=toLocalInput(r?.release_at);$('lock-at').value=toLocalInput(r?.lock_at);$('release-html').checked=!!r?.allow_html_base||base.html;$('release-css').checked=!!r?.allow_css_base||base.css;$('release-js').checked=!!r?.allow_js_base||base.js;$('release-hints').checked=!!r?.allow_extra_hints;$('release-guided').checked=!!r?.allow_guided_support;state.releaseFormVersion=releaseScope()==='group'?'':releaseVersion(r);const st=releaseStatus(r,array(state.overview?.exercises).find(e=>String(e.id)===String(eid)));$('release-state').textContent=`${sid?'Aluno':'Turma'} • ${st.label}${state.releaseDataHealthy?'':' • dados não confirmados'}`;renderReleaseRoster()}
