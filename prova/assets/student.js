(()=>{
  ...
  function waitingRoom(d){
    const me=d.me,selected=String(me?.clan_id||''),takenRoles=new Map((d.team||[]).filter(m=>m.role_id).map(m=>[String(m.role_id),m]));
    const clans=`<div class="clan-grid">${d.clans.map(c=>{
      const team=(d.me?.clan_id&&String(d.me.clan_id)===String(c.id))?d.team:[];
      return `<article class="clan-card ${selected===String(c.id)?'selected':''}"><div class="section-head"><div><h3>${esc(c.name)}</h3><p>${c.count}/${d.session.max_clan_size} participantes</p></div>${selected===String(c.id)?'<span class="status-pill good">Sua empresa</span>':''}</div><div class="members">${team.length?team.map(m=>`<div class="member-mini"><span>${esc(m.student?.full_name||'Aluno')}</span><small>${esc(d.roles.find(r=>String(r.id)===String(m.role_id))?.name||'Sem cargo')}</small></div>`).join(''):'<small class="muted">Entre para visualizar os integrantes.</small>'}</div>${d.session.status==='waiting_room'?(selected===String(c.id)?'<button class="button ghost small" data-leave-clan type="button">Sair da empresa</button>':`<button class="button ${c.count<d.session.max_clan_size?'primary':'ghost'} small" data-join-clan="${c.id}" type="button" ${c.count>=d.session.max_clan_size?'disabled':''}>${c.count>=d.session.max_clan_size?'Lotada':'Entrar'}</button>`):''}</article>`;
    }).join('')}</div>`;
    let roles='';
    if(me?.clan_id){roles=`<div class="role-grid">${d.roles.map(r=>{const holder=takenRoles.get(String(r.id)),mine=String(me.role_id||'')===String(r.id);return `<button class="role-card ${holder&&!mine?'taken':''} ${mine?'selected':''}" data-select-role="${r.id}" type="button" ${d.session.status!=='waiting_room'||(holder&&!mine)?'disabled':''}><span class="icon">${esc(r.icon)}</span><span><strong>${esc(r.name)}</strong><small>${esc(holder?(mine?'Sua área':`Ocupado por ${holder.student?.full_name||'outro aluno'}`):r.description)}</small></span></button>`}).join('')}</div>`;}
    return `<section class="panel section-panel"><div class="section-head"><div><p class="eyebrow">Pré-lobby • ${Number(d.session.lobby_duration_minutes||15)} min</p><h2>Organize sua empresa</h2><p>${d.session.status==='waiting_room'?'Você pode trocar de empresa e cargo enquanto o pré-lobby estiver aberto.':'A composição das equipes foi bloqueada pelo professor.'}</p></div><div id="lobby-timer" class="timer"><span>Tempo planejado</span><strong>${fmtTime(d.session.lobby_remaining_seconds)}</strong></div></div>${clans}</section>${me?.clan_id?`<section class="panel section-panel"><div class="section-head"><div><p class="eyebrow">Seu papel</p><h2>Escolha uma área</h2><p>Cada integrante escolhe uma área diferente. A equipe usa exatamente uma área por integrante; áreas não escolhidas ficam fora da prova daquela equipe.</p></div></div>${roles}</section>`:''}${d.session.status==='locked'?'<div class="notice warn">Equipes bloqueadas. Aguarde o professor iniciar os 50 minutos de desafios.</div>':d.session.status==='waiting_room'&&Number(d.session.lobby_remaining_seconds)<=0?'<div class="notice warn">Os 15 minutos planejados de organização terminaram. Aguarde o professor bloquear as equipes e iniciar a prova.</div>':''}`;
  }
  ...
  function challengesView(d){
    ...
    const teamMembers=(d.team||[]).map(m=>{const r=d.roles.find(x=>String(x.id)===String(m.role_id));const pr=m.progress||{};return `<div class="team-progress-card"><div class="team-progress-head"><div><strong>${esc(m.student?.full_name||'Aluno')} ${String(m.student_id)===leaderId?'<span class="leader-badge">Líder</span>':''}</strong><small>${esc(r?.name||'Área não escolhida')}</small></div><strong>${Number(pr.progress_percent||0)}%</strong></div><progress class="progress" max="100" value="${Number(pr.progress_percent||0)}"></progress><small class="muted">${Number(pr.completed_count||0)}/${Number(pr.total_count||0)} desafios • ${Number(pr.xp_earned||0)}/${Number(pr.xp_max||0)} XP ${pr.online?'• online':''}</small></div>`}).join('');
  ...
  $('exam-content').addEventListener('click',e=>{
    const t=e.target.closest('[data-open-session],[data-back-sessions],[data-join-clan],[data-leave-clan],[data-select-role],[data-submit-challenge]');if(!t)return;
