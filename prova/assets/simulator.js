(()=>{
  ...
  const teamMembers=()=>{
    ...
    const roles=state.catalog?.roles||[];return names().slice(0,7).map((p,i)=>({...p,role_key:roles[i%Math.max(1,roles.length)]?.role_key||'analysis',isLeader:String(p.id)===String(state.leaderId),ready:false}));
  };
  ...
  function renderLobby(){
    ...
    const maxSize=Number(session?.max_clan_size||7),readyTotal=(state.sessionDetail?.members||[]).filter(m=>m.role_id&&m.role_selected_at).length,memberTotal=(state.sessionDetail?.members||[]).length;
