...
async function requester(db:any,user:any){const{data:p}=await db.from("profiles").select("id,email,full_name,role,active").eq("id",user.id).maybeSingle();if(!p?.active||!["teacher","admin","super_admin"].includes(String(p.role)))return null;return{id:user.id,email:String(p.email||user.email||"").toLowerCase(),name:p.full_name||"Professor",role:String(p.role)}}
...
Deno.serve(async req=>{if(req.method==="OPTIONS")return new Response("ok",{headers:H});try{
  ...
  const r=await requester(db,user);if(!r)return J({error:"staff_only"},403);
