...
import { downloadJson, downloadText, sanitizeFilename, escapeHtml } from './core/utils.js';
...
const recordDailyAction = (draft, type) => {
  ...
  if (complete && draft.dailyBonusClaimedDate !== stats.date) {
    ...
    addBadge(draft, 'Operador do Dia');
    return true;
  }
  return false;
};
...
const renderProfileChip = (profile) => {
  ...
  elements.profileChip.innerHTML = `
    <span class="resource-pill">◇ ${profile.coins}</span>
...
const closeModal = ({ preserveTour = false } = {}) => {
  if (!preserveTour) stopGuidedTour('skipped');
...
const useHint = (id) => {
  ...
  if (!profile.hintsUsed[id]) {
    ...
    persistProfile((draft) => {
      ...
      draft.xp = Math.max(0, draft.xp - cost);
      return draft;
    }, { rerender: false });
...
const rewardChallenge = (challenge, attemptNumber) => {
  ...
  const profile = persistProfile((draft) => {
    ...
    draft.xp += challenge.xp;
    draft.coins += challenge.coins + bonusCoins;
    draft.stars += stars;
    ...
    addBadge(draft, 'Primeira Bandeira');
    if (attemptNumber === 1 && hints === 0) addBadge(draft, 'Precisão Cirúrgica');
    if (draft.combo >= 3) addBadge(draft, 'Combo Triplo');
    if (draft.combo >= 7) addBadge(draft, 'Sequência Implacável');
    if (count >= 5) addBadge(draft, 'Caçador de Flags');
    if (count >= 10) addBadge(draft, 'Operador Tático');
    ...
    if (trackDone === trackTotal) addBadge(draft, `Especialista ${challenge.track.toUpperCase()}`);
    if (count === challenges.length) addBadge(draft, 'Mestre do CTF DS');
    return draft;
...
const handleStoreItem = (id) => {
  ...
  if (!owned && profile.coins < item.price) {
    showToast('SALDO INSUFICIENTE', `Faltam ${item.price - profile.coins} Cyber Coins.`);
    return;
  }
  persistProfile((draft) => {
    if (!draft.inventory.includes(item.id)) {
  ...
  showToast(owned ? 'ITEM EQUIPADO' : 'ITEM DESBLOQUEADO', item.name);
  const current = getState().profile;
...
const exportProgress = async () => {
  ...
  const event = { id: crypto.randomUUID(), at: new Date().toISOString(), platform: 'ctfds', version: platformConfig.version };
...
const generateEvidence = async () => {
  ...
  profile.delivery.receipts = [...(profile.delivery.receipts || []), { id: crypto.randomUUID(), type: 'evidence_html', fileName: filename, at: new Date().toISOString() }].slice(-50);
...
const useTemporarySession = () => {
  ...
  const profile = createDefaultProfile(`temporary_${crypto.randomUUID()}`, enteredName.length >= 2 ? enteredName : 'Aluno temporário', enteredClass || 'Turma não informada');
...
document.addEventListener('click', (event) => {
  ...
  if (event.target.closest('[data-verify-integrity]')) {
    verifyAuditChain(getState().profile).then((valid) => showToast(valid ? 'INTEGRIDADE VERIFICADA' : 'ALTERAÇÃO DETECTADA', valid ? 'A cadeia de histórico local está consistente.' : 'O histórico apresenta inconsistência. Exporte um backup e procure o professor.', valid ? 'info' : 'important'));
    return;
  }
