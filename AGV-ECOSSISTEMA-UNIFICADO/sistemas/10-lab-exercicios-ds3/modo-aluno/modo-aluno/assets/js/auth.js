window.AppAuth = (() => {
  ...
  const scope = () => config().scope === 'professor' ? 'professor' : 'aluno';
    ...
    return `
      <div id="authGate" class="auth-gate" role="dialog" aria-modal="true" aria-labelledby="authPlatformTitle" aria-describedby="authPlatformDescription">
        ...
        <article class="auth-panel card">
          <div class="auth-brand"><span class="auth-logo">&lt;/&gt;</span><div><small>3º DS · ${scope() === 'professor' ? 'Professor' : 'Aluno'}</small><h1 id="authPlatformTitle">Programação no Desenvolvimento de Sistemas</h1><p id="authPlatformDescription">Sessão local criptografada neste navegador por até ${SESSION_DAYS} dias.</p></div></div>
          ...
          <form id="registerForm" class="auth-form" hidden>
            ...
            <label>${scope() === 'professor' ? 'Identificação' : 'Turma'}<input id="registerGroup" maxlength="30" placeholder="${scope() === 'professor' ? 'Professor de Programação no DS' : 'Ex.: 3º DS C'}"></label>
      ...
      <section id="platformHome" class="platform-home">
        <div class="platform-home-hero card">
          <div class="platform-home-copy">
            <span class="eyebrow">3º DS · Manhã · ${scope() === 'professor' ? 'Modo Professor' : 'Modo Aluno'}</span>
            ...
            <p class="platform-lead">${scope() === 'professor'
              ? 'Prepare e acompanhe atividades progressivas de desenvolvimento de sistemas com materiais pedagógicos, códigos de referência, Classroom e ambiente de demonstração.'
              : 'Aprofunde seus conhecimentos em desenvolvimento de sistemas por meio de projetos progressivos, organização de arquivos, boas práticas, acessibilidade, programação e construção de soluções completas.'}</p>
  ...
  return { init, log, showHome, openExercise, lock, logout, switchUser, exportLogs, currentUser: () => currentUser };
})();
