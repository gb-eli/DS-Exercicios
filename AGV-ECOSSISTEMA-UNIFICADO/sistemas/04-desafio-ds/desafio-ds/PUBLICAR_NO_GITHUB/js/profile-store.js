(function(){
  ...
  async function importProfileFile(file){const text=await file.text();let parsed;try{parsed=JSON.parse(text);}catch(_){throw new Error('Arquivo de perfil inválido.');}if(parsed?.format!==PROFILE_FORMAT||!parsed.record?.id||!parsed.record?.payload)throw new Error('Formato de perfil incompatível.');const existing=await storeGet(PROFILE_STORE,parsed.record.id);if(existing)throw new Error('Já existe um perfil com este identificador. Exclua o perfil local ou importe como cópia em outra versão.');parsed.record.updatedAt=nowIso();parsed.record.expiresAt=addDays(nowIso(),EXPIRATION_DAYS);await storePut(PROFILE_STORE,parsed.record);broadcast('imported',{id:parsed.record.id});return parsed.record;}
  ...
  async function recoverProfile({profileId,file,masterPassword,newPassword}){
    ...
    const kit=JSON.parse(await file.text());if(kit?.format!=='ds-teacher-recovery-1')throw new Error('Arquivo administrativo incompatível.');
