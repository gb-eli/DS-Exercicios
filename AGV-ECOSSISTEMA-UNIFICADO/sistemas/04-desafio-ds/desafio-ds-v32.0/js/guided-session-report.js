(function(){
  ...
  function eventDescription(event){
    const labels={
      ...
      evidencia_exportada:'Comprovante exportado',classroom_aberto:'Classroom aberto',
      explicacao_extra:'Explicação complementar acessada',plataforma_externa_aberta:'Ferramenta externa aberta',
      evidencia_externa_importada:'Comprovante externo importado',evidencia_externa_manual:'Comprovante registrado pelo aluno',evidencia_validada_professor:'Registro preservado de versão anterior',resultado_interno_aberto:'Registro interno iniciado',
  ...
  function summaryCards(report){
    ...
    return [
      ...
      ['Comprovantes completos',`${s.validEvidenceCount||0}/${s.externalEvidenceCount||0}`],
  ...
  function lessonBlocks(report){
    return (report.lessons||[]).map(item=>{
      ...
      const confidenceLabel=value=>({RECOGNIZED:'Comprovante reconhecido',TEACHER_VALIDATED:'Registro preservado de versão anterior',PARTIAL:'Comprovante incompleto',MANUAL:'Comprovante enviado pelo aluno',REVIEW_REQUIRED:'Falta arquivo ou link solicitado',INCOMPATIBLE:'Pertence a outra atividade'}[value]||value||'Não classificada');
      const evidences=(item.externalEvidence||[]).map(ev=>`<li><strong>${esc(ev.platform||'Ferramenta')}</strong> — <b>${esc(confidenceLabel(ev.confidence))}</b>: ${esc(ev.summary||'Sem resumo')}${ev.teacherReview?.teacher?`<br><small>Validação: ${esc(ev.teacherReview.teacher)} — ${esc(date(ev.teacherReview.at))}</small>`:''}</li>`).join('')||'<li>Nenhum comprovante externo.</li>';
      const answers=[];
      ...
      return `<section class="lesson ${esc(item.status)}">
        ...
        ${recordHtml}
        <div class="lesson-sub"><h4>Comprovantes relacionados</h4><ul>${evidences}</ul></div>
  ...
  function html(report,{forPrint=false}={}){
    ...
    return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><style>
      ...
      <h2>Ecossistema utilizado na disciplina</h2><table><thead><tr><th>Plataforma</th><th>Função pedagógica</th><th>Aulas relacionadas</th><th>Comprovantes</th></tr></thead><tbody>${toolsTable(report)}</tbody></table>
