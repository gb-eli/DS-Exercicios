(function(){
  ...
  const ECOSYSTEM_ROLES = {
    'desafio-ds':'Consolidar progresso, importar evidências e gerar o relatório final.',
    ...
    'github':'Publicar código, documentar projetos, manter versões e compartilhar evidências verificáveis.'
  };
  ...
  function requestEvidenceReview(item,index,refresh){
    if(!currentLesson||!currentProgress)return showFeedback('Abra a aula correspondente antes de validar a evidência.','warn');
    teacherAction={type:'evidence-review',evidenceId:item?.id||item?.evidenceId,index,refresh,taskId:item?.confidence?.taskId||'',previousStatus:item?.confidence?.status||''};
    openTeacherModal(teacherAction);
  }
    ...
    units.forEach((unit,unitIndex)=>{
      ...
      unitLessons.forEach(lesson=>{
        ...
        article.innerHTML=`
          ...
          <div class="guided-lesson-card-actions"><button class="btn ${progress.unlocked?'primary':'secondary'} guided-open-lesson" type="button">${progress.completed?'Revisar':progress.unlocked?'Continuar':'Solicitar autorização'}</button></div>`;
  ...
  function toolNamesAccessed(progress){
    ...
    (progress?.events||[]).forEach(event=>{
      ...
      if(type==='plataforma_externa_aberta' && event.detail){names.add(window.DS_Ecosystem?.catalog?.[event.detail]?.name||event.detail);}
  ...
  function buildDisciplineSessionReport(){
    ...
    rows.forEach(row=>{
      ...
      tasks.forEach(task=>{
        ...
        current.lessonIds.add(row.id);current.evidenceCount+=row.externalEvidence.filter(ev=>(ev.platformId===task.toolId || (ev.platform||'').toLowerCase()===(tool.name||'').toLowerCase())&&['RECOGNIZED','TEACHER_VALIDATED'].includes(ev.confidence)).length;toolMap.set(task.toolId,current);
      });
    });
    ...
    const validEvidenceCount=uniqueEvidence.filter(item=>['RECOGNIZED','TEACHER_VALIDATED'].includes(item.confidence)).length;
    const pendingEvidenceCount=uniqueEvidence.filter(item=>['PARTIAL','MANUAL','REVIEW_REQUIRED'].includes(item.confidence)).length;
  ...
  function openAutonomyGuide(){
    ...
    const panels={
      ...
      tools:`<div class="guided-guide-panel"><h3>Ferramentas com propósito</h3>${Object.entries(ECOSYSTEM_ROLES).map(([id,role])=>`<p><b>${escapeHtml(window.DS_Ecosystem?.catalog?.[id]?.name||id)}:</b> ${escapeHtml(role)}</p>`).join('')}<p>Uma plataforma externa só aparece quando faz parte da prática, do teste ou da evidência da aula.</p></div>`,
      delivery:`<div class="guided-guide-panel"><h3>Registro completo da aprendizagem</h3><ul><li>O relatório por aula registra respostas, tempo, explicações abertas, laboratório, evidências e autorização docente.</li><li>O relatório consolidado reúne toda a disciplina e pode ser salvo como PDF.</li><li>O JSON preserva os dados técnicos completos para auditoria e migração.</li><li>Abrir o Classroom não confirma a entrega: o aluno ainda precisa anexar e selecionar Entregar.</li></ul></div>`
    };
  ...
  function currentActionInstruction(){
    const actions=[
      ...
      'Utilize o laboratório interno ou a plataforma indicada e produza evidência.',
      'Resolva o desafio, explique seu raciocínio e valide o resultado.',
      'Revise, localize erros e descreva o que aprendeu.',
      'Confira o tempo, as evidências e gere o relatório antes de sair.'
    ];
  ...
  function renderEcosystemTool(container,phase){
    if(!container)return;
    window.DS_Ecosystem?.render?.(container,{lesson:currentLesson,profile,progress:currentProgress,onChange:()=>saveProgress(),findDuplicate:(item,lessonId)=>findGlobalEvidenceDuplicate(item,lessonId),onReviewEvidence:requestEvidenceReview,onInternalAction:()=>logEvent('resultado_interno_iniciado',currentLesson.id,{phase}),onEvent:(type,detail,extra)=>logEvent(type,detail,extra),onComplete:message=>{if(phase==='lab')markStepComplete(message);else saveProgress();},feedback:showFeedback});
  }
  ...
  function renderChallenge(){
    ...
    el.stepContent.innerHTML=`<span class="guided-kicker">AGORA É SUA VEZ</span><h1>Desafio da aula</h1><section class="guided-challenge-card"><p>${escapeHtml(currentLesson.challenge)}</p><ul><li>Explique a decisão.</li><li>Mostre um exemplo, teste ou evidência.</li><li>Registre como verificou o resultado.</li>${tasks.length?'<li>Relacione o resultado obtido na plataforma indicada.</li>':''}</ul></section>${tasks.length?`<div class="guided-evidence-summary"><strong>Integração com o ecossistema</strong><span>${ext.length} evidência(s) vinculada(s)</span><p>${currentLesson.requiresExternalEvidence?'A evidência externa é obrigatória para finalizar esta aula.':'A evidência externa é complementar.'}</p></div>`:''}<label class="guided-long-answer">Sua solução<textarea id="guidedChallengeAnswer" minlength="40" maxlength="1800" placeholder="Descreva sua resposta, código, análise, link ou plano...">${escapeHtml(value)}</textarea></label><button id="guidedSaveChallenge" class="btn primary" type="button">Salvar desafio</button>`;
  ...
  function renderFinish(){
    ...
    const ready=allPrevious&&timeOk&&externalOk;
    ...
    const evidenceCount=window.DS_Ecosystem?.evidenceList?.(currentProgress)?.length||0;
    el.stepContent.innerHTML=`
      ...
      <div class="guided-completion-grid">
        ...
        <div><span>Evidência externa</span><strong>${currentLesson.requiresExternalEvidence?(externalOk?`${evidenceCount} validada(s)`:'Obrigatória e pendente'):'Não obrigatória'}</strong></div>
        <div><span>Perfil</span><strong>${window.DS_ProfileManager?.isUnlocked?.()?'Protegido e criptografado':'Sessão temporária'}</strong></div>
      </div>
      ${!timeOk?`<div class="guided-time-warning"><strong>Você concluiu antes do tempo mínimo.</strong><p>Revise o conteúdo ou solicite autorização individual ao professor. A liberação ficará registrada com tempo, motivo e sessão.</p></div>`:''}
      ${!externalOk?`<div class="guided-time-warning"><strong>Falta vincular o resultado da plataforma utilizada.</strong><p>Volte ao Laboratório, importe o arquivo <code>.ds-evidence.json</code> ou registre o resultado e o teste realizado.</p><button id="guidedReturnEvidence" class="btn secondary" type="button">Voltar ao laboratório</button></div>`:''}
      <div class="guided-delivery-mode-tabs" role="tablist" aria-label="Modo de orientação da entrega"><button type="button" class="guided-delivery-mode ${delivery.mode==='quick'?'active':''}" data-mode="quick">Entrega rápida</button><button type="button" class="guided-delivery-mode ${delivery.mode==='guided'?'active':''}" data-mode="guided">Me guie</button><button type="button" class="guided-delivery-mode ${delivery.mode==='experienced'?'active':''}" data-mode="experienced">Já sei entregar</button></div>
      <div class="guided-delivery-help">${deliveryHelpHtml(delivery.mode)}</div>
      <section class="guided-export-panel"><h2>1. Exporte sua evidência</h2><p>O arquivo reúne respostas, tempo, laboratório, autorizações e resultados importados das plataformas do ecossistema.</p><p><strong>Nome esperado:</strong> ${escapeHtml(fileName('html'))}</p><div class="guided-export-actions"><button id="guidedExportHtml" class="btn primary" type="button">Exportar evidência .HTML</button><button id="guidedExportJson" class="btn secondary" type="button">Baixar dados .JSON</button><button id="guidedExportSessionPdf" class="btn secondary" type="button">Gerar PDF da sessão</button></div></section>
      <section class="guided-classroom-panel"><h2>2. Envie no Google Classroom</h2><p>Depois de exportar, abra a atividade, anexe o arquivo e selecione <strong>Entregar</strong>.</p><div class="guided-delivery-status"><span class="${delivery.exported?'done':''}">${delivery.exported?'Arquivo exportado':'Arquivo ainda não exportado'}</span><span class="${delivery.classroomOpened?'done':''}">${delivery.classroomOpened?'Classroom aberto':'Classroom ainda não aberto'}</span><span class="${delivery.declaredDelivered?'done':''}">${delivery.declaredDelivered?'Aluno declarou a entrega':'Entrega não confirmada'}</span></div><button id="guidedOpenClassroom" class="btn secondary" type="button">Abrir Classroom</button></section>
      ${window.DS_CodeCenter?.has?.(currentLesson)?`<section class="guided-code-next"><h2>Código da aula</h2><p>Visualize, copie, teste ou baixe o projeto, com comandos e solução de erros sob demanda.</p><div><button id="guidedOpenCodeCenter" class="btn secondary" type="button">Abrir código e instruções</button><button id="guidedDownloadCodeProject" class="btn ghost" type="button">Baixar projeto .ZIP</button></div></section>`:''}
      ${['intro-programacao','front-end','front-end-sub','programacao-ds','mobile-1'].includes(profile?.disciplineKey)?`<section class="guided-code-next"><h2>Publicação e continuidade</h2><p>Teste antes de publicar. Depois registre a URL ou importe a evidência na aula.</p><div><button id="guidedOpenGithub" class="btn ghost" type="button">Abrir GitHub</button><button id="guidedOpenVscode" class="btn ghost" type="button">Abrir VS Code Web</button></div></section>`:''}
    ...
    $('#guidedExportHtml').addEventListener('click',()=>exportEvidence('html'));$('#guidedExportJson').addEventListener('click',()=>exportEvidence('json'));$('#guidedExportSessionPdf')?.addEventListener('click',()=>exportDisciplineSession('pdf'));$('#guidedOpenClassroom').addEventListener('click',openClassroom);
    $('#guidedOpenCodeCenter')?.addEventListener('click',openCodeCenter);$('#guidedDownloadCodeProject')?.addEventListener('click',()=>{window.DS_CodeCenter?.downloadProject?.(currentLesson);logEvent('codigo_projeto_baixado_finalizacao',currentLesson.id);});
    $('#guidedOpenGithub')?.addEventListener('click',()=>window.open('https://github.com/','_blank','noopener,noreferrer'));$('#guidedOpenVscode')?.addEventListener('click',()=>window.open('https://vscode.dev/','_blank','noopener,noreferrer'));
    $('#guidedFinalizeLesson').addEventListener('click',()=>{currentProgress.completed=true;currentProgress.completedAt=new Date().toISOString();currentProgress.endedBy=currentProgress.teacherOverride?'concluida_com_liberacao':'concluida';saveProgress();logEvent('aula_concluida',currentLesson.title,{deliveryDeclared:!!delivery.declaredDelivered,externalEvidence:evidenceCount,teacherOverride:!!currentProgress.teacherOverride});alert('Aula concluída. Confira a entrega diretamente no Google Classroom.');returnToCatalog('aula concluída');});
  }
  ...
  function exportEvidence(format){
    ...
    const externalRows=(report.externalEvidence||[]).map(item=>`<li><strong>${escapeHtml(item.platform?.name||item.platform?.id)}</strong> — ${escapeHtml(item.activity?.title||'Atividade')}<br>${escapeHtml(item.result?.summary||'Sem resumo')}${item.source?.url?`<br>Link: ${escapeHtml(item.source.url)}`:''}</li>`).join('')||'<li>Nenhuma evidência externa vinculada.</li>';
  ...
  function buildReportData(){
    ...
    return {app:'Desafio DS — Modo Guiado',version:DATA.version,generatedAt:new Date().toISOString(),student:{name:profile.name,class:courseData().label,supportResources:(profile.supports||[]).map(x=>SUPPORT_LABELS[x]||x)},lesson:{id:currentLesson.id,number:displayNumber(currentLesson),legacyNumber:currentLesson.legacyNumber||currentLesson.number,title:currentLesson.title,module:currentLesson.module,unit:{id:unit?.id||'',title:unit?.title||''},discipline:disciplineData().label,objective:currentLesson.objective,minimumActiveSeconds:currentLesson.minimumActiveSeconds||DATA.minimumActiveSeconds,expectedMinutes:currentLesson.expectedMinutes||25,requiredPlatforms:(currentLesson.platformTasks||[]).map(x=>x.toolId)},session:{id:currentProgress.sessionId,startedAt:currentProgress.startedAt,activeSeconds:currentProgress.activeSeconds,activeTime:fmt(currentProgress.activeSeconds),idleEpisodes:currentProgress.idleEpisodes,teacherOverride:currentProgress.teacherOverride,schedule},progress:{completedSteps:currentProgress.completedSteps,extraUsed:currentProgress.extraUsed,exports:currentProgress.exports,completed:!!currentProgress.completed},externalEvidence:window.DS_Ecosystem?.evidenceList?.(currentProgress)||[],terms:window.DS_Terms?.evidence?.()||{status:'não validado'},permissions:window.DS_ProfileManager?.getPath?.('permissions.history',[])?.slice(-20)||[],assessment:{method:'proficiência, evidências e critérios do professor',xpInfluencesGrade:false,cosmeticsInfluenceGrade:false},answers:currentProgress.answers,labData:currentProgress.labData,events:currentProgress.events};
  }
  ...
  function openTeacherModal(action){
    teacherAction=action;const type=typeof action==='string'?action:action?.type;el.teacherName.value='';if(el.teacherPassword)el.teacherPassword.value='';el.teacherNote.value='';el.teacherStatus.textContent='';
    const title=$('#guidedTeacherTitle'),intro=title?.nextElementSibling;
    if(title)title.textContent=type==='evidence-review'?'Validar evidência importada':type==='release'?'Autorizar conclusão antecipada':'Ação do professor';
    if(intro)intro.textContent=type==='evidence-review'?'A autorização será vinculada somente a esta evidência, aula, atividade e perfil.':type==='release'?'A autorização vale somente para a conclusão antecipada desta aula e sessão.':'O EduAuth gera uma autorização limitada e registra a ação.';
    const hideDetails=type==='panel';
    el.teacherReason.previousElementSibling?.classList.toggle('hidden',hideDetails);
    el.teacherReason.classList.toggle('hidden',hideDetails);
    el.teacherNote.previousElementSibling?.classList.toggle('hidden',hideDetails);
    el.teacherNote.classList.toggle('hidden',hideDetails);
  ...
  async function confirmTeacherAction(){
    ...
    const action=teacherAction,type=typeof action==='string'?action:action?.type;
    if(type==='release'){
    ...
    }else if(type==='evidence-review'){
      const note=el.teacherNote.value.trim();if(note.length<10)return setTeacherStatus('Registre uma justificativa curta para a validação.','warn');
      closeTeacherModal();
      try{
        const item=(currentProgress.externalEvidence||[]).find(row=>(row.id||row.evidenceId)===action.evidenceId);if(!item)throw new Error('A evidência não está mais disponível nesta aula.');
        const taskId=action.taskId||item.confidence?.taskId;if(!taskId)throw new Error('Não foi possível identificar a atividade da evidência.');
        await window.DS_EduAuth.authorize({actionId:'teacher-maintenance',classId:profile.courseKey,subjectId:`${profile.courseKey}:${profile.disciplineKey}`,lessonId:currentLesson.id,activityId:'manutencao-perfil',resourceId:`evidence:${item.id||item.evidenceId}`,teacher:name,reason:note});
        item.review={status:'VALIDATED',teacher:name,reason:el.teacherReason.value,note,at:new Date().toISOString(),lessonId:currentLesson.id,taskId,authorization:'EDUAUTH_SIGNED'};
        item.confidence={status:'TEACHER_VALIDATED',label:'Validada pelo professor',counts:true,reasons:['Autorização docente vinculada à aula e à atividade.'],lessonId:currentLesson.id,taskId,checkedAt:new Date().toISOString()};
        logEvent('evidencia_validada_professor',item.platform?.id||'',{teacher:name,evidenceId:item.id||item.evidenceId,taskId,previousStatus:action.previousStatus});saveProgress();showFeedback('Evidência validada para esta aula e atividade.','ok');action.refresh?.();if(evidenceOk()&&currentStep===6)markStepComplete('evidência validada pelo professor');else saveProgress();
      }catch(error){showFeedback(error?.message||'A evidência não foi validada.','warn');}
    }else{
      closeTeacherModal();
      try{await window.DS_EduAuth.authorize({actionId:'teacher-audit',classId:profile.courseKey,subjectId:`${profile.courseKey}:${profile.disciplineKey}`,lessonId:currentLesson?.id||'painel-professor',activityId:'principal',resourceId:profile.disciplineKey,teacher:name,reason:'Auditoria pedagógica'});openTeacherPanel(name);}catch(error){showFeedback(error?.message||'Auditoria não autorizada.','warn');}
    }
  }
