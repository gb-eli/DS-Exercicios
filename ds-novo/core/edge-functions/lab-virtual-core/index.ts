...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==="complete"){
      if(["xp","coins","credits"].some(k=>Object.prototype.hasOwnProperty.call(body,k)))return json({error:"client_reward_forbidden"},400);
      ...
      const openedAt=Date.parse(tool.metadata?.openedAt||tool.updated_at||""),elapsed=Number.isFinite(openedAt)?Math.floor((Date.now()-openedAt)/1000):0;
      if(minSeconds>0&&elapsed<minSeconds)return json({error:"minimum_activity_time",required_seconds:minSeconds,elapsed_seconds:elapsed},409);
