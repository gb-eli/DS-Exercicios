window.AppAuth = (() => {
  ...
  function storageGet(name) { try { return localStorage.getItem(name); } catch { showStorageFailure(); return null; } }
  function storageSet(name, value) { try { localStorage.setItem(name, value); return true; } catch { showStorageFailure(); return false; } }
  function storageRemove(name) { try { localStorage.removeItem(name); return true; } catch { showStorageFailure(); return false; } }
  ...
  function logs() {
    try { return JSON.parse(storageGet(key('logs'))) || []; }
    catch (error) { return []; }
  }
  ...
  function log(action, details = {}) {
    ...
    storageSet(key('logs'), JSON.stringify(list.slice(-MAX_LOGS)));
  }
    ...
    return `
      <div id="authGate" class="auth-gate" role="dialog" aria-modal="true" aria-labelledby="authDialogTitle" aria-describedby="authDialogDescription">
        ...
        <article class="auth-panel card">
          <div class="auth-brand"><span class="auth-logo">&lt;/&gt;</span><div><small>1º DS · ${scope() === 'professor' ? 'Professor' : 'Aluno'}</small><h1 id="authDialogTitle">Introdução à Programação</h1><p id="authDialogDescription">Sessão local criptografada neste navegador por até ${SESSION_DAYS} dias.</p></div></div>
  ...
  function progressFor(exercise) {
    ...
    try {
      ...
      if (saved.validated) return { percentage: Math.max(95, percentage), label: 'Pronto para concluir', status: 'Pronto para concluir', tone: 'warning', hasProgress: true };
  ...
  async function openExercise(index, options={}) {
    ...
    await saveSession({lastExercise:window.EXERCICIOS[index].numero}); document.body.classList.remove('dashboard-mode'); $('#appDashboard').hidden=true; document.querySelector('main').hidden=false;
