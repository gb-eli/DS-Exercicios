window.Utils = (() => {
  ...
  function semanticCssMetrics(actual, exercise) {
    ...
    if (config.layout) {
      ...
      if (config.layout.exigirColunasFlexiveis) {
        const ok = candidates.some(entry => {
          ...
          return /repeat|minmax|auto-fit|auto-fill|wrap|\bfr\b|%|calc\(|clamp\(/.test(value);
        });
    ...
    if (config.larguraResponsiva) {
      const ok = entries.some(entry => {
        ...
        return /min\(|max\(|%|vw|calc\(|clamp\(/.test(width) || maxWidth || /minmax|auto-fit|auto-fill|\bfr\b|%|calc\(|clamp\(/.test(columns);
      });
  ...
  function semanticCssDiagnostics(actual, exercise) {
    ...
    const advisory = new Set(exercise?.validacao?.requisitosRecomendados || []);
    const issues = [...metrics.parsed.errors];
    metrics.checks.filter(check => !check.ok && !advisory.has(check.key)).forEach(check => issues.push({ title: check.title, detail: check.detail }));
  ...
  function semanticCssCompleteness(actual, exercise) {
    const metrics = semanticCssMetrics(actual, exercise);
    const advisory = new Set(exercise?.validacao?.requisitosRecomendados || []);
    const blocking = metrics.checks.filter(check => !advisory.has(check.key));
    const total = Math.max(1, blocking.length);
    const matched = blocking.filter(check => check.ok).length;
  ...
  function semanticHtmlDiagnostics(actual, exercise, file = 'html') {
    ...
    const strictExactTags = new Set(config.tagsExatasEstritas || Object.keys(config.tagsExatas || {}));
    Object.entries(config.tagsExatas || {}).forEach(([tag, expected]) => {
      const count = doc.querySelectorAll(tag).length;
      const strict = strictExactTags.has(tag);
      const invalid = strict ? count !== expected : count < expected;
      if (invalid) issues.push({
        title: `Quantidade de <${tag}>`,
        detail: strict
          ? `Use exatamente ${expected} elemento(s) <${tag}>. Foram encontrados ${count}.`
          : `Use pelo menos ${expected} elemento(s) <${tag}>. Foram encontrados ${count}. Elementos extras válidos não impedem a atividade.`
      });
    });
  ...
  function semanticHtmlCompleteness(actual, exercise, file = 'html') {
    ...
    const strictExactTags = new Set(config.tagsExatasEstritas || Object.keys(config.tagsExatas || {}));
    Object.entries(config.tagsExatas || {}).forEach(([tag, expected]) => metric(strictExactTags.has(tag) ? doc.querySelectorAll(tag).length === expected : doc.querySelectorAll(tag).length >= expected));
  ...
  function stripJavaScriptNonCode(code) {
    return removeBom(code)
      .replace(/\/\*[\s\S]*?\*\//g, ' ')
      .replace(/(^|[^:])\/\/.*$/gm, '$1 ')
      .replace(/`(?:\\.|[^`])*`/g, '``')
      .replace(/"(?:\\.|[^"\\])*"/g, '""')
  ...
  function semanticJsMetrics(actual, exercise) {
    const config = exercise?.validacao?.jsConceitual || {};
    const parsed = tokenizeJavaScript(actual, { strictDeclarations: false });
    const syntaxIssue = checkJavaScriptSyntax(actual);
    const structural = stripJavaScriptNonCode(actual);
    const tokens = parsed.tokens;
    const identifiers = tokens.filter(token => token.type === 'identifier').map(token => token.value);
    const operators = tokens.filter(token => token.type === 'operator').map(token => token.value);
    const checks = [];
    const add = (key, ok, title, detail) => checks.push({ key, ok: Boolean(ok), title, detail });
    const countKeyword = keyword => identifiers.filter(value => value === keyword).length;

    if (config.declaracoesMinimas) {
      const declarations = tokens.filter(token => token.type === 'identifier' && token.value === 'declaration').length;
      add('declarations', declarations >= config.declaracoesMinimas, 'Declarações insuficientes', `Crie pelo menos ${config.declaracoesMinimas} variável(is) ou constante(s) para organizar os dados.`);
    }
    if (config.funcoesMinimas) {
      const functions = findFunctions(actual);
      add('functions', functions.length >= config.funcoesMinimas, 'Função não identificada', `Crie pelo menos ${config.funcoesMinimas} função(ões). O nome pode ser diferente do exemplo.`);
    }
    if (config.condicoesMinimas) {
      const conditions = countKeyword('if') + countKeyword('switch');
      add('conditions', conditions >= config.condicoesMinimas, 'Condição não identificada', `Use pelo menos ${config.condicoesMinimas} estrutura(s) de decisão, como if/else ou switch quando o enunciado permitir.`);
    }
    if (config.lacosMinimos) {
      const loops = countKeyword('for') + countKeyword('while') + countKeyword('do');
      add('loops', loops >= config.lacosMinimos, 'Repetição não identificada', `Use pelo menos ${config.lacosMinimos} estrutura(s) de repetição adequada(s) ao problema.`);
    }
    if (config.retornosMinimos) {
      add('returns', countKeyword('return') >= config.retornosMinimos, 'Retorno não identificado', 'A função precisa devolver um resultado com return.');
    }
    if (config.acessosDomMinimos) {
      const domMatches = structural.match(/\b(?:getElementById|querySelector|querySelectorAll)\s*\(/g) || [];
      add('dom', domMatches.length >= config.acessosDomMinimos, 'Acesso ao DOM insuficiente', `Localize pelo menos ${config.acessosDomMinimos} elemento(s) da página usando uma API do DOM.`);
    }
    if (config.eventosMinimos) {
      const eventMatches = structural.match(/\baddEventListener\s*\(|\bon(?:click|input|change|submit|keydown)\s*=/g) || [];
      add('events', eventMatches.length >= config.eventosMinimos, 'Evento não identificado', `Implemente pelo menos ${config.eventosMinimos} evento(s) para produzir a interação pedida.`);
    }
    (config.idsObrigatorios || []).forEach(id => {
      const ids = new Set(findDomIds(actual));
      add(`id:${id}`, ids.has(id), `Elemento #${id} não acessado`, `A atividade exige interação com o elemento #${id}. A forma de acesso pode variar entre getElementById() e querySelector().`);
    });
    (config.operadoresGrupos || []).forEach((group, index) => {
      const alternatives = Array.isArray(group) ? group : [group];
      add(`operator:${index}`, alternatives.some(op => operators.includes(op)), 'Operador necessário não identificado', `Use um operador adequado deste grupo: ${alternatives.join(' ou ')}.`);
    });
    (config.chamadasObrigatorias || []).forEach(name => {
      const pattern = new RegExp(`\\b${String(name).replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*\\(`);
      add(`call:${name}`, pattern.test(structural), `Chamada ${name}() não identificada`, `Use ${name}() porque ela faz parte do objetivo desta atividade.`);
    });

    return { parsed, syntaxIssue, checks };
  }

  function semanticJsDiagnostics(actual, exercise) {
    const metrics = semanticJsMetrics(actual, exercise);
    const issues = [...metrics.parsed.errors];
    if (metrics.syntaxIssue) issues.unshift(metrics.syntaxIssue);
    metrics.checks.filter(check => !check.ok).forEach(check => issues.push({ title: check.title, detail: check.detail }));
    if (hasSmartQuotes(actual)) issues.unshift({ title: 'Aspas tipográficas encontradas', detail: 'Use aspas normais do teclado no JavaScript.' });
    if (hasPastedLineNumbers(actual)) issues.unshift({ title: 'Números de linha no código', detail: 'Remova números de linha que não fazem parte do programa.' });
    return {
      ok: issues.length === 0,
      summary: issues[0]?.detail || 'A solução atende aos requisitos conceituais do exercício.',
      issues,
      suggestions: [
        'Nomes de variáveis e funções podem ser diferentes quando o enunciado não exigir um nome específico.',
        'Função tradicional ou arrow function podem ser aceitas quando produzirem o mesmo comportamento.',
        'Espaços, indentação, comentários e ponto e vírgula opcional não fazem parte da correção conceitual.',
        'Execute e teste todos os cenários pedidos; presença de palavras no código não substitui comportamento.'
      ],
      firstLine: issues.find(item => item.line)?.line || null
    };
  }

  function semanticJsCompleteness(actual, exercise) {
    const metrics = semanticJsMetrics(actual, exercise);
    const total = Math.max(1, metrics.checks.length);
    const matched = metrics.checks.filter(check => check.ok).length;
    const syntaxOk = !metrics.syntaxIssue && metrics.parsed.errors.length === 0;
    let percentage = metrics.checks.length ? Math.round((matched / total) * 100) : (syntaxOk ? 70 : 25);
    if (!syntaxOk) percentage = Math.min(percentage, 88);
    const validation = semanticJsDiagnostics(actual, exercise);
    if (validation.ok) percentage = 100;
    return { percentage, remaining: 100 - percentage, status: !syntaxOk ? 'Revise a sintaxe' : percentage < 35 ? 'Primeiros passos' : percentage < 75 ? 'Em desenvolvimento' : percentage < 100 ? 'Quase pronto' : 'Arquivo validado', state: !syntaxOk ? 'error' : percentage < 35 ? 'starting' : percentage < 75 ? 'building' : percentage < 100 ? 'almost' : 'correct', syntaxOk, message: 'Progresso baseado nos requisitos conceituais do problema, não em cópia textual do gabarito.' };
  }
  ...
  function validateCode(file, actual, expected, exercise) {
    ...
    if (file === 'js' && exercise?.validacao?.tipo === 'js-conceitual') return semanticJsDiagnostics(cleanActual, exercise);
  ...
  function analyzeCompleteness(file, actual, expected, exercise) {
    ...
    } else if (file === 'js' && exercise?.validacao?.tipo === 'js-conceitual') {
      return semanticJsCompleteness(cleanActual, exercise);
    } else {
      const strictDeclarations = Boolean(exercise?.validacao?.strictDeclarations);
  ...
  function tipsHtml(exercise) {
    ...
    return `<ul class="drawer-list">${general.map(item => `<li>${escapeHtml(item)}</li>`).join('')}</ul>`;
  }
