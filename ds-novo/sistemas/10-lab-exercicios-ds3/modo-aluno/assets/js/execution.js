(function (global) {
  ...
  function requirements(exercise, file) {
    ...
    // Exercício 02 possui HTML presente: CSS precisa carregar corretamente; JS precisa provar a interação do formulário.
    if (number === 2) {
      if (file === 'js') {
        req.push(
    ...
    // Em exercícios com vários arquivos, os cenários de comportamento são testados uma vez no arquivo principal.
    // Os demais arquivos precisam apenas abrir o preview sem erro. Isso evita bloquear 100% por repetir a mesma interação.
    if (file !== primary) return req;
