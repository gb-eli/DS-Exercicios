...
const renderDossier = (challenge, mode) => {
  ...
  return `<details class="mission-dossier" ${detailed ? 'open' : ''}>
    ...
    ${challenge.reference ? `<a class="mission-reference" href="${escapeHtml(safeExternalUrl(challenge.reference.url))}" target="_blank" rel="noopener noreferrer">↗ REFERÊNCIA REAL: ${escapeHtml(challenge.reference.label)}</a>` : ''}
  </details>`;
};
...
const renderAuthorizedScope = (challenge) => `<section class="authorized-scope">
  <div><span>AMBIENTE AUTORIZADO</span><strong>CTF DS · sandbox local · dados fictícios</strong></div>
  <div class="scope-columns"><div><b>PERMITIDO</b><p>Investigar a missão, usar as ferramentas locais, editar o código preparado, analisar evidências fictícias e registrar o aprendizado.</p></div><div><b>PROIBIDO</b><p>Testar sites, redes, contas, dispositivos, credenciais ou pessoas reais; executar varreduras externas; distribuir conteúdo malicioso.</p></div></div>
  <small>Objetivo defensivo: ${escapeHtml(challenge.defense || 'compreender o risco, prevenir e mitigar a falha demonstrada.')}</small>
</section>`;
...
export const renderChallengeModal = (challenge, profile, result = null, showHint = false, tutorialVisible = Boolean(challenge.tutorial && profile.settings?.tutorialEnabled !== false)) => {
  ...
  return `<div class="modal-layer mission-modal-layer mission-theme-${escapeHtml(challenge.theme)}" style="${themeStyle(challenge.theme)}" data-modal-close="true">
    <section class="challenge-modal panel" data-local-flag="${escapeHtml(inspectValue)}" role="dialog" aria-modal="true" aria-labelledby="challenge-title">
      ...
      ${renderAuthorizedScope(challenge)}
      ...
      <div class="mission-brief-grid">
        ...
        <section class="objective-panel goal-panel"><span>OBJETIVO DA MISSÃO</span><div class="challenge-prompt">${escapeHtml(resolvedPrompt)}</div></section>
      </div>
      ...
      <p class="safety-note"><strong>Finalidade educacional:</strong> esta atividade ocorre em ambiente autorizado, local e simulado. Aprenda a identificar riscos, corrigir falhas e proteger sistemas. Não reproduza técnicas fora do escopo definido pelo professor.</p>
    </section>
  </div>`;
};
