...
import { verifyChallengeAnswer } from './security/challenge-verifier.js';
...
const RELEASE_ID = 'ctfds-2.3.0-security-wallet';
...
const openToolsDrawer = (toolId = activeTool) => {
  if (activeChallengeUI.id && activeChallengeUI.session) activeChallengeUI.session.toolRuns = (activeChallengeUI.session.toolRuns || 0) + 1;
...
const openChallenge = (id, result = null, showHint = false, tutorialVisible = null, options = {}) => {
  ...
  activeChallengeUI = { id, result, showHint, tutorialVisible: tutorialVisible ?? Boolean(challenge.tutorial && getState().profile.settings?.tutorialEnabled !== false), session: priorSession || { openedAt: Date.now(), toolRuns: 0, localTests: 0, evidenceClicks: 0, submissions: 0 } };
...
const rewardChallenge = (challenge, attemptNumber, verification = {}) => {
  ...
  return { stars, rewarded: true, profile, bonusCoins, dailyBonus, levelUp: profile.level > previousLevel, reviewRequired };
};
