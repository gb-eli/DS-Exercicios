(function(){
  ...
  function continueAsDiagnostic(){
    ...
    state.playStyle='diagnostico';state.playStyleLabel='Diagnóstico completo';state.score=0;state.expectedScore=0;state.maxLives=null;state.lives=999;state.combo=0;state.power=0;state.shield=0;state.multiplierNext=1;state.extraCards=0;activePower=null;state.overtime=false;state.competitiveFailureReason='';
  ...
  function openResultClassroom(){
    if(el.resultClassroomStatus){el.resultClassroomStatus.textContent='Classroom aberto — entrega não confirmada';el.resultClassroomStatus.classList.add('done');}
