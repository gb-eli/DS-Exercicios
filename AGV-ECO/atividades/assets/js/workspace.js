...
function defaultFiles(subjectSlug){
  ...
  return defs.map(f=>{
    const starter=safeDefaults.find(x=>x.filename===f.filename)?.content||'';
...
async function loadStudentSupport(){
  ...
  if(guidance){
    ...
    if(base.length) chunks.push(`<div class="support-note"><strong>Arquivos-base liberados</strong><span>${base.join(', ')}. O starter é aplicado na primeira criação dos arquivos e nunca sobrescreve trabalho já salvo.</span></div>`);
...
export async function mountWorkspace({profile,exercise,subject}){
  ...
  state.active=null; state.files=[]; state.lastSavedContent=new Map(); state.remoteEdit=false; state.teacherEditing=false; state.supportRelease=null;
