(()=>{
  ...
  const state={session:null,overview:null,students:[],classes:[],classSubjects:[],releases:[],selectedStudent:null,activities:[],reference:null,currentActivity:null,staffRole:'teacher'};
  ...
  function releaseFor(exerciseId,classId){return state.releases.find(x=>String(x.exercise_id)===String(exerciseId)&&String(x.class_id||'')===String(classId)&&!x.student_id)||null}
  ...
  function renderReleaseClasses(){const sel=$('release-class');sel.innerHTML=state.classes.map(c=>`<option value="${esc(c.id)}">${esc(c.name||c.code)}</option>`).join('');renderReleaseExercises()}
  function renderReleaseExercises(){const cid=$('release-class').value,items=exercisesForReleaseClass(cid),sel=$('release-exercise');sel.innerHTML=items.map(e=>`<option value="${esc(e.id)}">${String(e.exercise_number||'').padStart(2,'0')} — ${esc(e.title)}</option>`).join('');fillReleaseForm()}
  function fillReleaseForm(){const cid=$('release-class').value,eid=$('release-exercise').value,r=releaseFor(eid,cid);$('release-enabled').checked=r?r.enabled!==false:true;$('release-at').value=toLocalInput(r?.release_at);$('lock-at').value=toLocalInput(r?.lock_at);$('release-html').checked=!!r?.allow_html_base;$('release-css').checked=!!r?.allow_css_base;$('release-js').checked=!!r?.allow_js_base;$('release-hints').checked=!!r?.allow_extra_hints;$('release-guided').checked=!!r?.allow_guided_support;const now=Date.now();$('release-state').textContent=!r?'Sem override':!r.enabled?'Bloqueada':r.release_at&&Date.parse(r.release_at)>now?'Programada':r.lock_at&&Date.parse(r.lock_at)<=now?'Encerrada':'Liberada'}
  async function saveTeacherRelease(){const msg=$('release-message'),cid=$('release-class').value,eid=$('release-exercise').value;if(!cid||!eid){msg.textContent='Selecione turma e atividade.';msg.classList.add('error');return}const ra=$('release-at').value,la=$('lock-at').value;if(ra&&la&&new Date(la)<=new Date(ra)){msg.textContent='O encerramento precisa ser posterior à abertura.';msg.classList.add('error');return}const body={exercise_id:eid,class_id:cid,student_id:null,enabled:$('release-enabled').checked,release_at:fromLocalInput(ra),lock_at:fromLocalInput(la),allow_html_base:$('release-html').checked,allow_css_base:$('release-css').checked,allow_js_base:$('release-js').checked,allow_extra_hints:$('release-hints').checked,allow_guided_support:$('release-guided').checked,created_by:state.session?.user?.id||null,updated_at:new Date().toISOString()},old=releaseFor(eid,cid);try{msg.classList.remove('error');msg.textContent='Salvando…';let out;if(old?.id)out=await api(`/rest/v1/exercise_releases?id=eq.${encodeURIComponent(old.id)}`,{method:'PATCH',body});else out=await api('/rest/v1/exercise_releases',{method:'POST',body});await loadReleaseData();fillReleaseForm();msg.textContent='Liberação salva e registrada na auditoria.'}catch(e){msg.textContent=e.message;msg.classList.add('error')}}
  ...
  async function loadStaff(){
    ...
    const raw=await invoke(cfg.staffFunction,{action:'overview'});state.overview=normalizedOverview(raw);state.classes=state.overview.classes;state.students=state.overview.students;await loadReleaseData();renderClasses();renderStudents();renderReleaseClasses();
  ...
  $('student-search').addEventListener('input',renderStudents);$('class-filter').addEventListener('change',renderStudents);$('approve-activity').addEventListener('click',()=>reviewCurrent('approved'));$('request-changes').addEventListener('click',()=>reviewCurrent('changes_requested'));$('teacher-release-toggle')?.addEventListener('click',()=>$('teacher-release-panel')?.classList.toggle('hidden'));$('release-class')?.addEventListener('change',renderReleaseExercises);$('release-exercise')?.addEventListener('change',fillReleaseForm);$('release-save')?.addEventListener('click',saveTeacherRelease);$('teacher-import-toggle')?.addEventListener('click',()=>$('teacher-import-panel')?.classList.toggle('hidden'));$('teacher-import-run')?.addEventListener('click',importTeacherReferences);
  boot();
})();
