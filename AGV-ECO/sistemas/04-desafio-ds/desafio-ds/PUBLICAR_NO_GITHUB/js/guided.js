(function(){
  ...
  function renderMobileQualityTool(container,phase){
    const saved=currentProgress.labData?.[`${phase}-quality`]||{large:false,contrast:false,reduced:false,slow:false};
    container.innerHTML=`<div class="guided-quality-lab"><div class="guided-quality-controls"><label><input data-q="large" type="checkbox" ${saved.large?'checked':''}> Texto ampliado</label><label><input data-q="contrast" type="checkbox" ${saved.contrast?'checked':''}> Alto contraste</label><label><input data-q="reduced" type="checkbox" ${saved.reduced?'checked':''}> Movimento reduzido</label><label><input data-q="slow" type="checkbox" ${saved.slow?'checked':''}> Rede/hardware limitado</label><button data-save-quality class="btn primary" type="button">Salvar revisão de qualidade</button></div><div class="guided-quality-preview"><div class="guided-quality-loader"></div><h3>Atividade mobile</h3><p>Esta tela precisa continuar legível e funcional em diferentes condições.</p><button type="button">Ação principal</button></div></div>`;
    const read=()=>Object.fromEntries($$('[data-q]',container).map(n=>[n.dataset.q,n.checked]));const paint=()=>{const d=read(),preview=container.querySelector('.guided-quality-preview');Object.entries(d).forEach(([k,v])=>preview.classList.toggle(`q-${k}`,v));};container.addEventListener('change',paint);paint();container.querySelector('[data-save-quality]').onclick=()=>{const d=read();currentProgress.labData[`${phase}-quality`]=d;saveProgress();logEvent('qualidade_mobile','preferências testadas',{phase,...d});markStepComplete('revisão de acessibilidade e desempenho salva');};
  }
  ...
  function renderFinish(){
    ...
    el.stepContent.innerHTML=`
      ...
      ${['intro-programacao','front-end','front-end-sub','programacao-ds','mobile-1'].includes(profile?.disciplineKey)?`<section class="guided-code-next"><h2>Projeto de programação</h2><p>Quando a atividade solicitar código, organize os arquivos, leia o README e publique somente depois de testar.</p><div><button id="guidedOpenGithub" class="btn ghost" type="button">Abrir GitHub</button><button id="guidedOpenVscode" class="btn ghost" type="button">Abrir VS Code Web</button></div></section>`:''}
