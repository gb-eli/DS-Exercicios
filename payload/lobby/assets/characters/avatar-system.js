...
export function createAvatarSystem({THREE,spriteLabel,emojiSprite,quality='medium',mobile=false,hardware=4,diagnostics=globalThis.__agvLobbyDiag}={}){
  ...
  async function init(){
    ...
    try{rigApi=await import('../rigged-avatar.js?v=14.10.8.55');rigAsset=await rigApi.loadRiggedAvatarAsset(THREE,{timeout:3000});diagnostics?.record?.('avatar_system_ready',{mode:'rigged-glb-v2',clips:rigAsset.clips?.map(c=>c.name)||[]});}
