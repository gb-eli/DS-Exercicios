  ...
  function waitingRoom(d){
    ...
    if(selected){
      ...
      const assignments=team.map(m=>{
        ...
        const accepted=!!m.role_id&&!!m.role_selected_at;return `<div class="leader-member-row"><div><strong>${esc(m.student?.full_name||'Aluno')}${isLeader?' · Líder':''} ${accepted?'<span class="status-pill good">Confirmado</span>':m.role_id?'<span class="status-pill warn">CONFIRMAÇÃO PENDENTE</span>':''}</strong><small>${isSelf?'Você • ':''}${esc(roleById.get(String(m.role_id||''))?.name||'Sem área definida')}</small></div><select data-leader-role="${m.student_id}" ${!me?.is_leader||d.session.status!=='waiting_room'?'disabled':''}><option value="">Definir área…</option>${opts}</select>${me?.is_leader&&!isSelf&&d.session.status==='waiting_room'?`<button class="button ghost small" data-leader-kick="${m.student_id}" type="button">Solicitar troca</button>`:'<span></span>'}</div>`;
      }).join('');
      const leaderPanel=me?.is_leader?`<section class="panel section-panel leader-console" style="--guild-accent:${esc(room.accent_color||'#22d3ee')}"><div class="section-head"><div><p class="eyebrow">Gestão da equipe</p><h2>Dados da empresa e funções</h2><p>Complete os dados da empresa e distribua as funções antes do início.</p></div><span class="status-pill good">Líder</span></div><form id="room-settings-form" class="room-settings-grid professional-company-form"><input type="hidden" name="name" value="${esc(room.name||'')}"><input type="hidden" name="theme_key" value="${esc(room.theme_key||'corporate')}"><input type="hidden" name="accent_color" value="${esc(room.accent_color||'#22d3ee')}"><input type="hidden" name="mascot_key" value="${esc(room.mascot_key||'robot')}"><label>Nome da empresa<input name="company_name" maxlength="80" value="${esc(room.company_name||'')}" placeholder="Nome definido pela equipe"></label><label>CNPJ fictício<input name="company_cnpj" maxlength="24" value="${esc(room.company_cnpj||'')}" placeholder="00.000.000/0001-00"></label><label>Cidade<input name="company_city" maxlength="80" value="${esc(room.company_city||'')}" placeholder="Paranaguá - PR"></label><label>Telefone fictício<input name="company_phone" maxlength="30" value="${esc(room.company_phone||'')}" placeholder="(41) 99999-9999"></label><div class="guild-emblem-editor"><div id="guild-emblem-preview">${guildMark(room,'large')}</div><label>Marca / emblema<input id="guild-emblem-input" type="file" accept="image/png,image/jpeg,image/webp"><small>Opcional · PNG, JPEG ou WEBP · máximo 320 KB</small></label>${room.emblem_data_url?'<button class="button ghost small" data-remove-emblem type="button">Remover imagem</button>':''}</div><button class="button primary room-save" type="submit" ${d.session.status!=='waiting_room'?'disabled':''}>Salvar dados da empresa</button></form><div class="section-head compact-head"><div><p class="eyebrow">Funções exclusivas</p><h3>Definir cargo de cada integrante</h3><p>Cada função define as atividades e responsabilidades de cada integrante.</p></div></div><div class="leader-member-list">${assignments}</div></section>`:'';
    ...
    const steps=[['1','Entrar no lobby',true],['2','Escolher equipe',!!selected],['3','Definir liderança',!!myRoom?.leader_id],['4','Escolher funções',!!myRoom?.leader_id&&rolesReady&&identityReady],['5','Aguardar professor',d.session.status==='locked'||d.session.status==='running']];
    return `<section class="evaluation-status-strip"><div><i></i><span>LOBBY DA AVALIAÇÃO</span><strong>${inRooms}/${total||0} alunos em equipes</strong></div><div><span>SESSÃO</span><strong>SINCRONIZADA</strong></div><div><span>DISCIPLINA</span><strong>${esc(d.session.subject_name||'Prova coletiva')}</strong></div><div><span>STATUS DA EQUIPE</span><strong>${selected?'EQUIPE DEFINIDA':'AGUARDANDO EQUIPE'}</strong></div></section><section class="lobby-journey" aria-label="Etapas do pré-lobby">${steps.map(([number,label,done],index)=>`<div class="${done?'done':''} ${!done&&steps.slice(0,index).every(step=>step[2])?'current':''}"><span>${done?'✓':number}</span><small>${label}</small></div>`).join('')}</section><section class="panel lobby-master"><div class="section-head"><div><p class="eyebrow">Organização • ${Number(d.session.lobby_duration_minutes||15)} min</p><h2>Formação das equipes</h2><p>Veja a turma, escolha uma equipe e organize liderança, empresa e funções. Depois do início, a composição fica bloqueada.</p></div><div id="lobby-timer" class="timer"><span>Organização</span><strong>${Number(d.session.lobby_remaining_seconds||0)>0?fmtTime(d.session.lobby_remaining_seconds):'Aguardando professor'}</strong></div></div><div class="lobby-kpis"><div><strong>${total}</strong><small>alunos da turma</small></div><div><strong>${inRooms}</strong><small>em equipes</small></div><div><strong>${Math.max(0,total-inRooms)}</strong><small>aguardando equipe</small></div><div><strong>${leaders}</strong><small>lideranças definidas</small></div></div><div class="lobby-main-grid"><div><div class="lobby-section-title"><div><h3>Equipes disponíveis</h3><small>Até ${Number(d.session.max_clan_size||6)} integrantes por equipe</small></div></div><div class="lobby-room-grid">${roomCards}</div></div><aside><h3>Alunos da turma</h3><div class="lobby-roster">${roster}</div></aside></div></section>${ownRoom}<details class="panel section-panel ranking-panel"><summary><strong>🏆 Ranking geral</strong><span>equipes + alunos</span></summary><div class="ranking-tabs"><h4>Ranking das equipes</h4><div class="rank-list">${teamRanks||'<small class="muted">O ranking começa quando houver XP.</small>'}</div><h4>Ranking individual geral</h4><div class="rank-list">${studentRanks||'<small class="muted">O ranking começa quando houver XP.</small>'}</div></div></details>${d.session.status==='locked'?'<div class="notice warn"><strong>Equipes bloqueadas pelo professor.</strong> Aguarde o início da prova.</div>':''}`;
  }
