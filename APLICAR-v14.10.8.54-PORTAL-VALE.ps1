...
try{
  ...
  $config=Join-Path $Repo 'lobby/assets/config.js'
  if(-not(Test-Path $config)){throw 'config.js do Lobby ausente.'}
  $configText=Get-Content $config -Raw
  if($configText -notmatch '14\.10\.8\.(53|54)'){throw 'A base esperada é v14.10.8.53 ou uma reaplicação v14.10.8.54.'}
  ...
  foreach($f in $Manifest.files){
    ...
    if($f.status -eq 'modified'){
      ...
      if($actual -ne $base -and $actual -ne $target){throw "Arquivo local divergente; PATCH não sobrescreveu: $($f.path)"}
    } elseif($actual -and $actual -ne $target){
      throw "Arquivo novo já existe com conteúdo diferente: $($f.path)"
    }
  }
  foreach($f in $Manifest.files){
    $src=Join-Path $Payload ([string]$f.path);$dest=Join-Path $Repo ([string]$f.path)
  ...
  Write-Host "PASS - v14.10.8.54 aplicada: $($Manifest.totalPayloadFiles) arquivos no payload, 0 exclusões." -ForegroundColor Green
  Write-Host 'Execute VALIDAR-v14.10.8.54.ps1 e confira git status antes do commit.' -ForegroundColor Cyan
