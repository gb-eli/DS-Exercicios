...
Deno.serve(async req=>{
 ...
 try{
  ...
  const {data:{user},error:ue}=await uc.auth.getUser();if(ue||!user?.email)return J({error:'unauthorized'},401);const live=await requireLiveAuthSession(db,auth,user.id);if(!live.ok)return J({error:live.error,detail:live.detail||null},live.status);
  const [{data:p},{data:a}]=await Promise.all([db.from('profiles').select('role,active,must_change_password').eq('id',user.id).maybeSingle(),db.from('staff_allowlist').select('role,active').eq('email',user.email.toLowerCase()).eq('active',true).maybeSingle()]);
  const role=String(a?.role||p?.role||'');if(!p?.active)return J({error:'forbidden'},403);if(p.must_change_password)return J({error:'password_change_required'},403);if(!['admin','super_admin'].includes(role))return J({error:'forbidden'},403);
