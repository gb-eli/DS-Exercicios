...
Deno.serve(async req=>{
  ...
  try{
    ...
    const rl=await consumeRate(db,`lobby-moderation:${user.id}:${requestIp(req)||'noip'}`,60,60,120);
    if(rl.allowed===false){await logRisk(db,user,req,'rate_limit','high',{source:'lobby-moderation',action,count:rl.count});return new Response(JSON.stringify({error:'rate_limited',retry_after:rl.retry_after||120}),{status:429,headers:{...H,'Content-Type':'application/json','Retry-After':String(rl.retry_after||120)}})}
    const {data:p}=await db.from('profiles').select('id,email,role,active,must_change_password').eq('id',user.id).maybeSingle();
    if(!p?.active||p.must_change_password||!['teacher','admin','super_admin'].includes(String(p.role))){if(Number(rl.count||0)===1||Number(rl.count||0)%20===0)await logRisk(db,user,req,'staff_endpoint_probe','warning',{source:'lobby-moderation',action,role:p?.role||null});return J({error:'staff_only'},403)}
    await ensureAccess(db,user,req,'lobby-moderation');
    ...
    const audit=async(name:string,payload:Record<string,unknown>)=>{try{await db.from('admin_audit_log').insert({actor_user_id:user.id,action:name,target_user_id:studentId,payload:{...payload,ip_address:requestIp(req),user_agent:S(req.headers.get('user-agent'),300)}});}catch(_){} };
    if(action==='kick'){
