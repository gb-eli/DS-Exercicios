...
async function context(req:Request){
 ...
 const admin=createClient(url,service,{auth:{persistSession:false,autoRefreshToken:false}});const live=await requireLiveAuthSession(admin,auth,user.id);if(!live.ok)throw Object.assign(new Error(live.error),{status:live.status,detail:live.detail||null});
...
Deno.serve(async(req)=>{
 ...
 if(Number(req.headers.get('content-length')||0)>65536)return json({error:'payload_too_large'},413);
 try{
  const{user,admin,platformId}=await context(req);await ensureAccess(admin,user,req,'ctf-core-actions');
  const body=await req.json();
  const action=String(body.action||'');const limits:Record<string,[number,number,number]>={state:[60,60,60],lesson_start:[20,60,120],lesson_complete:[12,60,180],tool_used:[90,60,120],daily_sync:[30,60,120],hint:[12,60,180],store_purchase:[12,60,180],store_state:[60,60,60]};const [lim,win,blk]=limits[action]||[60,60,120],rl=await consumeRate(admin,`ctf-core:${action||'unknown'}:${user.id}:${requestIp(req)||'noip'}`,lim,win,blk);if(rl.allowed===false){await logRisk(admin,user,req,'rate_limit','high',{source:'ctf-core-actions',action,count:rl.count});return new Response(JSON.stringify({error:'rate_limited',retry_after:rl.retry_after||blk}),{status:429,headers:{...cors,'content-type':'application/json','Retry-After':String(rl.retry_after||blk)}})};
  ...
  if(action==='lesson_start'||action==='lesson_complete'){
   ...
   if(action==='lesson_start'){
    const startBucket=Math.floor(Date.now()/300000);const{data,error}=await admin.rpc('record_core_progress_event',{p_user_id:user.id,p_platform_code:PLATFORM_CODE,p_activity_id:activityId,p_event_type:'activity.started',p_progress:0,p_score:null,p_payload:{source:'ctf-core-actions'},p_idempotency_key:`${base}:start:${startBucket}`,p_occurred_at:new Date().toISOString()});
   ...
   let reward=null;
   if(existing.status!=='completed'){const minSeconds=Math.max(30,Number(lesson.metadata?.minSeconds||0));const{data:start}=await admin.from('progress_events').select('occurred_at').eq('user_id',user.id).eq('platform_id',platformId).eq('activity_id',activityId).eq('event_type','activity.started').order('occurred_at',{ascending:false}).limit(1).maybeSingle();const elapsed=start?.occurred_at?Math.floor((Date.now()-Date.parse(start.occurred_at))/1000):-1;if(elapsed<minSeconds||elapsed>7200){await logRisk(admin,user,req,'lesson_completion_timing','warning',{source:'ctf-core-actions',lessonId,elapsed,minSeconds});return json({error:elapsed>7200?'lesson_session_expired':'minimum_lesson_time',required_seconds:minSeconds,elapsed_seconds:Math.max(0,elapsed)},409)};
