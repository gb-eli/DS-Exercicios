...
export class AGVCoreSDK {
  ...
  auth={
    signIn:async(email,password)=>{
      ...
      try{await this.security.report({action:'auth.login_success',severity:'info',payload:{surface:this.platformId}})}catch{}
      return data;
    },
    signOut:async()=>{
  ...
  security={
    report:async({action='session.check',severity='info',payload={}}={})=>{
      return this._invoke('security-telemetry',{action,severity,payload:{...payload,platformId:this.platformId,platformVersion:this.platformVersion,integrationVersion:this.integrationVersion}});
    },
    sessionCheck:async(payload={})=>{
      try{return await this._invoke('security-telemetry',{action:'session.check',severity:'info',payload:{...payload,platformId:this.platformId,platformVersion:this.platformVersion,integrationVersion:this.integrationVersion}})}
