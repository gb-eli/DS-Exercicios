...
function stateKey() { return `ds1_80_${ROLE}_${currentUserId()}_ex_${ex().numero}_state_v3`; }
function legacyStateKey() { return `ds1_80_${ROLE}_${currentUserId()}_ex_${ex().numero}_state_v2`; }
...
function storageGet(key) { try { return localStorage.getItem(key); } catch { showStorageFailure(); return null; } }
function storageSet(key, value) { try { localStorage.setItem(key, value); return true; } catch { showStorageFailure(); return false; } }
function storageRemove(key) { try { localStorage.removeItem(key); return true; } catch { showStorageFailure(); return false; } }
...
function githubKey() { return `ds1_${ROLE}_${currentUserId()}_github_v2`; }
...
function savedGithubUrl() {
  ...
  // A chave antiga era global e poderia compartilhar o repositório entre alunos.
  // Ela é removida sem atribuição automática a nenhum usuário.
...
function loadState() {
  try { const raw = storageGet(stateKey()) || storageGet(legacyStateKey()); state = { ...freshState(), ...(JSON.parse(raw) || {}) }; }
...
function progress() {
  ...
  return Math.min(99,Math.round(populated*25+matched*55+run*20));
}
function renderProgress() {
  state.percentage=state.validated?100:progress();
  $("#progressPercent").textContent=`${state.percentage}%`; $("#progressBar").style.width=`${state.percentage}%`;
  $("#progressText").textContent=state.completed?"Atividade concluída":state.validated?"Pronto para concluir":state.percentage>=80?"Quase pronta":state.percentage?"Em desenvolvimento":"Ainda não começou";
...
async function validate() {
  ...
  if (issues.length) {
    ...
    message.innerHTML = "<strong>Atividade validada.</strong> A estrutura, a execução e os requisitos obrigatórios foram conferidos.";
...
function setCodeFont(delta){const root=document.documentElement,currentSize=parseFloat(getComputedStyle(root).getPropertyValue('--code-font-size'))||15;const next=Math.max(12,Math.min(22,currentSize+delta));root.style.setProperty('--code-font-size',`${next}px`);storageSet('ds1CodeFont',String(next));if(codeEditor)codeEditor.refresh();}
function init(){
  const savedFont=Number(storageGet('ds1CodeFont'));if(savedFont)document.documentElement.style.setProperty('--code-font-size',`${savedFont}px`);
