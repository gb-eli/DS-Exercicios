...
function referenceForActive(){return "";}
...
function openInfo(type){
  ...
  const tips=["Revise o objetivo e os conceitos da atividade.","Execute seu próprio código e interprete os erros antes de concluir."];
  const maps={
...
function renderTeacher(){return;}
function renderAll(){window.dispatchEvent(new CustomEvent('agv:activity-open',{detail:{numero:ex().numero}}));loadState();if($("#completeFolder"))$("#completeFolder").textContent=projectRoot();renderSummary();renderTabs();renderTutorial();renderPractice();renderClassroom();renderTeacher();$("#exerciseSelect").value=current;showView("tutorial");}
