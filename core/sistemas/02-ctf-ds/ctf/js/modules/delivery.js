...
import { missionBlocks, getBlockState, getCampaignBlockSummary } from '../core/mission-blocks.js';
  ...
  return `
    ...
    <section class="delivery-grid">
      <article class="card delivery-summary">
        ...
        <div class="delivery-metrics"><div><span>Blocos</span><b>${campaignBlocks.completed}/${campaignBlocks.total}</b></div><div><span>Missões</span><b>${completed}/${challenges.length}</b></div><div><span>Proficiência estimada</span><b>${proficiency.score}/100</b></div><div><span>Estrelas</span><b>${profile.stars}</b></div></div>
      ...
      <article class="card delivery-blocks"><div class="section-title" style="margin-top:0"><h2>CHECKPOINTS DOS BLOCOS</h2><small>${campaignBlocks.completed}/${campaignBlocks.total} CONCLUÍDOS</small></div><div class="delivery-block-list">${missionBlocks.map((block) => { const state = getBlockState(profile, block); return `<button data-view-block-checkpoint="${block.id}" ${state.complete ? '' : 'disabled'}><span>${String(block.number).padStart(2, '0')}</span><div><strong>${escapeHtml(block.title)}</strong><small>${state.complete ? `${escapeHtml(state.performance)} · ${state.score}/100` : `${state.completed}/${state.total} missões`}</small></div><b>${state.complete ? '✓' : state.state === 'locked' ? '🔒' : `${state.percentage}%`}</b></button>`; }).join('')}</div><p class="muted">Cada checkpoint registra um resumo formativo; moedas e emblemas continuam separados da nota.</p></article>
...
export const buildEvidenceHtml = (profile) => {
  ...
  const rows = completedIds.map((id) => {
    ...
    return `<tr><td>${safe(mission?.title || id)}</td><td>${safe(mission?.track || mission?.category || '')}</td><td>${safe(result?.stars || result?.rating || '✓')}</td><td>${safe(result?.proof ? result.proof.slice(0, 12) : 'legado')}</td><td>${safe(result?.completedAt ? formatDate(result.completedAt) : '')}</td></tr>`;
  }).join('');
