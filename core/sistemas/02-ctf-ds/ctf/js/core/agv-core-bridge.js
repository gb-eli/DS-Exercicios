...
export async function centralSignIn(email,password){email=String(email||'').trim().toLowerCase();if(!email.endsWith('@escola.pr.gov.br'))throw new Error('Use o e-mail institucional @escola.pr.gov.br.');let session;try{session=await raw('/auth/v1/token?grant_type=password',{body:{email,password},token:false})}catch(error){if(!/^\d{6,12}$/.test(String(password||'')))throw error;await raw('/auth/v1/signup',{body:{email,password,data:{cgm:password}},token:false});session=await raw('/auth/v1/token?grant_type=password',{body:{email,password},token:false})}saveSession(session);return loadIdentity(session.user);}
export async function changeCentralPassword(password){password=String(password||'');if(password.length<8||!/[A-Za-z]/.test(password)||!/\d/.test(password))throw new Error('Use ao menos 8 caracteres, com letra e número.');const user=await raw('/auth/v1/user',{method:'PUT',body:{password}});return loadIdentity(user);}
