...
export class ProfileStore {
  ...
  bindAgvCore() {
    const sync=event=>{const identity=event?.detail||globalThis.AGVPlatformCore?.identity;if(!identity?.user?.id)return;const local=this.profiles.find(p=>p.id===identity.user.id);const now=new Date().toISOString();const mapped=normalizeProfile({...(local||{}),id:identity.user.id,name:identity.profile?.full_name||identity.user.email||'Estudante',className:identity.classInfo?.name||identity.classInfo?.code||'Turma institucional',callsign:(local?.callsign||'AGV').slice(0,12),createdAt:local?.createdAt||now,lastSavedAt:now});if(local)Object.assign(local,mapped);else this.profiles.unshift(mapped);this.activeId=identity.user.id;this.persist();};
    globalThis.addEventListener?.('agv:core-identity',sync);if(globalThis.AGVPlatformCore?.identity)sync({detail:globalThis.AGVPlatformCore.identity});
  }
  ...
  addXp(amount, experienceId) {
    ...
    profile.xp += Math.max(0, Math.round(amount));profile.level = Math.max(1, Math.floor(profile.xp / 250) + 1);this.persist();
    if (experienceId && globalThis.AGVPlatformCore?.identity?.user?.id === profile.id) globalThis.AGVPlatformCore.progress('activity.completed',{activityId:String(experienceId),progress:100,score:Math.max(0,Math.round(amount)),payload:{legacyXp:Math.max(0,Math.round(amount)),legacyLevel:profile.level,source:'planetario-profile-store'}}).catch(()=>{});
    return true;
  }
