...
test('leader governance has quorum, ready check and teacher-controlled member changes',()=>{
  ...
  assert.match(student,/Confirmar função/);
  assert.match(student,/Solicitar troca/);
  assert.match(admin,/Tornar líder|Definir líder/);
...
test('final mission is squad-gated by occupied roles',()=>{
  ...
  assert.match(student,/ATIVIDADE FINAL BLOQUEADA/);
  assert.match(student,/INTEGRAÇÃO ENTRE FUNÇÕES/);
});
...
test('teacher can supervise guild chat and mobile has compact mission view',()=>{
  ...
  assert.match(admin,/Chat das equipes/);
  assert.match(admin,/somente leitura/i);
  assert.match(student,/Minha função \+ equipe/);
  assert.doesNotMatch(student,/RECONNECTING SQUAD/);
  assert.match(student,/Aguardando professor/);
  assert.match(css,/challenge-list\[data-filter="mine"\]/);
  assert.match(css,/@media\(max-width:760px\)/);
});

test('cache and release metadata are bumped',()=>{
  assert.match(read('prova/index.html'),/14\.10\.8\.(?:3[3-9]|[4-9]\d|\d{3,})/);
