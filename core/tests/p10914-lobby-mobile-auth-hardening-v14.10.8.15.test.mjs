...
test('P10.9.14 todas as entradas principais abrem Lobby com cache-bust atual',()=>{
  ...
  assert.match(hub,/lobby\/\?v=14\.10\.8\.65/g);
  assert.match(app,/\.\.\/lobby\/\?v=14\.10\.8\.65/);
  assert.match(prof,/\.\.\/lobby\/\?v=14\.10\.8\.65/);
  assert.match(read('atividades/index.html'),/app\.js\?v=14\.10\.8\.65/);
  assert.match(read('professor/index.html'),/professor\.js\?v=14\.10\.8\.65/);
});

test('P10.9.14 cadeia executável do Lobby está sincronizada na release atual',()=>{
  const index=read('lobby/index.html'),vendor=read('lobby/assets/vendor-loader.js'),boot=read('lobby/assets/boot.js'),lobby=read('lobby/assets/lobby.js'),adapter=read('lobby/assets/core/world-adapter.js');
