...
test('P6.7 wave2 CTF bridge is a valid unified-auth Core ES module surface',()=>{for(const name of ['centralSignOut','loadCoreCTFState','completeCoreChallenge','startCoreLesson','completeCoreLesson','recordCoreToolUse','syncCoreDaily','purchaseCoreHint','purchaseCoreStoreItem'])assert.match(ctf,new RegExp(`export (?:async function|const) ${name}`));assert.match(ctf,/export const hasCentralSession/);assert.doesNotMatch(ctf,/export (?:async function|const) centralSignIn/);assert.doesNotMatch(ctf,/changeCentralPassword/);assert.doesNotMatch(ctf,/grant_type=password/);assert.match(ctf,/ctf-complete-challenge/);assert.match(ctf,/ctf-core-actions/);});
