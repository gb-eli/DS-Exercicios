...
const metricsPath=path.join(root,'AUDITORIA-MOBILE-TABLET-EVIDENCIAS/metrics.json');
const baselinePath=path.join(root,'AUDITORIA-MOBILE-TABLET-EVIDENCIAS/before-v14.10.8.9/metrics.json');
const metrics=fs.existsSync(metricsPath)?JSON.parse(fs.readFileSync(metricsPath,'utf8')):null;
const baseline=fs.existsSync(baselinePath)?JSON.parse(fs.readFileSync(baselinePath,'utf8')):null;
const currentRelease=JSON.parse(fs.readFileSync(path.join(root,'release-current.json'),'utf8')).version;
...
test('P10.9.9 phone layout no longer expands the visual viewport because of code min-content',()=>{
  if(metrics&&baseline)for(const name of ['phone360','phone390','phone430']){
...
test('P10.9.9 tablet portrait gives the editor almost the full content width',()=>{
  if(metrics&&baseline)for(const name of ['tablet768','tablet820']){
    const before=byName(baseline,name),after=byName(metrics,name);
...
test('P10.9.9 touch controls and utility bar use mobile-safe sizing/density',()=>{
  ...
  if(metrics&&baseline){assert.equal(byName(metrics,'phone360').minTouchTarget,44);assert.ok(byName(metrics,'phone360').utility.h<byName(baseline,'phone360').utility.h);}
});
...
test('P10.9.9 runtime assets are cache-busted to the mobile/tablet release',()=>{
  const escaped=currentRelease.replace(/\./g,'\\.');
  assert.match(html,new RegExp(`app\\.css\\?v=${escaped}`));
  assert.match(html,new RegExp(`app\\.js\\?v=${escaped}`));
  assert.match(workspace,new RegExp(`weekend-support\\.js\\?v=${escaped}`));
});
