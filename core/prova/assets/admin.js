(()=>{
  ...
  function renderClans(d){
    return `<div class="clan-admin-grid">${d.clans.map(c=>{
      ...
      return `<article class="clan-admin-card" style="--guild-accent:${esc(c.accent_color||'#22d3ee')}">
        ...
        ${phasePills(metric)}
        ${members.length?members.map(m=>{const mm=d.metrics?.members?.[String(m.student_id)]||{},ready=!!m.role_id&&!!m.role_selected_at;return `<div class="admin-member"><div><strong><span class="online-dot ${mm.online?'on':''}"></span>${esc(m.student?.full_name||'Aluno')}${m.is_leader?' • 👑 Líder':''}</strong><small>${esc(roleName(m.role_id))} • ${ready?'READY':'aceite pendente'}</small></div><div class="member-progress"><span>${Number(mm.progress_percent||0)}%</span><small>${Number(mm.completed_count||0)}/${Number(mm.total_count||0)} missões • ${Number(mm.xp_earned||0)}/${Number(mm.xp_max||0)} XP</small></div><span class="status-pill ${ready?'good':'warn'}">${ready?'READY':'PENDENTE'}</span><button class="button ${m.is_leader?'primary':'ghost'} small" data-set-leader="${m.student_id}" data-clan-id="${c.id}" type="button">${m.is_leader?'Líder atual':'Definir líder'}</button></div>`}).join(''):'<div class="notice">Nenhum aluno nesta empresa.</div>'}
      </article>`;
    }).join('')}</div>`;
  }
  ...
  function renderRankings(d){
    ...
    return `<section class="panel section-panel">
      ...
      <div class="layout2 ranking-layout"><div><h3>Equipes — ranking principal</h3><div class="table-wrap"><table class="session-table compact-table"><thead><tr><th>#</th><th>Equipe</th><th>XP</th><th>Progresso</th><th>Online</th></tr></thead><tbody>${teamRows||'<tr><td colspan="5">Sem equipes pontuadas.</td></tr>'}</tbody></table></div></div><details><summary><strong>Ranking individual (apoio docente)</strong></summary><div class="table-wrap"><table class="session-table compact-table"><thead><tr><th>#</th><th>Aluno</th><th>XP</th><th>Progresso</th><th>Status</th></tr></thead><tbody>${studentRows||'<tr><td colspan="5">Sem alunos pontuados.</td></tr>'}</tbody></table></div></details></div>
    </section>`;
  }
  ...
  function renderTeamChat(d){
    const rows=(d.team_chat||[]).slice(0,120);if(!rows.length)return `<section class="panel section-panel"><div class="section-head"><div><p class="eyebrow">Supervisão</p><h2>Chat das guildas</h2><p>Somente leitura para a equipe docente.</p></div></div><div class="notice">Ainda não há mensagens.</div></section>`;
    return `<section class="panel section-panel"><div class="section-head"><div><p class="eyebrow">Supervisão</p><h2>Chat das guildas</h2><p>Histórico somente leitura para acompanhamento do trabalho colaborativo.</p></div><span class="status-pill">${rows.length} recentes</span></div><div class="staff-chat-list">${rows.map(x=>`<div class="staff-chat-row"><span>${fmtDate(x.created_at)}</span><strong>${esc(clanName(x.clan_id))} • ${esc(x.sender_name||studentName(x.sender_id))}</strong><p>${esc(x.message)}</p></div>`).join('')}</div></section>`;
  }

  function renderSettings(s){
    if(s.started_at||!['draft','waiting_room','locked'].includes(s.status))return '';
    return `<section class="panel section-panel"><div class="section-head"><div><p class="eyebrow">Configuração da sessão</p><h2>Lobby, tamanho dos grupos e peso</h2><p>O tempo de referência é apenas informativo. Depois do início, a operação permanece aberta até o professor encerrá-la.</p></div></div><div class="create-grid settings-grid">
      ...
      <label class="form-field">Referência de tempo (min)<input id="settings-duration" type="number" min="10" max="180" value="${Number(s.duration_minutes||50)}"><small>Não encerra automaticamente.</small></label>
      ...
      <div class="control-bar span2"><button id="save-session-settings" class="button primary" type="button">Salvar configuração</button><span class="muted">Encerramento da avaliação: exclusivamente manual pelo professor.</span></div>
    </div></section>`;
  }
  ...
  function renderDetail(){
    ...
    const timer=s.status==='waiting_room'?`<div class="timer"><span>Pré-lobby restante</span><strong>${fmtTime(s.lobby_remaining_seconds)}</strong><small>Organização de guildas / Ready Check</small></div>`:['running','paused'].includes(s.status)?`<div class="timer operation-clock"><span>Tempo de operação</span><strong>${fmtTime(s.elapsed_seconds||0)}</strong><small>${s.home_continuation_enabled?'continuação em casa ativa':'encerramento manual'}</small></div>`:'';
    $('admin-content').innerHTML=`
      <section class="panel exam-hero"><div><p class="eyebrow">${esc(className(s.class_id))} • ${esc(s.subject_name)}</p><h1>${esc(s.title)}</h1><p>${esc(s.description||'')}</p><div class="session-meta"><span class="status-pill ${kind}">${esc(label)}</span><span class="status-pill">${Number(s.lobby_duration_minutes||15)} min lobby • operação sem limite automático</span><span class="status-pill good">PROFESSOR ENCERRA</span>${s.home_continuation_enabled?'<span class="status-pill warn">🏠 CONTINUAÇÃO EM CASA</span>':''}<span class="status-pill">até ${Number(s.max_clan_size||6)} por equipe</span><span class="status-pill">${fmtScore(s.max_score)} pontos</span><span class="status-pill">Grupo ${Math.round(Number(s.group_weight||.5)*100)}% / Individual ${Math.round(Number(s.individual_weight||.5)*100)}%</span></div></div><div>${timer}<div class="control-bar hero-controls">${controls(s)}</div></div></section>
      ${renderSettings(s)}
      ...
      <div class="layout2"><section class="panel section-panel"><div class="section-head"><div><p class="eyebrow">Gestão manual</p><h2>Alunos, equipes e áreas</h2><p>Antes do início, o professor pode organizar equipes e áreas. Durante a operação, pode redefinir liderança em caso de ausência ou falha de acesso.</p></div></div><div class="roster-list">${d.roster.map(p=>memberEditor(p,rosterMap.get(String(p.id)))).join('')}</div></section><section class="panel section-panel"><div class="section-head"><div><p class="eyebrow">Fases</p><h2>Missões, nota e XP</h2><p>XP máximo e peso acadêmico só podem ser alterados antes do início da prova.</p></div></div>${renderChallenges(d)}</section></div>
  ...
  $('admin-content').addEventListener('click',e=>{
    ...
    if(t.dataset.setLeader){if(!confirm(`Definir ${studentName(t.dataset.setLeader)} como líder${st.data?.detail?.session?.status==='running'?' interino':''} desta guilda?`))return;return run(()=>api('staff_set_leader',{session_id:st.sessionId,clan_id:t.dataset.clanId,student_id:t.dataset.setLeader}));}
    if(t.dataset.removalRequest){const decision=t.dataset.removalDecision;if(!confirm(decision==='approve'?'Aprovar a troca? O aluno sairá da guilda e voltará ao lobby.':'Negar esta solicitação?'))return;return run(()=>api('resolve_member_removal',{session_id:st.sessionId,request_id:t.dataset.removalRequest,decision}));}
