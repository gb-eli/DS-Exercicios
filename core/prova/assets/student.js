(()=>{
  ...
  function waitingRoom(d){
    ...
    const roomCards=(d.clans||[]).map(c=>{
      ...
      const assigned=(c.members||[]).filter(m=>m.role_id).length,accent=/^#[0-9A-Fa-f]{6}$/.test(String(c.accent_color||''))?c.accent_color:'#22d3ee';
      return `<article class="panel lobby-room-card ${mine?'selected':''}" data-room-theme="${esc(c.theme_key||'cyber')}" style="--guild-accent:${esc(accent)}"><div class="guild-card-identity">${guildMark(c)}<div><span class="room-number">${esc(c.name)}</span><strong>${esc(c.company_name||'Guilda aguardando identidade')}</strong><small>${mascotInfo(c.mascot_key)[1]} • tema ${esc(c.theme_key||'cyber')}</small></div>${mine?'<span class="status-pill good">Sua guilda</span>':''}</div><div class="guild-readiness"><span><b>${Number(c.count||0)}/${Number(d.session.max_clan_size||6)}</b> integrantes</span><span><b>${assigned}/${Number(c.count||0)}</b> cargos</span><span class="${leader?'ready':''}"><b>${leader?'✓':'○'}</b> líder</span></div><div class="room-leader-line">${leader?`👑 Líder: <strong>${esc(leader.name)}</strong>`:'🗳️ Líder: <strong>em votação</strong>'}</div><div class="lobby-room-members">${members||'<small class="muted">Sala vazia.</small>'}</div>${action}</article>`;
    }).join('');
