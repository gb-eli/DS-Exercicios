(()=>{
  ...
  const mascotMap={robot:['🤖','Robô'],owl:['🦉','Coruja'],fox:['🦊','Raposa'],dragon:['🐉','Dragão'],wolf:['🐺','Lobo'],eagle:['🦅','Águia'],octopus:['🐙','Polvo'],capybara:['🦫','Capivara']};
  const mascotInfo=key=>mascotMap[String(key)]||mascotMap.robot;
  const safeEmblem=value=>/^data:image\/(png|jpeg|webp);base64,[A-Za-z0-9+/=]+$/.test(String(value||''))?String(value):'';
  const guildMark=(room,size='')=>{const image=safeEmblem(room?.emblem_data_url);return image?`<img class="guild-emblem ${size}" src="${esc(image)}" alt="Emblema de ${esc(room?.name||'guilda')}">`:`<span class="guild-mascot ${size}" aria-label="Mascote ${esc(mascotInfo(room?.mascot_key)[1])}">${mascotInfo(room?.mascot_key)[0]}</span>`;};
  ...
  function waitingRoom(d){
    ...
    const roomCards=(d.clans||[]).map(c=>{
      ...
      return `<article class="panel lobby-room-card ${mine?'selected':''}" data-room-theme="${esc(c.theme_key||'cyber')}" style="--guild-accent:${esc(accent)}"><div class="guild-card-identity">${guildMark(c)}<div><span class="room-number">${esc(c.name)}</span><strong>${esc(c.company_name||'Guilda aguardando identidade')}</strong><small>${mascotInfo(c.mascot_key)[1]} • tema ${esc(c.theme_key||'cyber')}</small></div>${mine?'<span class="status-pill good">Sua guilda</span>':''}</div><div class="guild-readiness"><span><b>${Number(c.count||0)}/${Number(d.session.max_clan_size||6)}</b> integrantes</span><span><b>${readyCount}/${Number(c.count||0)}</b> READY</span><span class="${leader?'ready':''}"><b>${leader?'✓':'○'}</b> líder</span></div><div class="room-leader-line">${leader?`👑 Líder: <strong>${esc(leader.name)}</strong>`:'🗳️ Líder: <strong>em votação</strong>'}</div><div class="lobby-room-members">${members||'<small class="muted">Sala vazia.</small>'}</div>${action}</article>`;
    }).join('');
    ...
    if(selected){
      ...
      const assignments=team.map(m=>{
        ...
        const accepted=!!m.role_id&&!!m.role_selected_at;return `<div class="leader-member-row"><div><strong>${isLeader?'👑 ':''}${esc(m.student?.full_name||'Aluno')} ${accepted?'<span class="status-pill good">READY</span>':m.role_id?'<span class="status-pill warn">ACEITE PENDENTE</span>':''}</strong><small>${isSelf?'Você • ':''}${esc(roleById.get(String(m.role_id||''))?.name||'Sem área definida')}</small></div><select data-leader-role="${m.student_id}" ${!me?.is_leader||d.session.status!=='waiting_room'?'disabled':''}><option value="">Definir área…</option>${opts}</select>${me?.is_leader&&!isSelf&&d.session.status==='waiting_room'?`<button class="button ghost small" data-leader-kick="${m.student_id}" type="button">Solicitar troca</button>`:'<span></span>'}</div>`;
      }).join('');
      const leaderPanel=me?.is_leader?`<section class="panel section-panel leader-console" style="--guild-accent:${esc(room.accent_color||'#22d3ee')}"><div class="section-head"><div><p class="eyebrow">Console do líder</p><h2>Personalizar a guilda</h2><p>Defina identidade visual, empresa fictícia e cargos antes do início.</p></div><span class="status-pill good">👑 Líder</span></div><form id="room-settings-form" class="room-settings-grid"><label>Nome da guilda<input name="name" maxlength="42" value="${esc(room.name||'')}"></label><label>Tema<select name="theme_key">${['cyber','neon','ocean','violet','sunset','matrix','corporate','mono'].map(x=>`<option value="${x}" ${String(room.theme_key||'cyber')===x?'selected':''}>${esc(x[0].toUpperCase()+x.slice(1))}</option>`).join('')}</select></label><label>Cor de destaque<input name="accent_color" type="color" value="${esc(room.accent_color||'#22d3ee')}"></label><label>Mascote<select name="mascot_key">${Object.entries(mascotMap).map(([key,value])=>`<option value="${key}" ${String(room.mascot_key||'robot')===key?'selected':''}>${value[0]} ${esc(value[1])}</option>`).join('')}</select></label><label>Nome da empresa<input name="company_name" maxlength="80" value="${esc(room.company_name||'')}" placeholder="Ex.: NovaByte Sistemas"></label><label>CNPJ fictício<input name="company_cnpj" maxlength="24" value="${esc(room.company_cnpj||'')}" placeholder="00.000.000/0001-00"></label><label>Cidade<input name="company_city" maxlength="80" value="${esc(room.company_city||'')}" placeholder="Paranaguá - PR"></label><label>Telefone fictício<input name="company_phone" maxlength="30" value="${esc(room.company_phone||'')}" placeholder="(41) 99999-9999"></label><div class="guild-emblem-editor"><div id="guild-emblem-preview">${guildMark(room,'large')}</div><label>Emblema da guilda<input id="guild-emblem-input" type="file" accept="image/png,image/jpeg,image/webp"><small>PNG, JPEG ou WEBP • máximo 320 KB</small></label>${room.emblem_data_url?'<button class="button ghost small" data-remove-emblem type="button">Remover emblema</button>':''}</div><button class="button primary room-save" type="submit" ${d.session.status!=='waiting_room'?'disabled':''}>Salvar identidade e empresa</button></form><div class="section-head compact-head"><div><p class="eyebrow">Funções exclusivas</p><h3>Definir cargo de cada integrante</h3><p>Cada cargo libera desafios coerentes com a responsabilidade escolhida.</p></div></div><div class="leader-member-list">${assignments}</div></section>`:'';
      ...
      ownRoom=`<section class="panel section-panel team-room-panel" data-room-theme="${esc(room.theme_key||'cyber')}" style="--guild-accent:${esc(accent)}"><div class="guild-room-hero">${guildMark(room,'hero')}<div><p class="eyebrow">Sua guilda</p><h2>${esc(room.name||'Sala')}</h2><p>${esc(room.company_name||'A equipe ainda pode criar o nome fictício da empresa.')}${room.company_city?` • ${esc(room.company_city)}`:''}</p><div class="guild-tags"><span>${mascotInfo(room.mascot_key)[0]} ${esc(mascotInfo(room.mascot_key)[1])}</span><span>🎨 ${esc(room.theme_key||'cyber')}</span><span>👥 ${team.length}/${Number(d.session.max_clan_size||6)}</span><span>🧩 ${assignedCount}/${team.length} cargos</span></div></div></div><div class="company-strip"><span><b>Empresa</b>${esc(room.company_name||'—')}</span><span><b>CNPJ fictício</b>${esc(room.company_cnpj||'—')}</span><span><b>Cidade</b>${esc(room.company_city||'—')}</span><span><b>Telefone</b>${esc(room.company_phone||'—')}</span></div><div class="section-head compact-head"><div><p class="eyebrow">Votação</p><h3>Escolha do líder da guilda</h3><p>Uma pessoa por integrante. O candidato com maior número de votos assume a liderança. Em empate, não há líder até o desempate.</p></div></div><div class="leader-vote-grid">${candidates}</div>${electionStatus}</section>${leaderPanel}${me?.role_id?`<section class="panel section-panel role-wait-card ${me.role_selected_at?'role-ready':''}"><p class="eyebrow">READY CHECK</p><h3>${esc(roleById.get(String(me.role_id||''))?.name||'Cargo designado')}</h3>${me.role_selected_at?'<div class="notice good"><strong>✓ Missão aceita.</strong> Você está READY para o início da operação.</div>':`<p class="muted">O líder designou esta função para você. Confira sua área e confirme que está pronto para assumir esta missão.</p><button class="button primary" data-accept-role type="button">Aceitar missão • READY</button>`}</section>`:`<section class="panel section-panel role-wait-card"><p class="eyebrow">Seu cargo</p><h3>Aguardando definição do líder</h3><p class="muted">O líder distribui cargos exclusivos. Depois você confirma a missão no Ready Check.</p></section>`}`;
    }
    ...
    const myRoom=selected?roomById.get(selected):null,myTeam=d.team||[],rolesReady=!!selected&&myTeam.length>0&&myTeam.every(member=>member.role_id),identityReady=!!myRoom?.company_name&&!!myRoom?.name;
    const steps=[['1','Entrar no lobby',true],['2','Escolher guilda',!!selected],['3','Eleger líder',!!myRoom?.leader_id],['4','Definir identidade e cargos',!!myRoom?.leader_id&&rolesReady&&identityReady],['5','Aguardar início',d.session.status==='locked'||d.session.status==='running']];
    return `<section class="matchmaking-live-strip"><div><i></i><span>MATCHMAKING ONLINE</span><strong>${inRooms}/${total||0} jogadores alocados</strong></div><div><span>DATA CENTER</span><strong>SA-EAST • SYNC</strong></div><div><span>MISSÃO</span><strong>${esc(d.session.subject_name||'Prova coletiva')}</strong></div><div><span>STATUS DO SQUAD</span><strong>${selected?'GUILDA CONECTADA':'PROCURANDO GUILDA'}</strong></div></section><section class="lobby-journey" aria-label="Etapas do pré-lobby">${steps.map(([number,label,done],index)=>`<div class="${done?'done':''} ${!done&&steps.slice(0,index).every(step=>step[2])?'current':''}"><span>${done?'✓':number}</span><small>${label}</small></div>`).join('')}</section><section class="panel lobby-master"><div class="section-head"><div><p class="eyebrow">Lobby geral • ${Number(d.session.lobby_duration_minutes||15)} min</p><h2>Formação das guildas</h2><p>Veja toda a turma, escolha uma guilda e organize liderança, identidade e cargos. Depois do início, a composição fica bloqueada.</p></div><div id="lobby-timer" class="timer"><span>Pré-lobby</span><strong>${fmtTime(d.session.lobby_remaining_seconds)}</strong></div></div><div class="lobby-kpis"><div><strong>${total}</strong><small>alunos da turma</small></div><div><strong>${inRooms}</strong><small>em guildas</small></div><div><strong>${Math.max(0,total-inRooms)}</strong><small>aguardando guilda</small></div><div><strong>${leaders}</strong><small>líderes definidos</small></div></div><div class="lobby-main-grid"><div><div class="lobby-section-title"><div><h3>Guildas disponíveis</h3><small>Até ${Number(d.session.max_clan_size||6)} integrantes por guilda</small></div></div><div class="lobby-room-grid">${roomCards}</div></div><aside><h3>Lobby geral da turma</h3><div class="lobby-roster">${roster}</div></aside></div></section>${ownRoom}<details class="panel section-panel ranking-panel"><summary><strong>🏆 Ranking geral</strong><span>guildas + alunos</span></summary><div class="ranking-tabs"><h4>Ranking das guildas</h4><div class="rank-list">${teamRanks||'<small class="muted">O ranking começa quando houver XP.</small>'}</div><h4>Ranking individual geral</h4><div class="rank-list">${studentRanks||'<small class="muted">O ranking começa quando houver XP.</small>'}</div></div></details>${d.session.status==='locked'?'<div class="notice warn"><strong>Guildas bloqueadas pelo professor.</strong> Aguarde o início da prova.</div>':''}`;
  }
  ...
  function challengesView(d){
    ...
    const list=phases.map(p=>`<div class="phase-title"><span>${p}</span><strong>Fase ${p}</strong></div>${d.challenges.filter(c=>Number(c.phase_no||1)===p).map(c=>{
      ...
      const body=submitted
        ...
        : mine&&!locked
          ...
          : locked?(c.squad_gate?`<div class="notice warn squad-gate"><strong>🔒 MISSÃO FINAL BLOQUEADA</strong><br>O squad precisa concluir as missões das áreas ocupadas. Progresso: ${Number(c.squad_gate.completed||0)}/${Number(c.squad_gate.required||0)} • pendentes ${Number(c.squad_gate.pending||0)}.</div>`:'<div class="notice warn">Conclua a etapa anterior para liberar esta missão.</div>'):'<div class="notice">Você pode ajudar a equipe a pensar, mas somente a área responsável — ou o líder nas missões coletivas — pode enviar.</div>';
      return `<article class="challenge-card ${locked?'locked':''} ${done?'done':''}" data-category="${category}" data-challenge-card="${c.id}"><div class="challenge-head"><div><h3>${esc(c.title)}</h3><div class="challenge-meta"><span class="status-pill">${esc(role)}</span><span class="status-pill">${Number(c.points||0).toLocaleString('pt-BR')} pts</span><span class="status-pill live">até ${Number(c.xp_max||0)} XP</span>${done?'<span class="status-pill good">Entregue</span>':draft?'<span class="status-pill warn">Rascunho</span>':locked?'<span class="status-pill warn">Aguardando</span>':mine?'<span class="status-pill live">Sua missão</span>':'<span class="status-pill">Área de colega</span>'}</div></div></div>${challengeGuide(c)}${body}</article>`;
    }).join('')}`).join('');
    ...
    const filter=st.challengeFilter||'all';return `${mobileDock}<div class="layout2"><section class="panel section-panel"><div class="section-head"><div><p class="eyebrow">Mission Board</p><h2>Missões por fases</h2><p>Cada integrante responde por sua área, mas a missão final só libera quando as áreas ocupadas concluírem seus handoffs.</p></div></div><div class="mission-filter-bar"><button class="button small ${filter==='mine'?'primary':'ghost'}" data-challenge-filter="mine" type="button">Minha missão + equipe</button><button class="button small ${filter==='all'?'primary':'ghost'}" data-challenge-filter="all" type="button">Squad completo</button></div><div class="notice handoff-notice"><strong>HANDOFF DE EQUIPE</strong> • Análise orienta requisitos; Design entrega referência ao Front-end; Banco apoia Back-end; QA testa as decisões; Cyber revisa riscos; Negócios conecta tudo ao objetivo do produto.</div><div class="challenge-list" data-filter="${filter}">${list}</div></section><aside class="content-grid sticky-side">
  ...
  function startMatchmakingSequence(session={}){
    ...
    const host=document.createElement('div'),started=Date.now(),fast=reducedMotion();
    host.className='matchmaking-overlay';
    host.setAttribute('role','status');host.setAttribute('aria-live','polite');
    host.innerHTML=`<div class="mm-wallpaper"><i></i><i></i><i></i></div><div class="mm-scanlines"></div><article class="mm-console"><header class="mm-topline"><span>DS // AVALIAÇÃO MULTIPLAYER</span><b>SECURE SESSION</b></header><div class="mm-main"><div class="mm-copy"><span class="mm-kicker">MATCHMAKING ACADÊMICO</span><h1 data-mm-title>BUSCANDO PARTIDA</h1><p data-mm-sub>Localizando a sessão avaliativa e preparando o ambiente seguro.</p><div class="mm-progress"><i data-mm-progress></i></div><div class="mm-stage-list"><div data-mm-stage="0" class="active"><b>01</b><span>Localizando sessão</span><em>BUSCANDO</em></div><div data-mm-stage="1"><b>02</b><span>Sincronizando servidor</span><em>AGUARDANDO</em></div><div data-mm-stage="2"><b>03</b><span>Buscando jogadores</span><em>AGUARDANDO</em></div><div data-mm-stage="3"><b>04</b><span>Montando esquadrão</span><em>AGUARDANDO</em></div><div data-mm-stage="4"><b>05</b><span>Carregando missão</span><em>AGUARDANDO</em></div></div></div><div class="mm-radar-column"><div class="mm-radar"><span></span><i></i><b>DS</b></div><div class="mm-network"><span>SERVIDOR</span><strong>SA-EAST</strong><small>sessão protegida</small></div></div></div><section class="mm-player-lock" data-mm-players>${Array.from({length:6},(_,i)=>`<div><span>${String(i+1).padStart(2,'0')}</span><b>PROCURANDO...</b><small>slot de squad</small></div>`).join('')}</section><footer><span data-mm-footer>Conectando ao lobby...</span><b>FULLSCREEN // SYNC // AUDIT</b></footer></article>`;
    document.body.appendChild(host);requestAnimationFrame(()=>host.classList.add('show'));
    const stages=[
      ['BUSCANDO PARTIDA','Localizando a sessão avaliativa e preparando o ambiente seguro.',18],
      ['SINCRONIZANDO SERVIDOR','Validando sua sessão e sincronizando o estado da prova.',38],
      ['BUSCANDO JOGADORES','Detectando participantes online e vagas disponíveis.',58],
      ['MONTANDO ESQUADRÃO','Carregando guildas, liderança, cargos e canal da equipe.',78],
      ['CARREGANDO MISSÃO','Preparando lobby, HUD, objetivos e recursos avaliativos.',94]
    ];
    const setStage=(index)=>{
      const stage=stages[index];if(!stage||!host.isConnected)return;
      host.querySelector('[data-mm-title]').textContent=stage[0];host.querySelector('[data-mm-sub]').textContent=stage[1];host.querySelector('[data-mm-progress]').style.width=`${stage[2]}%`;
      host.querySelectorAll('[data-mm-stage]').forEach((el,i)=>{el.classList.toggle('active',i===index);el.classList.toggle('done',i<index);el.querySelector('em').textContent=i<index?'OK':i===index?'PROCESSANDO':'AGUARDANDO';});
      host.querySelector('[data-mm-footer]').textContent=index<2?'Estabelecendo conexão segura...':index===2?'Procurando colegas conectados...':index===3?'Distribuindo jogadores no lobby...':'Quase pronto para entrar...';
    };
    const delays=fast?[0,80,160,240,320]:[0,520,1050,1580,2110];
    delays.forEach((delay,i)=>setTimeout(()=>setStage(i),delay));
    return {
      async complete(detail){
        const minimum=fast?380:2700,wait=Math.max(0,minimum-(Date.now()-started));if(wait)await new Promise(r=>setTimeout(r,wait));
        if(!host.isConnected)return;
        const room=(detail?.clans||[]).find(c=>String(c.id)===String(detail?.me?.clan_id||''));
        const members=(room?.members||detail?.team||[]).slice(0,6),players=host.querySelector('[data-mm-players]');
        if(players)players.innerHTML=Array.from({length:6},(_,i)=>{const m=members[i];const name=m?.name||m?.student?.full_name||'';return m?`<div class="locked"><span>${i===0?'👑':String(i+1).padStart(2,'0')}</span><b>${esc(name)}</b><small>${esc(m.role_name||'jogador sincronizado')}</small></div>`:`<div><span>${String(i+1).padStart(2,'0')}</span><b>VAGA DISPONÍVEL</b><small>aguardando jogador</small></div>`}).join('');
        host.querySelector('[data-mm-progress]').style.width='100%';host.querySelector('[data-mm-title]').textContent=detail?.session?.status==='running'?'PARTIDA LOCALIZADA':'LOBBY PRONTO';host.querySelector('[data-mm-sub]').textContent=room?`${room.name||'Guilda'} sincronizada. Preparando sua entrada.`:'Sessão sincronizada. Escolha sua guilda e prepare o esquadrão.';host.querySelector('[data-mm-footer]').textContent=`${(detail?.lobby_roster||[]).length||members.length||1} jogador(es) detectado(s) • conexão estável`;
        host.querySelectorAll('[data-mm-stage]').forEach(el=>{el.classList.remove('active');el.classList.add('done');el.querySelector('em').textContent='OK';});host.classList.add('complete');
        await new Promise(r=>setTimeout(r,fast?100:650));host.classList.add('leave');await new Promise(r=>setTimeout(r,fast?20:420));host.remove();
      },
      fail(){if(!host.isConnected)return;host.querySelector('[data-mm-title]').textContent='CONEXÃO INTERROMPIDA';host.querySelector('[data-mm-sub]').textContent='Não foi possível sincronizar esta partida agora.';host.classList.add('failed');setTimeout(()=>host.remove(),fast?300:1600);}
    };
  }
  function showMatchIntro(d){
    if(!d||d.session.status!=='running'||st.introSession===String(d.session.id))return;
    st.introSession=String(d.session.id);const room=(d.clans||[]).find(c=>String(c.id)===String(d.me?.clan_id||'')),host=document.createElement('div'),key=`agv-match-intro:${st.profile?.id||'x'}:${d.session.id}`,seen=sessionStorage.getItem(key)==='1';host.className=`match-intro ${seen?'reconnect-intro':''}`;
    host.innerHTML=seen?`<div class="match-intro-grid"></div><article><span class="match-kicker">RECONNECTING SQUAD</span>${guildMark(room,'hero')}<p>SESSION FOUND</p><h1>${esc(room?.name||'Guilda')}</h1><strong class="match-go">OPERAÇÃO SINCRONIZADA</strong></article>`:`<div class="match-intro-grid"></div><article><span class="match-kicker">MATCH FOUND</span>${guildMark(room,'hero')}<p>ESQUADRÃO SINCRONIZADO</p><h1>${esc(room?.name||'Guilda')}</h1><div class="match-countdown"><b>3</b><b>2</b><b>1</b></div><strong class="match-go">MISSÃO INICIADA</strong></article>`;
    sessionStorage.setItem(key,'1');document.body.appendChild(host);setTimeout(()=>host.classList.add('go'),seen?50:180);setTimeout(()=>host.remove(),seen?900:(d.accommodation?.reduce_motion||window.matchMedia?.('(prefers-reduced-motion: reduce)').matches)?1300:4300);
  }
  ...
  $('exam-content').addEventListener('click',e=>{
    const t=e.target.closest('[data-open-session],[data-back-sessions],[data-join-clan],[data-leave-clan],[data-leader-vote],[data-leader-kick],[data-accept-role],[data-challenge-filter],[data-remove-emblem],[data-submit-challenge],[data-open-team-chat]');if(!t)return;
    ...
    if(t.hasAttribute('data-accept-role'))return doAction(()=>api('accept_role',{session_id:st.sessionId}));
    ...
    if(t.hasAttribute('data-remove-emblem')){st.emblemDraft='';const preview=$('guild-emblem-preview');if(preview){const room=(st.detail?.clans||[]).find(c=>String(c.id)===String(st.detail?.me?.clan_id));preview.innerHTML=`<span class="guild-mascot large">${mascotInfo(room?.mascot_key)[0]}</span>`;}t.remove();return;}
