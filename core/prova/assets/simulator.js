(()=>{
  ...
  function showSimulatorIntro(){
    document.querySelector('.matchmaking-overlay.simulator-matchmaking')?.remove();
    document.querySelector('.match-intro.simulator-intro')?.remove();
    const g=guildInfo(state.joinedRoom),host=document.createElement('div'),roster=teamMembers();
    host.className='matchmaking-overlay simulator-matchmaking';
    host.innerHTML=`<div class="mm-wallpaper"><i></i><i></i><i></i></div><div class="mm-scanlines"></div><article class="mm-console"><header class="mm-topline"><span>DS // PLAYER EXPERIENCE LAB</span><b>SIMULAÇÃO MULTIPLAYER</b></header><div class="mm-main"><div class="mm-copy"><span class="mm-kicker">MATCHMAKING ACADÊMICO</span><h1 data-mm-title>BUSCANDO PARTIDA</h1><p data-mm-sub>Varrendo sessões avaliativas disponíveis...</p><div class="mm-progress"><i data-mm-progress></i></div><div class="mm-stage-list"><div data-mm-stage="0" class="active"><b>01</b><span>Buscando partida</span><em>PROCESSANDO</em></div><div data-mm-stage="1"><b>02</b><span>Buscando jogadores</span><em>AGUARDANDO</em></div><div data-mm-stage="2"><b>03</b><span>Montando lobby</span><em>AGUARDANDO</em></div><div data-mm-stage="3"><b>04</b><span>Formando esquadrão</span><em>AGUARDANDO</em></div><div data-mm-stage="4"><b>05</b><span>Carregando missão</span><em>AGUARDANDO</em></div></div></div><div class="mm-radar-column"><div class="mm-radar"><span></span><i></i><b>${g.tag}</b></div><div class="mm-network"><span>REGIÃO</span><strong>SA-EAST</strong><small>ping acadêmico: 12 ms</small></div></div></div><section class="mm-player-lock" data-mm-players>${Array.from({length:6},(_,i)=>`<div><span>${String(i+1).padStart(2,'0')}</span><b>PROCURANDO...</b><small>slot de squad</small></div>`).join('')}</section><footer><span data-mm-footer>Varrendo rede acadêmica...</span><b>FULLSCREEN // SQUAD // MISSION</b></footer></article>`;
    document.body.appendChild(host);requestAnimationFrame(()=>host.classList.add('show'));
    const stages=[
      ['BUSCANDO PARTIDA','Varrendo sessões avaliativas disponíveis...',15,'Varrendo rede acadêmica...'],
      ['BUSCANDO JOGADORES','Localizando integrantes online da turma...',37,'Jogadores encontrados: 08 / 30'],
      ['MONTANDO LOBBY','Criando canais, guildas e slots da partida...',59,'Lobby reservado • servidor estável'],
      ['FORMANDO ESQUADRÃO',`Sincronizando integrantes da ${g.name}...`,80,'Bloqueando posições do squad...'],
      ['CARREGANDO MISSÃO','Preparando objetivos, chat, HUD e avaliação...',96,'Ready-check em andamento...']
    ];
    stages.forEach((stage,i)=>setTimeout(()=>{if(!host.isConnected)return;host.querySelector('[data-mm-title]').textContent=stage[0];host.querySelector('[data-mm-sub]').textContent=stage[1];host.querySelector('[data-mm-progress]').style.width=`${stage[2]}%`;host.querySelector('[data-mm-footer]').textContent=stage[3];host.querySelectorAll('[data-mm-stage]').forEach((el,j)=>{el.classList.toggle('active',j===i);el.classList.toggle('done',j<i);el.querySelector('em').textContent=j<i?'OK':j===i?'PROCESSANDO':'AGUARDANDO';});if(i>=1){host.querySelector('[data-mm-players]').innerHTML=Array.from({length:6},(_,idx)=>{const m=idx<=Math.min(i,4)?roster[idx]:null;return m?`<div class="locked"><span>${idx===0?'👑':initials(m.full_name)}</span><b>${esc(m.full_name)}</b><small>${idx===0?'líder provisório':'jogador sincronizado'}</small></div>`:`<div><span>${String(idx+1).padStart(2,'0')}</span><b>PROCURANDO...</b><small>slot de squad</small></div>`}).join('');}},i*640));
    setTimeout(()=>{if(!host.isConnected)return;host.querySelector('[data-mm-title]').textContent='SQUAD CONFIRMADO';host.querySelector('[data-mm-sub]').textContent=`${g.name} pronta para o deploy avaliativo.`;host.querySelector('[data-mm-progress]').style.width='100%';host.querySelectorAll('[data-mm-stage]').forEach(el=>{el.classList.remove('active');el.classList.add('done');el.querySelector('em').textContent='OK'});host.classList.add('complete');},3350);
    setTimeout(()=>{host.classList.add('leave')},3900);
    setTimeout(()=>{host.remove();const intro=document.createElement('div');intro.className='match-intro simulator-intro';intro.innerHTML=`<div class="match-intro-grid"></div><article><span class="match-kicker">MATCH FOUND // DEPLOYING SQUAD</span><div class="sim-intro-emblem">${g.icon}</div><h1>${esc(state.roomName)}</h1><p>OPERAÇÃO AVALIATIVA • HUD SINCRONIZADO</p><div class="match-countdown"><b>3</b><b>2</b><b>1</b></div><strong class="match-go">MISSÃO INICIADA</strong></article>`;document.body.appendChild(intro);requestAnimationFrame(()=>intro.classList.add('go'));setTimeout(()=>intro.classList.add('launch'),2200);setTimeout(()=>intro.remove(),3300);},4250);
  }
