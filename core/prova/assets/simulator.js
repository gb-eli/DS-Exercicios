(()=>{
  ...
  function hero(title,sub){return `<div class="sim-student-top"><div><span class="brand-mark small">DS</span><div><strong>Modo Prova Prática</strong><small>${esc(tpl()?.subject_name||'')}</small></div></div><span class="status-pill warn">SIMULAÇÃO</span></div><div class="sim-page-title"><p class="eyebrow">${esc(sub)}</p><h2>${esc(title)}</h2></div>`;}
  ...
  function renderRoom(){
    ...
    return `${hero(state.roomName,'Sala da equipe')}
      <div class="sim-company-banner"><div><p class="eyebrow">Empresa fictícia</p><h2>${esc(state.company.name)}</h2><p>${esc(state.company.city)} • ${esc(state.company.cnpj)} • ${esc(state.company.phone)}</p></div><span class="status-pill live">Pré-lobby aberto</span></div>
