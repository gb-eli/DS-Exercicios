...
A versão 2.3 não publica respostas literais no catálogo de missões. Respostas determinísticas utilizam comprovantes AES-GCM selados; desafios com soluções equivalentes utilizam testes estruturais. Essa proteção reduz vazamentos por pesquisa no código, mas não substitui um backend. Consulte `ANTI_LEAK_SECURITY.md`.

## Resposta a adulterações

A plataforma não aplica banimento automático. Divergências de carteira, XP, inventário, extrato ou padrões atípicos de conclusão geram bloqueio proporcional e registro para revisão humana.
