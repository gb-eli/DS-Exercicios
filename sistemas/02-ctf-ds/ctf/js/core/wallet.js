...
const createWallet = (profile) => ({
  ...
  migration: { fromLegacyFields: true, at: new Date().toISOString(), appVersion: '3.2.0' },
});
...
const rawAppend = (profile, input) => {
  ...
  const tx = {
    ...
    appVersion: '3.2.0', toolVersion: input.toolVersion || '', deviceSessionId: input.deviceSessionId || '',
...
const reconcileCoreMirror = (profile) => {
  const core = profile?.core || {};
  const coins = Math.max(0, Math.trunc(Number(core.wallet?.balance || 0)));
  const xp = Math.max(0, Math.trunc(Number(core.metrics?.xp || 0)));
  const stars = Math.max(0, Math.trunc(Number(profile.stars || 0)));
  profile.coins = coins;
  profile.xp = xp;
  profile.stars = stars;
  profile.inventory = Array.isArray(profile.inventory) ? profile.inventory : [...DEFAULT_ITEMS];
  profile.badges = Array.isArray(profile.badges) ? profile.badges : [...DEFAULT_BADGES];
  profile.wallet = {
    ...(profile.wallet || {}),
    schemaVersion: WALLET_SCHEMA_VERSION,
    status: 'VALID',
    requiresReview: false,
    coreMirror: true,
    ledger: Array.isArray(profile.wallet?.ledger) ? profile.wallet.ledger : [],
    balances: {
      COINS: { available: coins, pending: 0, blocked: 0 },
      XP: { available: xp, pending: 0, blocked: 0 },
      STARS: { available: stars, pending: 0, blocked: 0 },
    },
    incidents: Array.isArray(profile.wallet?.incidents) ? profile.wallet.incidents : [],
    headHash: profile.wallet?.headHash || GENESIS,
    lastReconciledAt: new Date().toISOString(),
  };
  return { profile, valid: true, errors: [], balances: cloneBalances(profile.wallet.balances), inventory: [...profile.inventory], badges: [...profile.badges] };
};

export const reconcileProfileState = (profile, { recordIncidents = true, clearReview = false } = {}) => {
  if (profile?.core?.authority === 'agv-core') return reconcileCoreMirror(profile);
...
export const awardToProfile = (profile, { sourceId, source = 'activity', coins = 0, xp = 0, stars = 0, metadata = {} }) => {
  if (profile?.core?.authority === 'agv-core') {
    if (stars) profile.stars = Math.max(0, Math.trunc(Number(profile.stars || 0) + Number(stars || 0)));
    if (coins || xp) {
      profile.core.pendingLocalRewards ||= [];
      profile.core.pendingLocalRewards.push({ sourceId, source, coins, xp, at: new Date().toISOString(), metadata });
      profile.core.pendingLocalRewards = profile.core.pendingLocalRewards.slice(-100);
    }
    return profile;
  }
...
export const spendXp = (profile, amount, sourceId, metadata = {}) => {
  if (profile?.core?.authority === 'agv-core') throw new Error('Gasto de XP temporariamente bloqueado enquanto hints migram para o AGV Education Core.');
...
export const grantBadge = (profile, badge, sourceId = '') => {
  if (profile?.core?.authority === 'agv-core') { profile.badges ||= []; if (profile.badges.includes(badge)) return false; profile.badges.push(badge); return true; }
  reconcileProfileState(profile);
...
export const purchaseStoreItem = (profile, item) => {
  if (profile?.core?.authority === 'agv-core') throw new Error('Cyber Store local desativada no piloto. Compras serão liberadas quando o inventário central estiver conectado.');
...
export const getWalletSummary = (profile) => {
  if (profile?.core?.authority === 'agv-core') {
    const balance = Math.max(0, Math.trunc(Number(profile.core?.wallet?.balance ?? profile.coins ?? 0)));
    const xp = Math.max(0, Math.trunc(Number(profile.core?.metrics?.xp ?? profile.xp ?? 0)));
    return {
      status: profile.core?.wallet?.status === 'active' ? 'VALID' : 'BLOCKED',
      coins: { available: balance, pending: 0, blocked: 0 },
      xp: { available: xp, pending: 0, blocked: 0 },
      stars: { available: Math.max(0, Math.trunc(Number(profile.stars || 0))), pending: 0, blocked: 0 },
      transactionCount: Number(profile.core?.storeTransactions?.length || 0),
      incidents: [],
      headHash: 'AGV-CORE',
      valid: profile.core?.wallet?.status !== 'blocked',
    };
  }
  const result = reconcileProfileState(profile);
  return {
...
export const walletRecentTransactions = (profile, limit = 12) => {
  if (profile?.core?.authority === 'agv-core') {
    return (profile.core?.storeTransactions || []).slice(0, limit).map((tx) => ({
      id: tx.id, type: 'PURCHASE', currency: 'COINS', amount: -Math.abs(Number(tx.amount || 0)),
      source: 'AGV Education Core', sourceId: tx.metadata?.item_id || 'store', itemId: tx.metadata?.item_id || '',
      createdAt: tx.created_at || new Date().toISOString(), core: true,
    }));
  }
  reconcileProfileState(profile);
