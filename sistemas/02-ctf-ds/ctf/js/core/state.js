...
import { normalizeMissionDrafts } from './mission-progress.js';
import { normalizeMissionBlocks } from './mission-blocks.js';
import { normalizeNarrativeProfile } from './narrative-engine.js';
...
export const createDefaultProfile = (accountId, studentName = 'Aluno', className = 'Turma não informada') => ({
  ...
  missionDrafts: {},
  missionBlocks: {},
  narrative: { version: 1, outcomes: {}, arcs: {}, lastMissionId: '', updatedAt: 0 },
  skills: {
    'Defesa': 5,
    'Web': 5,
    'Criptografia': 5,
  ...
  settings: {
    ...
    qualityPreset: 'auto',
    focusMode: false,
    preferLandscape3d: true,
    autoFallback3d: true,
    deviceDiagnostics: { version: 0, capturedAt: 0, recommendedQuality: '', support: {}, hardware: {}, storage: {}, benchmark: {}, viewport: {}, network: {}, preferences: {} },
  },
});
...
export const normalizeProfile = (profile, accountId = profile?.accountId || profile?.username || 'operator', studentName = profile?.studentName || 'Aluno', className = profile?.className || 'Turma não informada') => {
  ...
  const normalized = {
    ...
    missionDrafts: { ...base.missionDrafts, ...(profile?.missionDrafts || {}) },
    missionBlocks: { ...base.missionBlocks, ...(profile?.missionBlocks || {}) },
    narrative: { ...base.narrative, ...(profile?.narrative || {}), outcomes: { ...base.narrative.outcomes, ...(profile?.narrative?.outcomes || {}) }, arcs: { ...base.narrative.arcs, ...(profile?.narrative?.arcs || {}) } },
  ...
  normalizeMissionDrafts(normalized);
  normalizeMissionBlocks(normalized);
  normalizeNarrativeProfile(normalized);
  normalized.storageSchemaVersion = 15;
