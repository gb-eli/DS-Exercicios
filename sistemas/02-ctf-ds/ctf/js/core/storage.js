...
export const coreLinkedAccountId = (userId) => `core_${String(userId || '').replace(/[^a-f0-9-]/gi, '').slice(0, 40)}`;

export const registerCoreLinkedAccount = async (userId, studentName, className, password, initialProfile = null) => {
  const accountId = coreLinkedAccountId(userId);
  const existing = await dbGet(PROFILE_STORE, accountId);
  if (existing) throw new Error('O cache central já existe neste dispositivo.');
  const dataKey = await crypto.subtle.generateKey({ name: 'AES-GCM', length: 256 }, true, ['encrypt', 'decrypt']);
  unlockedKeys.set(accountId, dataKey);
  const now = Date.now();
  const profile = {
    ...
    accountId,
    username: accountId,
    studentName: String(studentName || 'Aluno').trim(),
    className: String(className || 'Turma não informada').trim(),
    createdAt: initialProfile?.createdAt || now,
    updatedAt: now,
    expiresAt: now + TTL_MS,
    storageSchemaVersion: 15,
    core: { ...(initialProfile?.core || {}), linkedUserId: userId, authority: 'agv-core' },
  };
  await appendAuditEvent(profile, 'core_profile_linked', { className: profile.className, authority: 'agv-core' });
  const record = {
    accountId,
    loginHash: accountId,
    schema: SCHEMA,
  ...
  await dbPut(PROFILE_STORE, record);
  return { accountId, studentName: profile.studentName, className: profile.className, coreLinked: true };
};

export const authenticateCoreLinkedAccount = async (userId, password) => {
  const accountId = coreLinkedAccountId(userId);
  const record = await dbGet(PROFILE_STORE, accountId);
  if (!record) return null;
  const result = await authenticateLocalAccount('', '', password, accountId);
  return { ...result, coreLinked: true };
};

export const resetCoreLinkedCache = async (userId) => removeProfile(coreLinkedAccountId(userId));
