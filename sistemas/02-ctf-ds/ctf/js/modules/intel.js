...
const links = (items = []) => items.map((source) => `<a class="source-link" href="${escapeHtml(safeExternalUrl(source.url))}" target="_blank" rel="noopener noreferrer">${escapeHtml(source.label)} ↗</a>`).join('');
...
const renderCaseCard = (item) => `<article class="case-card card">
  ...
  <a class="source-link" href="${escapeHtml(safeExternalUrl(item.source.url))}" target="_blank" rel="noopener noreferrer">${escapeHtml(item.source.label)} ↗</a>
</article>`;
...
export const renderVulnerabilityModal = (item, profile) => {
  ...
  return `<div class="modal-layer" data-modal-close="true"><article class="intel-modal panel" role="dialog" aria-modal="true" aria-labelledby="intel-title">
    ...
    ${cases.length ? `<section class="lesson-section"><h3>CASOS REAIS RELACIONADOS</h3><div class="mini-case-list">${cases.map((itemCase) => `<div><strong>${itemCase.year} // ${escapeHtml(itemCase.company)}</strong><p>${escapeHtml(itemCase.title)}</p><a class="source-link" href="${escapeHtml(safeExternalUrl(itemCase.source.url))}" target="_blank" rel="noopener noreferrer">Abrir fonte oficial ↗</a></div>`).join('')}</div></section>` : ''}
