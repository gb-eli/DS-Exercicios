...
import { missionSequence, missionBlocks, getActiveMissionBlock, getBlockState, getCampaignBlockSummary, isMissionBlockUnlocked } from '../core/mission-blocks.js';
...
export const renderDashboard = (profile) => {
  ...
  const unlocked = challenges.filter((item) => isMissionBlockUnlocked(profile, item) && isUnlocked(item, profile.completed)).length;
  ...
  const nextMission = missionSequence.find((item) => isMissionBlockUnlocked(profile, item) && isUnlocked(item, profile.completed) && !profile.completed[item.id]);
  ...
  return `
    ...
    <div class="grid grid-4" style="margin-top:16px">
      ...
      <article class="metric-card card ${wallet.status !== 'VALID' ? 'wallet-blocked' : ''}"><div class="metric-top"><span>CYBER COINS</span><span class="metric-icon">◇</span></div><div class="metric-value">${formatNumber(profile.coins)}</div><div class="metric-sub">${wallet.status === 'VALID' ? 'Saldo derivado do extrato · apenas personalização' : 'Carteira bloqueada para conferência'}</div></article>
    ...
    <div class="section-title"><h2>PROGRESSÃO POR BLOCOS</h2><small>CHECKPOINTS A CADA PACOTE DE MISSÕES</small></div>
    <article class="card dashboard-block-checkpoint"><div><p class="eyebrow">BLOCO ATUAL ${String(activeBlock.number).padStart(2, '0')} // ${campaignBlocks.completed}/${campaignBlocks.total} CHECKPOINTS</p><h2>${escapeHtml(activeBlock.title)}</h2><p>${escapeHtml(activeBlock.subtitle)}</p><div class="progress-track"><span class="progress-fill" style="width:${activeBlockState.percentage}%"></span></div><small>${activeBlockState.completed}/${activeBlockState.total} missões · desempenho ${escapeHtml(activeBlockState.performance)} · ${activeBlockState.percentage}%</small></div><div class="dashboard-block-actions"><span>${escapeHtml(activeBlock.icon)}</span><strong>${activeBlockState.percentage}%</strong><button class="primary-button" data-open-block="${activeBlock.id}">ABRIR BLOCO</button></div></article>
    <div class="dashboard-block-mini-grid">${missionBlocks.map((block) => { const state = getBlockState(profile, block); return `<button class="dashboard-block-mini ${state.state}" data-open-block="${block.id}" ${state.unlocked ? '' : 'disabled'}><span>${String(block.number).padStart(2, '0')}</span><div><strong>${escapeHtml(block.title)}</strong><small>${state.completed}/${state.total} · ${state.complete ? 'checkpoint concluído' : state.state === 'locked' ? 'bloqueado' : `${state.percentage}%`}</small></div><b>${state.complete ? '✓' : state.state === 'locked' ? '🔒' : '→'}</b></button>`; }).join('')}</div>

    <div class="section-title"><h2>TRILHAS DE ESPECIALIZAÇÃO</h2><small>EXPLORE AS COMPETÊNCIAS TÉCNICAS DA CAMPANHA</small></div>
    ...
    <article class="card intel-callout"><div><strong>Aprender com incidentes reais sem reproduzir ataques:</strong><p>Consulte dossiês sobre aplicações, bancos, Pix, criptoativos, APIs, mobile, nuvem, IoT, supply chain, IA e privacidade.</p></div><button class="secondary-button" data-view="intel">ABRIR THREAT INTEL</button></article><article class="card educational-use-banner"><div><strong>FINALIDADE EDUCACIONAL E ESCOPO AUTORIZADO</strong><p>Use as técnicas somente nos simuladores locais e nos ambientes indicados pelo professor. Toda prática ofensiva deve vir acompanhada de prevenção, detecção e mitigação.</p></div><button class="secondary-button" data-open-terms>CONSULTAR REGRAS</button></article>`;
};
