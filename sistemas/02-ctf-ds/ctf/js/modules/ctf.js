...
export const renderMissionCard = (challenge, profile) => {
  ...
  const coreProvisioned = profile.core?.authority !== 'agv-core' || (profile.core?.provisionedChallengeIds || []).includes(challenge.id);
  const unlocked = progress.id !== 'locked' && coreProvisioned;
  ...
  const reason = !coreProvisioned ? 'Aguardando provisionamento no AGV Education Core' : !blockState?.unlocked ? `Conclua o Bloco ${String((block?.number || 1) - 1).padStart(2, '0')} e o checkpoint anterior` : prerequisiteReason;
  ...
  const statusText = !coreProvisioned ? 'Aguardando Core' : progress.id === 'in-progress' ? `Salvo ${formattedTime(draft?.lastSavedAt)}` : progress.label;
