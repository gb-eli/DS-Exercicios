...
import { simulatorCatalog, isSimulationTool, renderSimulationWorkspace, runSimulation } from './simulation-suite.js';
import { immersiveCatalog, isImmersiveTool, renderImmersiveWorkspace } from './immersive-suite.js';
...
export const renderToolWorkspace = (toolId = 'base64', context = {}) => {
  if (isImmersiveTool(toolId)) return `<div class="simulation-suite-banner immersive-suite-banner"><div><span>IMMERSIVE 3D/360</span><small>Ambiente procedural, local e carregado sob demanda, com fallback 2D completo.</small></div><button type="button" class="secondary-button compact" data-open-terms>ESCOPO AUTORIZADO</button></div>${renderImmersiveWorkspace(toolId, context)}`;
  if (isSimulationTool(toolId)) return `<div class="simulation-suite-banner"><div><span>SIMULATION SUITE</span><small>Interface realista com dados fictícios, sem conexão externa.</small></div><button type="button" class="secondary-button compact" data-open-terms>ESCOPO AUTORIZADO</button></div>${renderSimulationWorkspace(toolId, context)}`;
  return `<div class="tool-tutorial-inline"><div><span>TUTORIAL DISPONÍVEL</span><small>Veja o cursor virtual preencher e executar um exemplo seguro.</small></div><button type="button" class="secondary-button compact" data-start-tool-tutorial="${toolId}">▶ COMO USAR</button></div>${workspace(toolId)}`;
};
...
export const runTool = async (toolId, formData) => {
  if (isSimulationTool(toolId)) return runSimulation(toolId, formData);
