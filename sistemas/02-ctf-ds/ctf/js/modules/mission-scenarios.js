...
export const renderMissionScenario = (challenge, draft = null) => {
    ...
    <section class="mission-simulator" aria-label="Cenário visual da missão">
      ...
      <div class="evidence-board" data-evidence-board>
        ...
        <div class="evidence-chips">
          ${scene.clues.map((clue, index) => { const selected = draft?.evidence?.includes(String(index)); return `<button type="button" class="evidence-chip ${selected ? 'selected' : ''}" data-evidence-clue="${index}" aria-pressed="${selected ? 'true' : 'false'}"><span>0${index + 1}</span>${escapeHtml(clue)}</button>`; }).join('')}
        </div>
        <output class="evidence-counter">${draft?.evidence?.length || 0}/${scene.clues.length} pistas marcadas</output>
      </div>
    </section>`;
};
