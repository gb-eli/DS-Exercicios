(function(){
  ...
  async function unlockRecord(record,password,eventType='profile_unlocked'){
    const wasExpired=new Date(record.expiresAt).getTime()<Date.now();
    const dek=await unwrapDekWithPassword(record.studentWrap,password,record.id);
    const data=await decryptPayload(dek,record);
    if(wasExpired){record.expiresAt=addDays(nowIso(),EXPIRATION_DAYS);record.updatedAt=nowIso();await appendAudit(data,'profile_reactivated',{previousExpiration:true});}
    currentSession={type:'profile',id:record.id,record,dek,data,dirty:wasExpired};
    await appendAudit(data,eventType,{reactivated:wasExpired});queueSave(30);resetLockTimer();broadcast('unlocked',{reactivated:wasExpired});return clone(data);
  }
  async function unlockProfile(id,password){const record=await storeGet(PROFILE_STORE,id);if(!record)throw new Error('Perfil não encontrado.');try{return await unlockRecord(record,password);}catch(error){if(/OperationError|decrypt|unwrap/i.test(String(error?.name||error?.message||'')))throw new Error('Senha local incorreta. Verifique e tente novamente.');throw new Error(error?.message||'Não foi possível abrir o perfil.');}}
  ...
  async function listProfiles(){const rows=await storeAll(PROFILE_STORE);const now=Date.now();return rows.sort((a,b)=>new Date(b.updatedAt)-new Date(a.updatedAt)).map(row=>({id:row.id,displayName:row.displayName,classKey:row.classKey,updatedAt:row.updatedAt,expiresAt:row.expiresAt,expired:new Date(row.expiresAt).getTime()<now,hasRecovery:!!row.teacherWrap}));}
  ...
  async function init(){
    ...
    window.addEventListener('pagehide',()=>{if(currentSession?.type==='profile'&&currentSession.dirty)saveNow();});
    document.addEventListener('visibilitychange',()=>{if(document.hidden&&currentSession?.type==='profile'&&currentSession.dirty)saveNow();});
  }
