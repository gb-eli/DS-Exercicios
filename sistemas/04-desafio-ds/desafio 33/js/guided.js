(function(){
  ...
  const STEP_NAMES = ['Tema e contexto','O que é e para que serve','Exemplo explicado','Apoio e contexto','Demonstração','Prática guiada','Laboratório interno','Atividade prática','Retomada e fixação','Comprovante final'];
      ...
      unitLessons.forEach(lesson=>{
        ...
        article.innerHTML=`
          ...
          <div class="guided-lesson-card-main">
            <div class="guided-lesson-tags"><span>${escapeHtml(lesson.module)}</span><span>${progress.completed?'Concluída':progress.unlocked?'Em andamento':'Autorização necessária'}</span>${lesson.legacyNumber&&lesson.legacyNumber!==displayNumber(lesson)?`<span class="legacy-tag">origem ${lesson.legacyNumber}</span>`:''}${lesson.pilotV33?'<span class="v33-pilot-tag">Aula piloto v33</span>':''}</div>
  ...
  function pilotContext(){
    return {
      lesson:currentLesson, progress:currentProgress, container:el.stepContent,
      markComplete:markStepComplete, save:saveProgress, feedback:showFeedback, log:logEvent
    };
  }
  function renderPilotStep(name){
    return !!window.DS_PilotV33?.render?.(name,pilotContext());
  }
  ...
  function decorateStepContent(){
    ...
    if(currentLesson.pilotV33)return;
  ...
  function renderIntro(){
    if(renderPilotStep('intro'))return;
  ...
  function renderDiagnostic(){
    if(renderPilotStep('concept'))return;
  ...
  function renderMainExplanation(){
    if(renderPilotStep('example'))return;
  ...
  function renderExtraSelection(){
    if(renderPilotStep('support'))return;
  ...
  function renderDemonstration(){
    if(renderPilotStep('demo'))return;
  ...
  function renderGuidedPractice(){
    if(renderPilotStep('practice'))return;
  ...
  function renderChallenge(){
    if(renderPilotStep('activity'))return;
  ...
  function renderReview(){
    if(renderPilotStep('review'))return;
