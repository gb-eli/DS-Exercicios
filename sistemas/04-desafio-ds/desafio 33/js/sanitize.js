(function(){
  ...
  function safeUrl(value,{allowRelative=true,allowHttp=false}={}){
  ...
  function assertSafeStructure(value,{maxDepth=16,maxKeys=5000}={}){
    let count=0;
    const visit=(node,depth)=>{
      if(depth>maxDepth)throw new Error('Estrutura acima da profundidade permitida.');
      if(node===null||typeof node!=='object')return;
      if(Array.isArray(node)){node.forEach(item=>visit(item,depth+1));return;}
      const proto=Object.getPrototypeOf(node);
      if(proto!==Object.prototype&&proto!==null)throw new Error('Objeto com protótipo não permitido.');
      for(const key of Object.keys(node)){
        count+=1;if(count>maxKeys)throw new Error('Arquivo possui campos demais.');
        if(['__proto__','prototype','constructor'].includes(key))throw new Error('Campo potencialmente perigoso rejeitado.');
        visit(node[key],depth+1);
      }
    };
    visit(value,0);return value;
  }
  function parseJsonSafe(source,options={}){
    const text=String(source??'');
    if(text.length>(options.maxChars||6_000_000))throw new Error('Arquivo maior que o limite permitido.');
    const parsed=JSON.parse(text);
    return assertSafeStructure(parsed,options);
  }
  ...
  window.DS_Sanitize = Object.freeze({text,multiline,name,escapeHtml,sanitizePreviewHtml,sanitizePreviewCss,safeUrl,assertSafeStructure,parseJsonSafe,safeStylePercent});
})();
