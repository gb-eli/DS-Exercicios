...
(function(){
  ...
  async function optionalRest(path,fallback=[]){try{return await rest(path)}catch(error){console.warn('[LABDS] consulta opcional indisponível',error);return fallback}}
  function normalizeAdaptation(row){
    if(!row?.config)return null;
    const config=row.config&&typeof row.config==='object'?row.config:{};
    const features=config.features&&typeof config.features==='object'?config.features:{};
    const supervision=config.supervision&&typeof config.supervision==='object'?config.supervision:{};
    return {
      id:row.id||null,
      profileKey:String(config.profile_key||config.adaptation_profile||'guided'),
      defaultMode:String(config.default_mode||'conventional')==='adapted'?'adapted':'conventional',
      features:{
        shortInstructions:features.short_instructions!==false,stepByStep:features.step_by_step!==false,focusCues:features.focus_cues!==false,
        largerControls:Boolean(features.larger_controls),reducedVisualLoad:Boolean(features.reduced_visual_load),predictableFeedback:Boolean(features.predictable_feedback),
        motorFriendly:Boolean(features.motor_friendly),microSteps:Boolean(features.micro_steps),extraHelp:Boolean(features.extra_help),homeDetailedGuidance:Boolean(features.home_detailed_guidance)
      },
      supervision
    };
  }
  async function loadIdentity(user){const profiles=await rest(`profiles?select=id,full_name,email,role,active,must_change_password&id=eq.${encode(user.id)}&limit=1`);const profile=profiles?.[0];if(!profile?.active)throw new Error('Conta inativa ou perfil central não localizado.');const memberships=await rest(`class_memberships?select=class_id,is_primary&user_id=eq.${encode(user.id)}&active=eq.true&order=is_primary.desc&limit=1`);let classInfo=null;if(memberships?.[0]?.class_id){const classes=await rest(`classes?select=id,code,name,shift&id=eq.${encode(memberships[0].class_id)}&limit=1`);classInfo=classes?.[0]||null;}const accommodations=profile.role==='student'?await optionalRest(`student_accommodations?select=id,config,updated_at&student_id=eq.${encode(user.id)}&exercise_id=is.null&accommodation_type=eq.learning_mode&active=eq.true&order=updated_at.desc&limit=1`,[]):[];const adaptation=normalizeAdaptation(accommodations?.[0]);if(adaptation){const preferences=await optionalRest(`pedagogical_adaptation_preferences?select=mode&student_id=eq.${encode(user.id)}&adaptation_key=eq.${encode(adaptation.profileKey)}&limit=1`,[]);const preferred=String(preferences?.[0]?.mode||'');adaptation.mode=preferred==='adapted'||preferred==='conventional'?preferred:adaptation.defaultMode;}identity={user,profile,classInfo,adaptation};return identity;}
  ...
  function markCoreDegraded(message='Sincronização temporariamente indisponível'){updateStatus('AGV Core • prática disponível','warn');console.warn('[LABDS] Core em modo degradado:',message);}
  ...
  async function requireSession(){
    ...
    if(info&&!info.profile.must_change_password){
      ...
      updateStatus(info.adaptation?.mode==='adapted'?'AGV Core • apoio pedagógico ativo':'AGV Core • conectado');
      return info;
    }
  ...
  window.LABDS.AGVCore={requireSession,signOut,loadCoreState,toolOpened,completeActivity,getIdentity:()=>identity,getAdaptation:()=>identity?.adaptation||null,markCoreDegraded,hasSession:()=>Boolean(loadStoredSession()?.access_token),info:Object.freeze({platformId:'lab-virtual',mode:'core-authority',supabaseUrl:SUPABASE_URL})};
})();
