...
function load() {
  ...
  try {
    ...
    order.forEach(key => {
      if (state.support[key]?.activeBlocks?.length) state.origin[key] = 'misto';
      state.support[key].markerMissing = Boolean(state.support[key]?.activeBlocks?.length && !AutoCompleteSupport.hasMarker(state.answers[key], key));
    });
...
function renderTabs() {
  $('#studentFileTabs').innerHTML = order.map(key => `
    <button data-file="${key}" class="${file === key ? 'active' : ''} ${state.done[key] ? 'done' : ''}" ${state.unlocked[key] ? '' : 'disabled'}>${Utils.escapeHtml(physicalPath(key))}</button>
  `).join('');
...
function renderPractice() {
  ...
  const steps = exercise().passos[file] || [];
  const studiedIndex = Math.max(0, Math.min(steps.length - 1, (state.explained[file] || 1) - 1));
  ...
  $('#referenceFileName').textContent = showFullReference
    ? `${physicalPath(file)} · arquivo completo para digitar/adaptar`
  ...
  if ($('#referenceHelp')) $('#referenceHelp').textContent = showFullReference
    ? 'Este é o arquivo completo de referência. Use-o para digitar ou adaptar o arquivo solicitado; a plataforma não cola a solução inteira automaticamente.'
    : 'Este painel mostra somente o trecho estudado. Use “Ver arquivo completo” quando precisar conferir a estrutura inteira.';
...
function currentSupportBlocks() {
  return AutoCompleteSupport.blocks(exercise(), file);
}
...
function openAutocompleteModal() {
  ...
  const available = blocks.filter(block => !state.support[file]?.activeBlocks?.some(active => active.id === block.id));
  if (!available.length) {
    Utils.toast('Todos os blocos autorizados deste arquivo já foram utilizados.');
    return;
  }
  autocompleteSelection = available[0].id;
  $('#autocompleteBlock').innerHTML = available.map(block => `<option value="${Utils.escapeHtml(block.id)}">${Utils.escapeHtml(block.title)}</option>`).join('');
  $('#autocompleteBlock').value = autocompleteSelection;
  const modal = $('#autocompleteModal');
  modal.hidden = false;
  modal.setAttribute('aria-hidden', 'false');
  setModalBackgroundInert(true);
  renderAutocompleteModal();
  setTimeout(() => $('#autocompleteBlock')?.focus(), 0);
}
...
function confirmAutocomplete() {
  const block = selectedAutocompleteBlock();
  const eligibility = supportEligibility(block);
  if (!block || !eligibility.unlocked) return;
  const currentCode = $('#studentEditor').value;
  const result = AutoCompleteSupport.insertBlock(currentCode, file, block);
  if (!result.ok) {
    $('#autocompleteEligibility').innerHTML = `<strong>Não foi possível inserir</strong><p>${Utils.escapeHtml(result.message)}</p>`;
    return;
  }
  const support = state.support[file] || (state.support[file] = defaultSupportState());
  const event = {
    id: crypto.randomUUID ? crypto.randomUUID() : `${Date.now()}-${Math.random().toString(16).slice(2)}`,
    blockId: block.id,
    title: block.title,
    file,
    fileName: physicalPath(file),
    previousCode: currentCode,
    insertedAt: new Date().toISOString(),
    linesInserted: result.linesInserted,
    approximatePercentage: AutoCompleteSupport.approximatePercentage(block, exercise().arquivos[file]),
    origin: eligibility.origin,
    reason: eligibility.reason,
    authorizationId: eligibility.authorization?.id || null,
    platformVersion: window.APP_CONFIG?.version || ''
  };
  support.everUsed = true;
  support.activeBlocks.push(event);
  support.history.push({ action: 'insert', ...event });
  support.reviewConfirmed = false;
  support.markerMissing = false;
  state.answers[file] = result.code;
  state.activity[file].executions = 0;
  state.execution[file] = ExecutionFlow.invalidate(state.execution[file]);
  resetCompletionAfterEdit();
  ...
  $('#studentEditor').value = result.code;
  if (eligibility.authorization?.id) AutoCompleteSupport.consumeAuthorization(eligibility.authorization.id, window.AppAuth?.currentUser?.()?.username || '');
  window.AppAuth?.log('autocompletar_utilizado', {
    arquivo: file,
    bloco: block.id,
    linhas: result.linesInserted,
    percentualAproximado: event.approximatePercentage,
    origem: eligibility.origin,
    motivo: eligibility.reason,
    tempoAtivo: state.activity[file].activeSeconds,
    tentativas: state.attempts[file],
    dicas: state.activity[file].hintsUsed
  });
  save();
  syncEditor();
  renderPreviewStale();
...
function renderDeliveryGuide() {
  ...
  $('#deliveryNextStep').textContent = steps.every(item => item.done) ? `${prefix}Pronto para baixar o ZIP e abrir o Classroom.${supportNote}` : `${prefix}Próximo passo: ${next}.${supportNote}`;
}
...
function updatePhase() {
  if (view === 'explain') {
    ...
    $('#phaseText').textContent = 'O arquivo completo não é exibido no modo Aluno. Reveja os trechos explicados ou comece a prática.';
...
function renderProgress() {
  ...
  else if (state.completion?.confirmedAt || view === 'completion') percentage = 98;
...
function renderCompletion() {
  ...
  const evidence = state.completion?.evidence;
  $('#completionStatus').textContent = evidence ? 'Atividade concluída · evidência gerada' : 'Conclusão confirmada · evidência pendente';
...
function renderWorkspace() {
  ...
  tree.querySelectorAll('[data-workspace-open]').forEach(button => button.addEventListener('click', () => {
    ...
    file = key; view = 'practice'; renderAll(); VSCodeEditor?.focus?.();
  }));
...
function showWorkspaceSupportFile(key) {
  ...
  $('#referenceFileName').textContent = `${physicalPath(key)} · arquivo de apoio`;
  if ($('#referenceModeLabel')) $('#referenceModeLabel').textContent = 'somente leitura';
  if ($('#referenceHelp')) $('#referenceHelp').textContent = 'Este arquivo acompanha o projeto e será incluído no ZIP. Ele não faz parte da validação principal deste exercício.';
  if ($('#toggleReferenceMode')) { $('#toggleReferenceMode').hidden = true; }
  $('#toggleReference').textContent = 'Ocultar referência';
  Utils.toast(`${physicalPath(key)} aberto como arquivo de apoio somente leitura.`);
}
...
function openInfo(type) {
  ...
  if (type === 'dicas') { registerActivity('hint'); AppShell.openInfo('Dicas para realizar a atividade', Utils.tipsHtml(exercise(), false)); renderSupportPanel(); }
  if (type === 'explicacao') {
