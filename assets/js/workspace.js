import { runPython } from './python-runtime.js?v=14.10.8.36';
import { EXERCISE_MANIFEST } from '../data/exercise-manifest.js?v=14.10.8.36';
import { EXERCISE_MANIFEST_CURRENT } from '../data/exercise-manifest-current.js?v=14.10.8.36';
import { EXERCISE_REFERENCES } from '../data/exercise-reference.js?v=14.10.8.36';
import { EXERCISE_REFERENCE_EXTRAS } from '../data/exercise-reference-extra.js?v=14.10.8.36';
import { EXERCISE_REFERENCE_SYNCED } from '../data/exercise-reference-synced.js?v=14.10.8.36';
import { EXERCISE_REFERENCE_3DS_RESTORED } from '../data/exercise-reference-3ds-restored.js?v=14.10.8.36';
import { EXERCISE_REFERENCE_DS2_CORRECTED } from '../data/exercise-reference-ds2-corrected.js?v=14.10.8.36';
import { EXERCISE_REFERENCE_CATALOG_CURRENT } from '../data/exercise-reference-catalog-current.js?v=14.10.8.36';
import { validateExercise, renderValidation } from './validation.js?v=14.10.8.36';
import {
...
} from './supervision.js?v=14.10.8.36';

import { supabase, handleSessionInvalid } from './supabase.js?v=14.10.8.36';
import { createStoreZip, downloadBlob, downloadTextFile } from './downloads.js?v=14.10.8.36';
import { buildHtmlPreview } from './preview-builder.js?v=14.10.8.36';
import { shouldRecoverCachedDraft } from './draft-recovery.js?v=14.10.8.36';
import { getWeekendWindow, formatWeekendCountdown, buildWeekendDiagnostics, WEEKEND_SUPPORT_TIME_ZONE } from './weekend-support.js?v=14.10.8.36';
import { callWeekendVoucher, copyWeekendVoucherCode } from './weekend-voucher.js?v=14.10.8.36';
...
function persistLocalDraft(file,content){
  ...
  try{
    localStorage.setItem(cacheKey(file),JSON.stringify({
      ...
      editSequence:Number(state.editSequences.get(file.id)||0)
    }));
    return true;
  }catch(error){
...
function mirrorSaveState(text,cls=''){
  ...
  if(!pill)return;
  const normalized=cls==='error'?'error':cls==='saving'?'saving':cls==='pending'?'pending':'ok';
  pill.dataset.state=normalized;
  pill.textContent=normalized==='error'?'Rascunho preservado':normalized==='saving'?'Salvando…':normalized==='pending'?'Alterado':'Salvo';
...
async function ensureFiles(){
  ...
  remote.forEach(file=>{
    ...
    state.editSequences.set(file.id,0);
    state.confirmedSequences.set(file.id,0);
...
function formatSaveTime(v){
  ...
  try{return new Intl.DateTimeFormat('pt-BR',{hour:'2-digit',minute:'2-digit'}).format(date);}
  catch{return date.toLocaleTimeString?.('pt-BR',{hour:'2-digit',minute:'2-digit'})||'';}
}

function currentEditSequence(fileId){return Number(state.editSequences.get(fileId)||0);}
function nextEditSequence(fileId){
  const next=currentEditSequence(fileId)+1;
  state.editSequences.set(fileId,next);
  return next;
}
function saveController(fileId){
  let controller=state.saveControllers.get(fileId);
  if(!controller){
    controller={pending:null,inFlight:null,running:null,lastResult:{synced:true,skipped:true}};
    state.saveControllers.set(fileId,controller);
  }
  return controller;
}
function isLatestSequence(fileId,sequence){return Number(sequence)>=currentEditSequence(fileId);}
...
function scheduleSave(){
  ...
  const sequence=nextEditSequence(fileId);
  const content=state.active.content;
  const localProtected=persistLocalDraft(state.active,content);
  setSaveState(localProtected?'Alterações protegidas — aguardando sincronização':'Alterações pendentes — cópia local indisponível neste navegador',localProtected?'pending':'error');

  // A edição entra imediatamente na ÚNICA pendência do arquivo. O debounce atrasa
  // apenas o início da rede. Assim, uma barreira (troca de arquivo, autocorreção,
  // GitHub, Classroom etc.) já enxerga a versão mais nova e pode forçar o flush.
  const controller=saveController(fileId);
  controller.pending={content:String(content??''),force:Boolean(controller.pending?.force),sequence,notBefore:Date.now()+1200};
  clearTimeout(state.saveTimer);
  state.saveTimer=setTimeout(()=>startFileSaveController(fileId,controller),1200);
}

async function performFileSave(fileId,content,force=false,sequence=currentEditSequence(fileId)){
  const file=state.files.find(f=>f.id===fileId); if(!file)return {synced:false,skipped:true,sequence};
  if(state.lastSavedContent.get(fileId)===content){
    state.confirmedSequences.set(fileId,Math.max(Number(state.confirmedSequences.get(fileId)||0),Number(sequence||0)));
    const current=state.active?.id!==fileId||$('code-editor').value===content;
    if(state.active?.id===fileId&&isLatestSequence(fileId,sequence)){
      state.dirty=!current;
      if(current)setSaveState(`Salvo · ${formatSaveTime(file.saved_at)}`,'ok');
    }
    return {synced:true,current,skipped:true,file,sequence,confirmed:true};
  }

  const localProtected=persistLocalDraft(file,content);
  // Só a versão mais recente pode controlar o indicador "Salvando…".
  if(state.active?.id===fileId&&isLatestSequence(fileId,sequence)){
    setSaveState(localProtected?'Salvando…':'Salvando… — cópia local indisponível',localProtected?'saving':'error');
  }

  let data,result;
  try{
    result=await callStudentFiles({action:'save',exercise_id:state.exercise.id,file_id:fileId,content,client_sequence:Number(sequence||0)});
    data=result.file;
    const explicitlyConfirmed=result.confirmed===true;
    const legacyRowConfirmation=result.confirmed==null&&data&&String(data.content??'')===String(content??'');
    if(!data||(!explicitlyConfirmed&&!legacyRowConfirmation))throw new Error('Servidor não confirmou o arquivo salvo.');
    if(String(data.content??'')!==String(content??''))throw new Error('Confirmação do servidor não corresponde à versão enviada.');
  }catch(error){
    if(error.status===423)setWorkspacePaused(true,error.details?.reason||'Atividade bloqueada pelo professor.');
    // Uma falha antiga não pode sobrescrever o estado visual de uma edição mais nova.
    if(state.active?.id===fileId&&isLatestSequence(fileId,sequence)){
      if(error.code==='malicious_code_rejected'){
        ...
        setSaveState(localProtected?'Não foi possível salvar — rascunho preservado':'Não foi possível salvar — cópia local indisponível','error');
      }
    }
    return {synced:false,error,localProtected,sequence,confirmed:false};
  }

  file.revision=data.revision;
  file.saved_at=data.saved_at;
  state.lastSavedContent.set(fileId,String(data.content??''));

  // Nunca substitui uma edição local mais nova pela resposta de uma requisição anterior.
  ...
  // A resposta antiga só pode confirmar a edição mais nova quando o conteúdo é
  // literalmente idêntico (caso A → B → A). Caso contrário, ela jamais controla
  // o estado visual nem remove o rascunho local da versão atual.
  const confirmsCurrentContent=currentContent===String(data.content??'');
  const latest=isLatestSequence(fileId,sequence)||confirmsCurrentContent;
  const confirmedSequence=confirmsCurrentContent?currentEditSequence(fileId):Number(sequence||0);
  state.confirmedSequences.set(fileId,Math.max(Number(state.confirmedSequences.get(fileId)||0),confirmedSequence));

  if(latest&&confirmsCurrentContent){
    localStorage.removeItem(cacheKey(file));
  }else{
    persistLocalDraft(file,currentContent);
  }

  if(state.active?.id===fileId&&latest){
    state.dirty=currentContent!==data.content;
    if(!state.dirty)setSaveState(`Salvo · ${formatSaveTime(data.saved_at)}`,'ok');
    else setSaveState('Alterações protegidas — aguardando sincronização','pending');
  }
  ...
  return {synced:true,current:currentContent===data.content,file:data,sequence,confirmed:true};
}

async function runFileSaveController(fileId,controller){
  try{
    while(controller.pending){
      // Coalescência: enquanto uma requisição está em trânsito, somente a fotografia
      // mais recente fica pendente; intermediárias nunca viram uma pilha de requests.
      const snapshot=controller.pending;
      // Autosave respeita 1,2 s desde a ÚLTIMA edição. Se uma requisição anterior
      // terminou antes disso, deixa a pendência para o timer já agendado. Barreiras
      // explícitas usam force=true e ignoram esse atraso.
      if(!snapshot.force&&Number(snapshot.notBefore||0)>Date.now())break;
      controller.pending=null;
      controller.inFlight=snapshot;
      controller.lastResult=await performFileSave(fileId,snapshot.content,snapshot.force,snapshot.sequence);
    ...
    if(state.saveQueues.get(fileId))state.saveQueues.delete(fileId);
  }
}

function startFileSaveController(fileId,controller=saveController(fileId)){
  if(controller.running)return controller.running;
  controller.running=runFileSaveController(fileId,controller);
  state.saveQueues.set(fileId,controller.running);
  return controller.running;
}

function saveFileSnapshot(fileId,content,force=false,sequence=null){
  const file=state.files.find(f=>f.id===fileId);
  if(!file)return Promise.resolve({synced:false,skipped:true});
  let seq=Number(sequence||0);
  if(!seq){
    seq=currentEditSequence(fileId);
    const knownLatest=state.lastSavedContent.get(fileId);
    if(!seq||String(knownLatest??'')!==String(content??''))seq=nextEditSequence(fileId);
  }
  const controller=saveController(fileId);

  if(state.lastSavedContent.get(fileId)===content&&!controller.inFlight&&!controller.pending){
    state.confirmedSequences.set(fileId,Math.max(Number(state.confirmedSequences.get(fileId)||0),seq));
    try{localStorage.removeItem(cacheKey(file));}catch(_){}
    return Promise.resolve({synced:true,current:true,skipped:true,file,sequence:seq,confirmed:true});
  }
  if(controller.inFlight?.content===String(content??'')&&!controller.pending&&controller.running){
    return controller.running;
  }
  if(controller.pending?.content===String(content??'')){
    controller.pending.sequence=Math.max(controller.pending.sequence,seq);
    controller.pending.force=Boolean(controller.pending.force||force);
    return startFileSaveController(fileId,controller);
  }

  // Substitui a pendência anterior pela edição mais nova. Nunca cria uma pilha de
  // requisições para cada tecla pressionada.
  if(!controller.pending||seq>=controller.pending.sequence){
    controller.pending={content:String(content??''),force:Boolean(force||controller.pending?.force),sequence:seq,notBefore:0};
  }
  return startFileSaveController(fileId,controller);
}

async function waitForPendingSaves(){
  // Barreira real: troca de arquivo, autocorreção, entrega, GitHub e Classroom
  // não respeitam o debounce; promovem a fotografia pendente para flush imediato.
  for(let pass=0;pass<20;pass+=1){
    const running=[];
    for(const [fileId,controller] of state.saveControllers){
      if(controller.pending){
        controller.pending.force=true;
        controller.pending.notBefore=0;
        if(!controller.running)startFileSaveController(fileId,controller);
      }
      if(controller.running)running.push(controller.running);
    }
    if(!running.length){
      const stillPending=[...state.saveControllers.values()].some(controller=>Boolean(controller.pending));
      if(!stillPending)return;
      continue;
    }
    await Promise.allSettled(running);
  }
  throw new Error('A sincronização demorou mais do que o esperado. O rascunho local foi preservado.');
}
...
async function saveActiveFile(force=false){
  ...
  let sequence=currentEditSequence(state.active.id);
  const controller=saveController(state.active.id);
  const represented=controller.pending?.content===content||controller.inFlight?.content===content||state.lastSavedContent.get(state.active.id)===content;
  if(!sequence||!represented)sequence=nextEditSequence(state.active.id);
  persistLocalDraft(state.active,content);
  return saveFileSnapshot(state.active.id,content,force,sequence);
}
...
async function saveAllBeforeExport(){
  ...
  // Não usa uma segunda via concorrente de save_all. Todos os arquivos passam
  // pela mesma fila serial por arquivo, evitando corrida com o autosave.
  const tasks=[];
  for(const file of state.files.filter(f=>f?.id)){
    const content=String(file.id===state.active?.id?$('code-editor').value:(file.content??''));
    let sequence=currentEditSequence(file.id);
    const controller=saveController(file.id);
    const represented=controller.pending?.content===content||controller.inFlight?.content===content||state.lastSavedContent.get(file.id)===content;
    if(!sequence||!represented)sequence=nextEditSequence(file.id);
    persistLocalDraft(file,content);
    tasks.push(saveFileSnapshot(file.id,content,true,sequence));
  }

  const results=await Promise.allSettled(tasks);
  await waitForPendingSaves();
  const failed=results.find(item=>item.status==='rejected'||item.value?.synced===false);
  state.dirty=Boolean(state.active&&$('code-editor').value!==state.lastSavedContent.get(state.active.id));
  if(failed){
    setSaveState('Não foi possível salvar — rascunho preservado','error');
    return {synced:false,files:state.files,error:failed.reason||failed.value?.error};
  }
  if(state.dirty){
    setSaveState('Alterações protegidas — aguardando sincronização','pending');
    return {synced:false,files:state.files};
  }
  const lastSaved=state.active?.saved_at||new Date().toISOString();
  setSaveState(`Salvo · ${formatSaveTime(lastSaved)}`,'ok');
  return {synced:true,files:state.files};
}
...
export async function mountWorkspace({profile,exercise,subject}){
  ...
  state.active=null; state.files=[]; state.saveQueues=new Map(); state.saveControllers=new Map(); state.editSequences=new Map(); state.confirmedSequences=new Map(); state.lastSavedContent=new Map(); state.remoteEdit=false; state.teacherEditing=false; state.supportRelease=null; state.recoveredFiles=new Set(); state.referenceSelections=new Map(); state.referenceMatchCache=new Map();
  setToolsOpen(false);
  setOutputOpen(false);
...
export async function unmountWorkspace(){
  ...
  state.saveControllers=new Map();
  state.editSequences=new Map();
  state.confirmedSequences=new Map();
  state.active=null;
}
