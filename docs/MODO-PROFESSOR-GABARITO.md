...
## v8 — revisão docente e importação privada

O Console Professor agora possui fluxo de revisão central:

- `completed + pending` = aluno entregou, professor ainda não aprovou;
- **Aprovar** = `reviewed + approved`;
- **Solicitar ajustes** = volta a `in_progress + changes_requested`;
- feedback e decisão ficam no progresso e a ação é registrada em `admin_audit_log`.

Admin/super_admin também podem selecionar o JSON privado com os 88 gabaritos. A importação ocorre em lotes pequenos via `agv-teacher-activity v3`. O JSON privado não deve ser copiado para a pasta publicada nem para o ZIP aluno.

As três atividades de Análise e Método do 1DS usam correção server-side (`lab-analysis-validate`) e trigger no banco para impedir conclusão sem validação oficial. O endpoint devolve apenas feedback, nunca a chave de respostas.
