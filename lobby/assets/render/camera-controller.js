...
export function createCameraController({THREE,camera,canvas,getCollisionRoots=()=>[],initialYaw=Math.PI,initialPitch=.32,initialDistance=6.8,initialFov=null,initialSensitivity=1}={}){
  if(!THREE||!camera||!canvas)throw new Error('camera_controller_missing_dependency');
  let yaw=initialYaw,pitch=initialPitch,distance=initialDistance,mode='explore',intro=0,disposed=false,userFov=Number.isFinite(Number(initialFov))?clamp(Number(initialFov),45,100):null,sensitivity=clamp(Number(initialSensitivity)||1,.45,2);
  ...
  function setFov(value){const n=Number(value);userFov=Number.isFinite(n)?clamp(n,45,100):null;return getFov();}function getFov(){return userFov||current().fov;}function setSensitivity(value){const n=Number(value);if(Number.isFinite(n))sensitivity=clamp(n,.45,2);return sensitivity;}
  ...
  return{update,toggleMode,setMode,getMode,getYaw,setYaw,setPitch,getPitch,getDistance,setFov,getFov,setSensitivity,getSensitivity:()=>sensitivity,isFirstPerson:()=>mode==='firstperson',isColliding:()=>lastCollision,dispose(){if(disposed)return;disposed=true;canvas.removeEventListener('pointerdown',onPointerDown);canvas.removeEventListener('pointermove',onPointerMove);canvas.removeEventListener('pointerup',onPointerEnd);canvas.removeEventListener('pointercancel',onPointerEnd);canvas.removeEventListener('wheel',onWheel);canvas.removeEventListener('contextmenu',onContext);}};
}
