...
export async function createMuseuHardwareLite({canvas,state,onInteract,onPlayerState,onAreaChange,onQualityChange,onPerf,onWorldTime,onFirstFrame,signal}={}){
  ...
  const player={x:start.x,z:start.z,dir:Math.PI},locomotion=createPlayerLocomotion({worldId:'museu-hardware-agv'});
  ...
  function frame(now){
    ...
    let ix=((keys.has('KeyD')||keys.has('ArrowRight'))?1:0)-((keys.has('KeyA')||keys.has('ArrowLeft'))?1:0)+joy.x,iz=((keys.has('KeyS')||keys.has('ArrowDown'))?1:0)-((keys.has('KeyW')||keys.has('ArrowUp'))?1:0)+joy.y,l=Math.hypot(ix,iz);if(l>1){ix/=l;iz/=l;}const motion=locomotion.stepHorizontal(dt,{x:ix,z:iz,running});if(l>.02||motion.speed>.01){move(motion.x,motion.z);if(l>.02)player.dir=Math.atan2(ix,iz);}
  ...
  const api={
    ...
    setLocalAction:()=>{},setLocalEmote(kind){chatBubbles.set(state?.user?.id||'local',{message:String(kind||'🎮').slice(0,8),until:performance.now()+3500});return true;},teleportTo(target){if(!target)return false;locomotion.haltHorizontal();player.x=clampMuseuHardware(Number(target.x)||0,MUSEU_HARDWARE_BOUNDS.minX+3,MUSEU_HARDWARE_BOUNDS.maxX-3);player.z=clampMuseuHardware(Number(target.z)||0,MUSEU_HARDWARE_BOUNDS.minZ+3,MUSEU_HARDWARE_BOUNDS.maxZ-3);return true;},
