import { WORLD_X, WORLD_Z, CAMPUS_ZONE_LAYOUT, CAMPUS_DECOR, presenceToWorld, worldToPresence, areaAtWorld } from './world/campus-manifest.js?v=14.10.8.52';
import { CAMPUS_EXPERIENCES, PARKOUR_PLATFORMS, LITE_PARKOUR_CHECKPOINTS, PARKOUR_START, nearestExperience } from './world/campus-experiences.js?v=14.10.8.52';
import { createCheckpointChallenge } from './game/challenge-manager.js?v=14.10.8.52';
import { createRideManager } from './game/ride-manager.js?v=14.10.8.52';
...
export function createLobbyLite({canvas,zones,state,isStaff,onInteract,onPlayerState,onQualityChange,onAreaChange,onChallengeEvent}){
  ...
  let stopped=false,raf=0,last=performance.now(),running=false,jump=0,jumpV=0,lastArea='central',emote=null,emoteUntil=0,zoom=1,zoomTarget=1,panX=0,panZ=0,panTargetX=0,panTargetZ=0,sizeDirty=true,viewport={w:1,h:1,dpr:1},portraitMode=null;
  ...
  function drawBackground(w,h,time){
    ...
    ctx.globalAlpha=.08;ctx.fillStyle='#9ce8ff';for(let i=0;i<(saveData?12:45);i++){const x=(i*173.31+time*2.2)%w,y=(i*97.17+time*.8)%h;ctx.beginPath();ctx.arc(x,y,(i%3)+.5,0,Math.PI*2);ctx.fill();}ctx.globalAlpha=1;
  }
  ...
  function drawExperience(exp,w,h,time){
    ...
    if(exp.type==='pool'){
      ...
      rr(-3.25*s,1.45*s,1.5*s,.55*s,.14*s,'rgba(16,24,32,.9)',hexToRgba(accent,.45));text('ESTAÇÃO',-2.5*s,1.72*s,{size:Math.max(6,.34*s),weight:900,color:accent,align:'center'});
      ...
      text('3 m',2.05*s,-1.42*s,{size:Math.max(7,.4*s),weight:900,color:accent,align:'center'});
    }
  ...
  function drawAvatar(wx,wz,label,color,self=false,role='student',dir=0,time=0,showLabel=true,w=viewport.w,h=viewport.h){
