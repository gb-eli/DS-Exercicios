...
export function createLobbyLite({canvas,zones,state,isStaff,onInteract,onPlayerState,onQualityChange,onAreaChange,onChallengeEvent,onWorldTime,onInteriorChange}){
  ...
  function frame(now){
    if(stopped)return;raf=requestAnimationFrame(frame);if(document.hidden){last=now;lastRenderGate=0;return;}const minFrame=1000/Math.max(15,fpsCap||60);if(lastRenderGate&&now-lastRenderGate<minFrame-1)return;lastRenderGate=now;frameCounter.tick(now);
  ...
  return{
    ready:Promise.resolve(),setQuality:()=>onQualityChange?.('low'),getQuality:()=> 'low',getFPS:()=>frameCounter.getFPS(),getObservabilitySnapshot:()=>captureRenderTelemetry({fps:frameCounter.getFPS(),quality:'lite',npcCount:CAMPUS_NPC_PATROLS.length,vehicleCount:CAMPUS_TRAFFIC_FLEET.length,worldId:'campus-ds',interior:activeToolInterior?`tool:${activeToolInterior}`:null,source:'campus-lite'}),
