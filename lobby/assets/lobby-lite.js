...
import { CAMPUS_EXPERIENCES, CAMPUS_TRAIN_STATIONS, CAMPUS_TRAIN_ROUTE, PARKOUR_PLATFORMS, LITE_PARKOUR_CHECKPOINTS, PARKOUR_START, nearestExperience } from './world/campus-experiences.js?v=14.10.8.65';
...
import { CAMPUS_INTERIOR_MAP, CAMPUS_INTERIOR_INTERACTIONS } from './world/campus-interiors.js?v=14.10.8.65';
...
import { CAMPUS_TRAFFIC_FLEET, CAMPUS_DRIVABLE_VEHICLES, CAMPUS_NPC_PATROLS, CAMPUS_DYNAMIC_SIGNS, CAMPUS_INTERIOR_SIGNATURES, resolveCampusCityEvent, resolveDynamicSign, sampleCampusRoute } from './world/campus-mobility-systems.js?v=14.10.8.65';
...
import { resolveWorldTime, skyPalette } from './world/dynamic-world.js?v=14.10.8.65';
...
export function createLobbyLite({canvas,zones,state,isStaff,onInteract,onPlayerState,onQualityChange,onAreaChange,onChallengeEvent,onWorldTime,onInteriorChange}){
  ...
  function drawLandscape(w,h,time){
    ...
    for(const plaza of CAMPUS_THEME_PLAZAS){const q=project(plaza.x,plaza.z,w,h),ms=metric(w,h);ctx.fillStyle=hexToRgba(plaza.accent,.08);ctx.beginPath();ctx.arc(q.x,q.y,plaza.radius*ms,0,Math.PI*2);ctx.fill();ctx.strokeStyle=hexToRgba(plaza.accent,.38);ctx.lineWidth=Math.max(1,.09*ms);ctx.stroke();text(plaza.icon,q.x,q.y,{size:Math.max(8,.52*ms),weight:900,color:plaza.accent,align:'center'});}
    ...
    // Ruas, calçadas, muros e pista de pouso — base legada preservada para regressão visual.
  ...
  function drawToolInterior(w,h,time){
    ...
    const ms=metric(w,h),c=project(profile.origin[0],profile.origin[2],w,h),rw=profile.width*ms,rd=profile.depth*ms;
    ctx.save();
    rr(c.x-rw/2,c.y-rd/2,rw,rd,Math.max(8,.55*ms),'rgba(8,20,27,.98)',hexToRgba(profile.accent,.72),Math.max(1.3,.08*ms));
    ctx.strokeStyle='rgba(195,232,242,.16)';ctx.lineWidth=Math.max(1,.05*ms);
    for(let i=-3;i<=3;i++){ctx.beginPath();ctx.moveTo(c.x+i*2*ms,c.y-rd/2);ctx.lineTo(c.x+i*2*ms,c.y+rd/2);ctx.stroke();}
    for(let i=-2;i<=2;i++){ctx.beginPath();ctx.moveTo(c.x-rw/2,c.y+i*2*ms);ctx.lineTo(c.x+rw/2,c.y+i*2*ms);ctx.stroke();}
    ...
    for(const room of floorMap?.rooms||[]){const accent=room.kind==='cyber'?'#ff6b7a':room.kind==='science'?'#8f8cff':room.kind==='gamer'?'#ff7fd5':room.kind==='finance'?'#61e7a6':profile.accent;card(room.x,room.z,String(room.label||'SALA').toUpperCase(),accent,Math.min(4.6,Number(room.w||3.4)),Math.min(2.0,Number(room.d||1.4)));}
  ...
  const draw=now=>{
    ...
    const selfColor=state.avatarStyle?.accentCss||(isStaff()?'#ffd166':'#36d2ff');if(activeCampusVehicle){const q=project(player.x,player.z,w,h),ms=metric(w,h);ctx.save();ctx.translate(q.x,q.y);ctx.rotate(-player.dir);rr(-.7*ms,-.36*ms,1.4*ms,.72*ms,.2*ms,hexToRgba(activeCampusVehicle.accent||'#ffae63',.96),'rgba(245,253,255,.7)');text(activeCampusVehicle.kind==='bike'?'◉':activeCampusVehicle.kind==='bus'?'▰':'◆',0,0,{size:Math.max(6,.34*ms),weight:950,color:'#f6feff',align:'center'});ctx.restore();}else drawAvatar(player.x,player.z,(state.profile?.full_name||'Usuário').split(' ')[0],selfColor,true,state.profile?.role||'student',player.dir,time,true,w,h,state.avatarStyle||null);const ownBubble=chatBubbles.get(state.user?.id);if(ownBubble&&ownBubble.until>Date.now())drawChatBubble(player.x,player.z,ownBubble.message,w,h);drawFooter(w,h);
  };
  ...
  return{
    ...
    enterToolInterior,exitToolInterior,useToolElevator:ref=>changeToolFloor(ref,'elevator'),useToolStairs:ref=>changeToolFloor(ref,'stairs'),exitToolInteriorToGarage:()=>exitToolInterior({garage:true}),getToolInterior:()=>activeToolInterior?{id:activeToolInterior,floor:activeToolFloor}:null,
    ...
    setWorldTimeMode:m=>{worldTimeMode=['auto','day','night'].includes(m)?m:'auto';lastWorldClock='';return worldTimeMode;},getWorldTimeMode:()=>worldTimeMode,setFPSCap:v=>{fpsCap=clamp(Number(v)||60,15,60);return fpsCap;},getFPSCap:()=>fpsCap,setFov:()=>null,getFov:()=>null,setCameraMode:()=> 'lite',
    showChatMessage(senderId,message){chatBubbles.set(senderId,{message:String(message||'').slice(0,90),until:Date.now()+6500});return true;},startChallenge:(...args)=>activeToolInterior?false:startChallenge(...args),restartChallenge:()=>activeToolInterior?false:restartChallenge(),cancelChallenge,startExperience:(...args)=>activeToolInterior?false:startExperience(...args),cancelExperience:()=>rides.cancel(),jump:()=>{if(jump<=.01&&!rides.isActive()&&!train.isTraveling())jumpV=5.6;},setRun:v=>{running=!!v;},setLocalEmote(kind){emote=kind;emoteUntil=Date.now()+4500;},
