import { CAMPUS_RIDES } from '../world/campus-experiences.js?v=14.10.8.52';
...
export function createRideManager({definitions=CAMPUS_RIDES,onEvent,clock=defaultClock,reducedMotion=false}={}){
  ...
  function start(id){const def=definitions[id];if(!def)return false;current=def;startedAt=clock();lastTick=0;emit('experience-start',{title:id==='coaster'?'Trilho Panorâmico':id==='slide'?'Escorregador Turbo':id==='tower'?'Torre Mirante':id==='playground'?'Parquinho DS':'Piscina Neon',message:id==='coaster'?'Passeio panorâmico iniciado. A câmera volta à exploração ao final.':id==='slide'?'Subindo e preparando uma descida curta e segura.':id==='tower'?'Subida ao mirante iniciada.':id==='playground'?'Balanço ativado por alguns segundos.':'Pausa no deck da Piscina Neon.',camera:def.camera});return true;}
