import { CAMPUS_TRAIN_STATIONS, CAMPUS_TRAIN_ROUTE } from '../world/campus-experiences.js?v=14.10.8.58';
...
function closestStationIndex(x,z){let best=0,bestD=Infinity;CAMPUS_TRAIN_STATIONS.forEach((s,i)=>{const d=Math.hypot(x-s.x,z-s.z);if(d<bestD){bestD=d;best=i;}});return best;}
...
export function createTrainManager({clock=()=>performance.now(),onEvent,reducedMotion=false}={}){
  ...
  const stationTimeline=CAMPUS_TRAIN_STATIONS.map(station=>({...station,routeDistance:distanceAtStation(station)})).sort((a,b)=>a.routeDistance-b.routeDistance);
  ...
  function startTrip(destinationId,from={x:0,z:0}){
    const dest=CAMPUS_TRAIN_STATIONS.find(s=>s.id===destinationId);if(!dest)return false;
    const origin=CAMPUS_TRAIN_STATIONS[closestStationIndex(from.x,from.z)],startD=distanceAtStation(origin),endRaw=distanceAtStation(dest);let delta=endRaw-startD;if(delta<=1)delta+=METRICS.total;
  ...
  return{stations:()=>CAMPUS_TRAIN_STATIONS.map(s=>({...s})),sampleVisual,startTrip,tickTrip,cancel(){if(!trip)return false;trip=null;onEvent?.({type:'train-cancel',message:'Viagem de monotrilho cancelada.'});return true;},isTraveling:()=>!!trip};
}
