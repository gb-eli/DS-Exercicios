import { WORLD_X, WORLD_Z, CAMPUS_ZONE_LAYOUT, EXTERIOR_BUILDING_COLLIDERS, presenceToWorld, worldToPresence, areaAtWorld } from './world/campus-manifest.js?v=14.10.8.58';
import { createCampusEnvironment, createCampusLighting } from './world/campus-environment.js?v=14.10.8.58';
import { createCameraController } from './render/camera-controller.js?v=14.10.8.58';
import { createAvatarSystem } from './characters/avatar-system.js?v=14.10.8.58';
import { createPortalSystem } from './game/portal-manager.js?v=14.10.8.58';
import { detectPerformanceProfile, chooseInitialQuality, createResizeController, createAdaptiveQualityController } from './render/performance-manager.js?v=14.10.8.58';
import { CAMPUS_EXPERIENCES, CAMPUS_TRAIN_STATIONS, LITE_PARKOUR_CHECKPOINTS, PARKOUR_START, parkourPlatformAt, campusSurfaceAt } from './world/campus-experiences.js?v=14.10.8.58';
...
import { resolveWorldTime } from './world/dynamic-world.js?v=14.10.8.58';

const THREE_URL='../vendor/three/three.module.min.js?v=14.10.8.58';
...
export async function createLobby3D({canvas,zones,state,isStaff,className,onInteract,onPlayerState,onQualityChange,onPerf,onError,onAreaChange,onFirstFrame,onInteriorChange,onContextLost,onChallengeEvent,onWorldTime,signal,initialQuality=null}){
  ...
  function frame(nowMs){
    ...
    let ix=(right?1:0)-(left?1:0)+moveJoy.x,iz=(down?1:0)-(up?1:0)+moveJoy.y;if(seated||presentation||rides.isActive()||train.isTraveling()){ix=0;iz=0;}const len=Math.hypot(ix,iz);if(len>1){ix/=len;iz/=len;}
    ...
    const trainRide=train.tickTrip(nowMs),ride=trainRide||rides.tick(nowMs);let moving=Math.hypot(mx,mz),groundY=0,support=null;
    if(ride){self.position.set(ride.x,ride.y,ride.z);playerY=ride.y||0;vertical=0;onGround=true;self.rotation.y=ride.heading||self.rotation.y;cameraController.setYaw((ride.heading||0)+Math.PI);moving=.8;}
      ...
      support=!activeInterior?campusSurfaceAt(self.position.x,self.position.z,-.04):null;groundY=support?.h||0;if(onGround&&groundY>playerY&&groundY-playerY<=.72){playerY=groundY;vertical=0;}vertical-=20*dt;playerY+=vertical*dt;if(playerY<=groundY){playerY=groundY;vertical=0;onGround=true;}else onGround=false;
    ...
    cameraController.update({playerPosition:self.position,moving,running,time:motionTime,dt});self.visible=!cameraController.isFirstPerson();updateAdaptive(nowMs);
