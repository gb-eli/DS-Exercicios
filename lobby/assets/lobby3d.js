...
import { createCampusEnvironment, createCampusLighting } from './world/campus-environment.js?v=14.10.8.76-stage45-space';
...
export async function createLobby3D({canvas,zones,state,isStaff,className,onInteract,onPlayerState,onQualityChange,onPerf,onError,onAreaChange,onFirstFrame,onInteriorChange,onContextLost,onChallengeEvent,onWorldTime,onVehicleTelemetry,signal,initialQuality=null}){
  ...
  const vehicleDefById=id=>CAMPUS_DRIVABLE_VEHICLES.find(item=>item.id===id)||CAMPUS_AERIAL_VEHICLES.find(item=>item.id===id)||null;
  ...
  const keydown=e=>{if(['INPUT','TEXTAREA','SELECT'].includes(document.activeElement?.tagName))return;if(['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','ShiftLeft','ShiftRight','Space','KeyE','Enter','Digit1','Digit2','Digit3','KeyC'].includes(e.code))e.preventDefault();keys.add(e.code);if(['KeyW','KeyA','KeyS','KeyD','ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'].includes(e.code)&&localAction&&!contextLocked()){localAction=null;seated=null;self.userData.localAction=null;}if((e.code==='KeyE'||e.code==='Enter')&&!e.repeat)onInteract?.();if(e.code==='Space'&&!e.repeat&&!activeCampusVehicle&&!securityViewState.active&&!viewpointState.active&&onGround&&!contextLocked()){vertical=7.8;onGround=false;}if(e.code==='Digit1')state.emoteRequested?.('wave');if(e.code==='Digit2')state.emoteRequested?.('like');if(e.code==='Digit3')state.emoteRequested?.('spark');if(e.code==='KeyC'&&!e.repeat&&!securityViewState.active&&!viewpointState.active)cameraController.toggleMode();};const keyup=e=>keys.delete(e.code);window.addEventListener('keydown',keydown);window.addEventListener('keyup',keyup);
  ...
  function useCampusVehicle(ref,options={}){
    ...
    rides.cancel({silent:true});train.cancel();cancelChallenge();
