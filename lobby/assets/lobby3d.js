import { WORLD_X, WORLD_Z, CAMPUS_ZONE_LAYOUT, EXTERIOR_BUILDING_COLLIDERS, presenceToWorld, worldToPresence, areaAtWorld } from './world/campus-manifest.js?v=14.10.8.51';
import { createCampusEnvironment, createCampusLighting } from './world/campus-environment.js?v=14.10.8.51';
import { createCameraController } from './render/camera-controller.js?v=14.10.8.51';
import { createAvatarSystem } from './characters/avatar-system.js?v=14.10.8.51';
import { createPortalSystem } from './game/portal-manager.js?v=14.10.8.51';
import { detectPerformanceProfile, chooseInitialQuality, createResizeController, createAdaptiveQualityController } from './render/performance-manager.js?v=14.10.8.51';
import { LITE_PARKOUR_CHECKPOINTS, parkourPlatformAt } from './world/campus-experiences.js?v=14.10.8.51';

const THREE_URL='../vendor/three/three.module.min.js?v=14.10.8.51';
...
export async function createLobby3D({canvas,zones,state,isStaff,className,onInteract,onPlayerState,onQualityChange,onPerf,onError,onAreaChange,onFirstFrame,onInteriorChange,onContextLost,onChallengeEvent,signal,initialQuality=null}){
  ...
  const profile=detectPerformanceProfile(),mobile=profile.mobile,hardware=profile.hardware,cores=profile.cores;let quality=chooseInitialQuality(profile,initialQuality);
  ...
  const cameraCollisionRoots=[...environment.cameraCollisionRoots,...(environment.experienceRoots||[])];
  ...
  const liveSign=spriteLabel('PRAÇA CENTRAL • CAMPUS DS','#72e6ff',5.4,{bg:'rgba(2,13,20,.82)'});liveSign.position.set(0,7.15,-.4);scene.add(liveSign);let lastSignMinute=-1;
  ...
  let challengeActive=false,challengeIndex=0,challengeStartedAt=0,challengeCooldown=0;
  function syncChallengeMarkers(time=0){
    challengeMarkers.forEach((ring,i)=>{ring.visible=challengeActive;const active=i===challengeIndex,done=i<challengeIndex;ring.material.color.set(done?0x51e7a3:active?0xffffff:0xff6b7a);ring.material.opacity=done?.35:active?.45+.2*(.5+.5*Math.sin(time*5)):.14;ring.scale.setScalar(active?1.0+.08*Math.sin(time*5):1);});
  }
  function startChallenge(id='parkour'){
    if(id!=='parkour')return false;challengeActive=true;challengeIndex=0;challengeStartedAt=performance.now();challengeCooldown=0;syncChallengeMarkers();
    onChallengeEvent?.({type:'start',id:'parkour',title:'Circuito Parkour 3D',message:'Passe pelos 5 checkpoints na ordem. Corra, pule e explore o circuito enquanto aguarda a atividade.'});return true;
  }
  function cancelChallenge(){if(!challengeActive)return false;challengeActive=false;challengeIndex=0;syncChallengeMarkers();onChallengeEvent?.({type:'cancel',id:'parkour',message:'Circuito encerrado.'});return true;}
  function updateChallenge(nowMs,time){
    syncChallengeMarkers(time);if(!challengeActive||challengeCooldown>nowMs)return;const cp=LITE_PARKOUR_CHECKPOINTS[challengeIndex];if(!cp)return;
    if(Math.hypot(self.position.x-cp[0],self.position.z-cp[1])<1.35){
      challengeIndex++;challengeCooldown=nowMs+420;
      if(challengeIndex>=LITE_PARKOUR_CHECKPOINTS.length){const elapsed=Math.max(1,(nowMs-challengeStartedAt)/1000);challengeActive=false;challengeIndex=0;syncChallengeMarkers(time);onChallengeEvent?.({type:'complete',id:'parkour',elapsed,message:`Circuito 3D concluído em ${elapsed.toFixed(1).replace('.',',')} s. Treino local: não altera nota ou atividade.`});}
      else onChallengeEvent?.({type:'checkpoint',id:'parkour',index:challengeIndex,total:LITE_PARKOUR_CHECKPOINTS.length,message:`Checkpoint ${challengeIndex}/${LITE_PARKOUR_CHECKPOINTS.length}`});
    }
  }
  ...
  function frame(nowMs){if(stopped||signal?.aborted)return;raf=requestAnimationFrame(frame);if(document.hidden)return;resizeController.update();const dt=Math.min(.045,(nowMs-last)/1000||.016);last=nowMs;const time=nowMs/1000;const left=keys.has('KeyA')||keys.has('ArrowLeft'),right=keys.has('KeyD')||keys.has('ArrowRight'),up=keys.has('KeyW')||keys.has('ArrowUp'),down=keys.has('KeyS')||keys.has('ArrowDown');let ix=(right?1:0)-(left?1:0)+moveJoy.x,iz=(down?1:0)-(up?1:0)+moveJoy.y;if(seated||presentation){ix=0;iz=0;}const len=Math.hypot(ix,iz);if(len>1){ix/=len;iz/=len;}const running=keys.has('ShiftLeft')||keys.has('ShiftRight')||runHeld,speed=running?6.4:3.65,cameraYaw=cameraController.getYaw(),sy=Math.sin(cameraYaw),cy=Math.cos(cameraYaw),mx=ix*cy+iz*sy,mz=-ix*sy+iz*cy,nx=self.position.x+mx*speed*dt,nz=self.position.z+mz*speed*dt;if(canStand(nx,self.position.z))self.position.x=nx;if(canStand(self.position.x,nz))self.position.z=nz;const moving=Math.hypot(mx,mz);if(moving>.03){const desired=Math.atan2(mx,mz);let delta=(desired-self.rotation.y+Math.PI*3)%(Math.PI*2)-Math.PI;self.rotation.y+=delta*Math.min(1,dt*11);}vertical-=20*dt;playerY+=vertical*dt;const support=!activeInterior?parkourPlatformAt(self.position.x,self.position.z,-.05):null,groundY=support?.h||0;if(playerY<=groundY){playerY=groundY;vertical=0;onGround=true;}else onGround=false;self.position.y=playerY;avatarSystem.animate(self,{speed:moving*speed,jump:Math.max(0,playerY-groundY),time,vertical,dt});syncOtherAvatars(time,dt);updatePortalVisuals(time);updateCampusClock(nowMs);updateChallenge(nowMs,time);for(const root of environment.experienceRoots||[]){if(root.userData?.water)root.userData.water.material.opacity=.23+.08*(.5+.5*Math.sin(time*2.2));if(root.userData?.coasterCar){const a=time*.62;root.userData.coasterCar.position.set(Math.cos(a)*2.45,1.18+.16*Math.sin(a*2),Math.sin(a)*2.45);root.userData.coasterCar.rotation.y=-a+Math.PI/2;}if(root.userData?.swingSeats)root.userData.swingSeats.forEach((seat,i)=>{seat.position.z=Math.sin(time*1.5+i*Math.PI)*.22;});}updatePatrols(time,dt);updateAutomaticDoors(dt);if(activeInterior){const r=interiors.get(activeInterior);if(r){for(const t of r.terminals){if(t.screen&&t.mesh?.userData?.powered){t.screen.material.emissiveIntensity=1.05+.35*(.5+.5*Math.sin(time*3+t.id.length));}}if(r.presentationPad)r.presentationPad.material.opacity=.32+.22*(.5+.5*Math.sin(time*2.4));}}if(canopy?.userData?.inner){canopy.userData.inner.material.emissiveIntensity=1.05+.28*(.5+.5*Math.sin(time*.9));canopy.userData.inner.rotation.z=time*.012;}npcs.forEach((n,i)=>{const bx=n.def.x,bz=n.def.z;n.avatar.position.x=bx+Math.sin(time*.22+i)*.55;n.avatar.position.z=bz+Math.cos(time*.18+i)*.35;n.worldRef.x=n.avatar.position.x;n.worldRef.z=n.avatar.position.z;avatarSystem.animate(n.avatar,{speed:.22,jump:0,time:time+i*.7,vertical:0,dt});n.avatar.rotation.y=Math.atan2(Math.cos(time*.22+i),-Math.sin(time*.18+i));avatarSystem.applyLOD(n.avatar,self.position.distanceTo(n.avatar.position),{staff:true});});meetPads.forEach((pad,i)=>{pad.userData.ring.rotation.z=time*(i%2?.22:-.18);pad.userData.core.material.opacity=.12+.07*(.5+.5*Math.sin(time*2+i));});
    centralFountain.userData.pool.material.opacity=.62+.08*Math.sin(time*2.4);centralFountain.userData.crown.rotation.y=time*.65;centralFountain.userData.crown.position.y=3.28+Math.sin(time*1.8)*.08;for(let i=0;i<beacon.userData.rings.length;i++){const r=beacon.userData.rings[i];r.rotation.y=time*(i%2?.23:-.18)+i*.4;r.rotation.z+=dt*(i%2?.18:-.12);}if(ambientCount)motes.rotation.y=time*.0015;
    cameraController.update({playerPosition:self.position,moving,running,time,dt});updateAdaptive(nowMs);
