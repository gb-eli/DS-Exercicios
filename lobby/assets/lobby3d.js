...
export async function createLobby3D({canvas,zones,state,isStaff,className,onInteract,onPlayerState,onQualityChange,onPerf,onError,onAreaChange,onFirstFrame,onInteriorChange,onContextLost,onChallengeEvent,onWorldTime,signal,initialQuality=null}){
  ...
  const scene=new THREE.Scene();scene.background=new THREE.Color(0x07111a);scene.fog=new THREE.FogExp2(0x0a1821,.014);const sky=skyDome(),moonVisual=moonDisc(),sunVisual=sunDisc();scene.add(sky,moonVisual,sunVisual);
  ...
  const toolInteriors=new Map();
  for(const profile of CAMPUS_INTERIOR_PROFILES){const room=toolInterior(profile);if(profile.receptionist){const av=avatarSystem.createAvatar({accent:profile.accent,staff:true,label:profile.receptionist.name,seed:`reception-${profile.id}`});av.position.set(profile.receptionist.x-profile.origin[0],0,profile.receptionist.z-profile.origin[2]);av.scale.setScalar(.88);room.floors.get(0)?.add(av);room.receptionistAvatar=av;}room.group.visible=false;scene.add(room.group);cameraCollisionRoots.push(room.group);toolInteriors.set(profile.id,room);worldObjects.push(...room.refs);}
  const interiorGuideLine=new THREE.Line(new THREE.BufferGeometry(),new THREE.LineBasicMaterial({color:0x72e6ff,transparent:true,opacity:.92}));interiorGuideLine.visible=false;scene.add(interiorGuideLine);let activeToolGuide=null;
  ...
  const trafficVehicles=CAMPUS_TRAFFIC_FLEET.map(def=>{const mesh=campusMobilityVehicle(def);scene.add(mesh);return{def,mesh};});
  ...
  const cityNpcPatrols=[];for(const def of CAMPUS_NPC_PATROLS.slice(0,quality==='low'?3:CAMPUS_NPC_PATROLS.length)){const avatar=avatarSystem.createAvatar({accent:def.accent,staff:true,label:def.name,seed:def.id});avatar.scale.setScalar(.88);scene.add(avatar);const worldRef={...def,type:'city-npc',x:0,z:0,radius:2.5};cityNpcPatrols.push({def,avatar,worldRef});worldObjects.push(worldRef);}
  const dynamicCitySigns=CAMPUS_DYNAMIC_SIGNS.map(def=>{const sprite=spriteLabel(def.messages?.[0]||def.id,def.accent,3.6,{bg:'rgba(3,13,18,.9)'});sprite.position.set(def.x,2.8,def.z);scene.add(sprite);const ref={...def,type:'dynamic-sign',name:'Painel urbano AGV',radius:2.2};worldObjects.push(ref);return{def,sprite,lastText:''};});
  ...
  const canStand=(x,z)=>{
    ...
    if(insideBuilding&&!(['roof','bridge'].includes(surface?.type)&&playerY>=surface.h-.85))return false;
  ...
  const cameraController=createCameraController({THREE,camera,canvas,getCollisionRoots:()=>cameraCollisionRoots,initialYaw:Math.PI,initialPitch:.32,initialDistance:6.8,initialFov:Number(state.graphics?.fov)||null});
  ...
  function startExperience(id){if(activeInterior||activeToolInterior||!CAMPUS_EXPERIENCES.some(item=>item.id===id&&item.id!=='parkour'))return false;cancelChallenge();seated=null;presentation=null;activeStation=null;localAction=id==='playground'?'cheer':null;self.userData.localAction=localAction;return rides.start(id);}
  ...
  function frame(nowMs){
    ...
    let vehicleRide=null;if(activeCampusVehicle){const elapsed=nowMs-vehicleRideStartedAt,k=elapsed/Math.max(1000,activeCampusVehicle.durationMs||18000);if(k<1)vehicleRide=sampleCampusRoute(activeCampusVehicle.routeId,(activeCampusVehicle.startT||0)+k);}const trainRide=train.tickTrip(nowMs),ride=vehicleRide||trainRide||rides.tick(nowMs);let moving=Math.hypot(mx,mz),groundY=0,support=null;
    ...
    if(transit?.train){const trainPos=train.sampleVisual(nowMs);transit.train.position.set(trainPos.x,trainPos.y,trainPos.z);transit.train.rotation.y=trainPos.heading||0;}
    ...
    cameraController.update({playerPosition:self.position,moving,running,time:motionTime,dt});self.visible=!activeCampusVehicle&&!cameraController.isFirstPerson();updateAdaptive(nowMs);
  ...
  function enterToolInterior(id){
    const room=toolInteriors.get(id),profile=CAMPUS_INTERIOR_MAP[id];if(!room||!profile)return false;cancelCampusVehicle();rides.cancel({silent:true});train.cancel();cancelChallenge();lastExteriorPosition={x:self.position.x,z:self.position.z};
    if(activeInterior){interiors.get(activeInterior)?.group&&(interiors.get(activeInterior).group.visible=false);activeInterior=null;}
    for(const [,r] of toolInteriors)r.group.visible=false;room.group.visible=true;activeToolInterior=id;activeToolFloor=0;activeToolGuide=null;interiorGuideLine.visible=false;for(const [n,g] of room.floors)g.visible=n===0;
    ...
    onInteriorChange?.({inside:true,key:`tool:${id}`,kind:'tool',floor:0,label:`${profile.name} • ${profile.floorLabel(0)}`});return true;
  }
  function exitToolInterior({garage=false}={}){
    if(!activeToolInterior)return false;const id=activeToolInterior,room=toolInteriors.get(id),profile=CAMPUS_INTERIOR_MAP[id];room?.group&&(room.group.visible=false);activeToolInterior=null;activeToolFloor=0;activeToolGuide=null;interiorGuideLine.visible=false;seated=null;presentation=null;activeStation=null;localAction=null;self.userData.localAction=null;
    const target=garage&&profile?.garage?{x:profile.garage.x,z:profile.garage.z}:{x:lastExteriorPosition?.x??profile?.entrance?.x??0,z:lastExteriorPosition?.z??profile?.entrance?.z??0};self.position.set(target.x,0,target.z);playerY=0;vertical=0;onGround=true;self.rotation.y=Math.PI;cameraController.setYaw(0);onInteriorChange?.({inside:false,key:null,kind:'tool',floor:null,label:garage&&profile?.garage?profile.garage.name:'Campus DS'});return true;
  }
