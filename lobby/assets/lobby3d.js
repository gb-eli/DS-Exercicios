...
import { createCampusEnvironment, createCampusLighting } from './world/campus-environment.js?v=14.10.8.96-f9411-graphics-streaming';
...
import { visualQualityProfile,rendererPixelRatio } from './render/visual-quality-profile.js?v=14.10.8.96-f9411-graphics-streaming';
...
import { CAMPUS_VIEWPOINTS, CAMPUS_VIEWPOINT_MAP, CAMPUS_VIEWPOINT_DEFAULT, CAMPUS_VIEWPOINT_LANDMARK_MAP, viewpointZoomFov, viewpointLandmarks } from './world/campus-viewpoints.js?v=14.10.8.96-f948-camera-v2';
...
import { createCampusPhysicsPilot } from './core/physics-v2/campus-physics-pilot.js?v=14.10.8.96-f9413-rapier-bvh-pilot';
import { createVehicleRegistry } from './core/vehicle-v2/vehicle-registry.js?v=14.10.8.96-f9412-vehicle-core';
import { vehicleNetworkPacket } from './core/vehicle-v2/vehicle-network-protocol.js?v=14.10.8.96-f9412-vehicle-core';
import { interiorLightBudget } from './world/world-environment-profiles.js?v=14.10.8.96-f9411-graphics-streaming';
...
function addInteriorLighting(root,{accent=0x72e6ff,quality='medium',width=16,depth=10,height=4.45}={}){
  const budget=interiorLightBudget(quality),fill=new THREE.HemisphereLight(0xe8f8ff,0x071016,quality==='low'?.48:.62);root.add(fill);
  const points=[],positions=[[-width*.28,-depth*.2],[width*.28,-depth*.2],[-width*.28,depth*.22],[width*.28,depth*.22]];
  for(let i=0;i<positions.length;i++){
    const [x,z]=positions[i],light=new THREE.PointLight(accent,budget.intensity,Math.max(width,depth)*.72,2);light.position.set(x,height,z);light.visible=i<budget.points;root.add(light);points.push(light);
  }
  root.userData.interiorLighting={fill,points,quality};return root.userData.interiorLighting;
}

function labInterior({accent,key,title,origin=[0,0,55],quality='medium'}){
  ...
  addInteriorLighting(g,{accent:secondary,quality,width:18,depth:12,height:4.35});
...
function toolInterior(profile,quality='medium'){
  ...
  for(const floorDef of profile.floors){
    ...
    addInteriorLighting(fg,{accent:secondary,quality,width:w*.9,depth:d*.82,height:3.35});
...
export async function createLobby3D({canvas,zones,state,isStaff,className,onInteract,onPlayerState,onQualityChange,onPerf,onError,onAreaChange,onFirstFrame,onInteriorChange,onContextLost,onChallengeEvent,onWorldTime,onVehicleTelemetry,signal,initialQuality=null}){
  ...
  environment=createCampusEnvironment({THREE,scene:exteriorRoot,zones,quality,spriteLabel,device:visualDevice});environment.setQuality?.(quality);
  ...
  const ensureClassInterior=key=>{let room=interiors.get(key);if(room)return room;const zone=zones.find(item=>item.key===key),origin=interiorOrigins[key];if(!zone||!origin)return null;const color=new THREE.Color(zone.accent).getHex(),built=labInterior({accent:color,key,title:zone.label,origin,quality});built.group.visible=true;interiorRoot.add(built.group);room={...built,origin,zone};interiors.set(key,room);mountScopedWorldObjects(room.refs,`class:${key}`);return room;};
  ...
  const viewpointCamera=new THREE.PerspectiveCamera(58,1,.08,1200),viewpointState={active:false,selectedId:CAMPUS_VIEWPOINT_DEFAULT,zoom:1,focusId:null,yaw:Math.PI,pitch:0};
  let viewpointPointer=null,viewpointLastX=0,viewpointLastY=0;
  const viewpointSnapshot=()=>{const def=CAMPUS_VIEWPOINT_MAP[viewpointState.selectedId]||CAMPUS_VIEWPOINT_MAP[CAMPUS_VIEWPOINT_DEFAULT];return{active:viewpointState.active,selectedId:def?.id||CAMPUS_VIEWPOINT_DEFAULT,name:def?.name||'Mirante AGV',district:def?.district||'Campus',description:def?.description||'',zoom:viewpointState.zoom,maxZoom:def?.maxZoom||50,focusId:viewpointState.focusId,yaw:viewpointState.yaw,pitch:viewpointState.pitch,landmarks:viewpointLandmarks(def).map(({id,name,icon,category,description})=>({id,name,icon,category,description}))};};
  function viewpointAngles(def,target){const dx=(target?.x||0)-def.camera.x,dy=(target?.y||0)-def.camera.y,dz=(target?.z||0)-def.camera.z,h=Math.max(.001,Math.hypot(dx,dz));return{yaw:Math.atan2(-dx,-dz),pitch:Math.atan2(dy,h)};}
  function aimViewpoint(def,target){const a=viewpointAngles(def,target||def.defaultTarget);viewpointState.yaw=a.yaw;viewpointState.pitch=Math.max(-1.48,Math.min(1.48,a.pitch));}
  function configureViewpointCamera(aspect=1){const def=CAMPUS_VIEWPOINT_MAP[viewpointState.selectedId]||CAMPUS_VIEWPOINT_MAP[CAMPUS_VIEWPOINT_DEFAULT];if(!def)return null;viewpointCamera.aspect=Math.max(.2,Number(aspect)||1);viewpointCamera.fov=viewpointZoomFov(def,viewpointState.zoom);viewpointCamera.position.set(def.camera.x,def.camera.y,def.camera.z);const cp=Math.cos(viewpointState.pitch),distance=300;viewpointCamera.lookAt(def.camera.x-Math.sin(viewpointState.yaw)*cp*distance,def.camera.y+Math.sin(viewpointState.pitch)*distance,def.camera.z-Math.cos(viewpointState.yaw)*cp*distance);viewpointCamera.updateProjectionMatrix();return viewpointCamera;}
  function openViewpoint(id,options={}){if(activeInterior||activeToolInterior||activeCampusVehicle||rides.isActive()||train.isTraveling())return false;const def=CAMPUS_VIEWPOINT_MAP[id]||CAMPUS_VIEWPOINT_MAP[CAMPUS_VIEWPOINT_DEFAULT];if(!def)return false;closeSecurityCameras();viewpointState.active=true;viewpointState.selectedId=def.id;viewpointState.zoom=Math.max(1,Math.min(def.maxZoom||50,Number(options.zoom)||1));viewpointState.focusId=CAMPUS_VIEWPOINT_LANDMARK_MAP[options.focusId]&&def.focusIds.includes(options.focusId)?options.focusId:null;aimViewpoint(def,viewpointState.focusId?CAMPUS_VIEWPOINT_LANDMARK_MAP[viewpointState.focusId]?.target:def.defaultTarget);cameraController?.setInputEnabled?.(false);return viewpointSnapshot();}
  function setViewpointZoom(zoom){const def=CAMPUS_VIEWPOINT_MAP[viewpointState.selectedId]||CAMPUS_VIEWPOINT_MAP[CAMPUS_VIEWPOINT_DEFAULT];viewpointState.zoom=Math.max(1,Math.min(def?.maxZoom||50,Number(zoom)||1));const snapshot=viewpointSnapshot();window.dispatchEvent?.(new CustomEvent('agv:viewpoint-state',{detail:snapshot}));return snapshot;}
  function setViewpointFocus(id){const def=CAMPUS_VIEWPOINT_MAP[viewpointState.selectedId];if(id&&(!CAMPUS_VIEWPOINT_LANDMARK_MAP[id]||!def?.focusIds?.includes(id)))return false;viewpointState.focusId=id||null;aimViewpoint(def,viewpointState.focusId?CAMPUS_VIEWPOINT_LANDMARK_MAP[viewpointState.focusId]?.target:def.defaultTarget);return viewpointSnapshot();}
  function closeViewpoint(){const was=viewpointState.active;viewpointState.active=false;viewpointPointer=null;cameraController?.setInputEnabled?.(true);return was;}
  function onViewpointPointerDown(e){if(!viewpointState.active||e.target!==canvas)return;viewpointPointer=e.pointerId;viewpointLastX=e.clientX;viewpointLastY=e.clientY;canvas.setPointerCapture?.(e.pointerId);}
  function onViewpointPointerMove(e){if(!viewpointState.active||e.pointerId!==viewpointPointer)return;const dx=e.clientX-viewpointLastX,dy=e.clientY-viewpointLastY;viewpointLastX=e.clientX;viewpointLastY=e.clientY;if(viewpointState.focusId){viewpointState.focusId=null;window.dispatchEvent?.(new CustomEvent('agv:viewpoint-state',{detail:viewpointSnapshot()}));}viewpointState.yaw-=dx*.0038*Math.max(.55,Math.min(1.45,Number(state.graphics?.mouseSensitivity)||1));const sign=state.graphics?.invertY?1:-1;viewpointState.pitch=Math.max(-1.48,Math.min(1.48,viewpointState.pitch+dy*.0032*sign*Math.max(.55,Math.min(1.45,Number(state.graphics?.mouseSensitivity)||1))));}
  function onViewpointPointerEnd(e){if(e.pointerId===viewpointPointer)viewpointPointer=null;}
  function onViewpointWheel(e){if(!viewpointState.active)return;const factor=e.deltaY<0?1.18:1/1.18;setViewpointZoom(viewpointState.zoom*factor);e.preventDefault();}
  canvas.addEventListener('pointerdown',onViewpointPointerDown);canvas.addEventListener('pointermove',onViewpointPointerMove);canvas.addEventListener('pointerup',onViewpointPointerEnd);canvas.addEventListener('pointercancel',onViewpointPointerEnd);canvas.addEventListener('wheel',onViewpointWheel,{passive:false});
  ...
  function attachNetworkPassenger({session,seatIndex=1}={}){if(!session?.driver_id)return false;rides.cancel({silent:true});train.cancel();vehiclePreviousCamera=cameraController.getMode();networkPassengerState={driverId:session.driver_id,seatIndex:Number(seatIndex)||1};const registered=vehicleRegistry.register({id:session.vehicle_id||`network-${session.driver_id}`,name:session.vehicle_name||'Veículo online',kind:session.vehicle_kind||'utility',mobilityType:'ground',seatCapacity:Math.max(2,Number(session.seat_capacity)||4)});activeCampusVehicle={...registered,seatMode:'network-passenger',speedPreset:'network',speedLabel:'Online',driverId:session.driver_id,seatIndex:networkPassengerState.seatIndex};self.userData.localAction='sit';playerVehicleShell.visible=false;const pos=occupantPlacement(session,networkPassengerState.seatIndex);vehicleCore.exit('network_passenger_switch');vehicleCore.configure(activeCampusVehicle,vehiclePhysics);vehicleCore.enter({role:'network-passenger',preset:'normal',pose:{x:pos?.x??self.position.x,y:0,z:pos?.z??self.position.z,heading:pos?.heading??0},seatIndex:networkPassengerState.seatIndex,driverId:session.driver_id});if(pos){self.position.set(pos.x,0,pos.z);self.rotation.y=pos.heading;cameraController.setYaw(pos.heading+Math.PI);cameraController.setMode('vehicle');}return true;}
  function detachNetworkPassenger(){if(activeCampusVehicle?.seatMode!=='network-passenger')return false;const session=(vehicleNetworkState.sessions||[]).find(row=>row.driver_id===activeCampusVehicle.driverId),pos=session?occupantPlacement(session,activeCampusVehicle.seatIndex):null;if(pos){const side=1.7;self.position.set(clamp(pos.x+Math.cos(pos.heading)*side,-WORLD_X+1,WORLD_X-1),0,clamp(pos.z-Math.sin(pos.heading)*side,-WORLD_Z+1,WORLD_Z-1));}vehicleCore.exit('network_passenger_exit');activeCampusVehicle=null;networkPassengerState=null;self.userData.localAction=null;vehicleSpeed=0;playerVehicleShell.visible=false;cameraController.setMode(vehiclePreviousCamera||'explore');return true;}
  ...
  const canStand=(x,z)=>{
    ...
    const surface=campusSurfaceAt(x,z,.03),bvhEligible=playerY<1.5&&!['roof','bridge','step','ramp'].includes(surface?.type),bvhBlocked=bvhEligible?campusPhysicsPilot?.intersectsStaticSphere?.({x,y:.72,z,radius:.62}):null,insideBuilding=bvhBlocked==null?colliders.some(c=>x>c.minX-.62&&x<c.maxX+.62&&z>c.minZ-.62&&z<c.maxZ+.62):bvhBlocked;
  ...
  const vehicleRegistry=createVehicleRegistry([...CAMPUS_DRIVABLE_VEHICLES,...CAMPUS_AERIAL_VEHICLES]);
  const fallbackVehiclePhysics=createKinematicVehiclePhysicsAdapter({
    canMoveGround:({to,definition})=>{const staticOk=canVehicleStand(to.x,to.z,definition.kind);if(!staticOk)return{ok:false,collision:{type:'static',label:'obstáculo fixo'}};const dynamicHit=dynamicVehicleHit(to.x,to.z,definition.kind,{includePlayer:false});return dynamicHit?{ok:false,collision:dynamicHit}:{ok:true,pose:to};},
    canMoveAerial:({to,definition})=>{const radius=aerialCollisionRadius(definition.kind);if(to.x<-WORLD_X+radius||to.x>WORLD_X-radius||to.z<-WORLD_Z+radius||to.z>WORLD_Z-radius)return{ok:false,collision:{type:'boundary',label:'limite do espaço aéreo'}};const clearance=aerialClearanceAt(to.x,to.z,definition.kind),pose=clearance>to.y?{...to,y:clearance}:to;return canAerialOccupy(pose.x,pose.z,pose.y,definition.kind)?{ok:true,pose}:{ok:false,collision:{type:'air-clearance',label:'obstáculo aéreo'}};}
  });
  campusPhysicsPilot=createCampusPhysicsPilot({THREE,colliders,bounds:{x:WORLD_X,z:WORLD_Z},fallbackPhysics:fallbackVehiclePhysics,quality,device:visualDevice,dynamicCollisionCheck:(x,z,kind)=>dynamicVehicleHit(x,z,kind,{includePlayer:false})});
  const vehiclePhysics=campusPhysicsPilot.adapter;
  const vehicleCore=createVehicleController({worldId:'campus-ds',physicsAdapter:vehiclePhysics});
  ...
  const accentForSelf=()=>state.avatarStyle?.accentCss||(state.currentClass?(zones.find(z=>z.code===state.currentClass.code)?.accent||'#36d2ff'):'#ffd166');const selfLabel=(state.profile?.role==='teacher'?'Prof. ':state.profile?.role==='student'?'':'ADM ')+(state.profile?.full_name||'Usuário').split(' ')[0];const self=makeAvatar({accent:accentForSelf(),staff:isStaff(),label:selfLabel,seed:state.user?.id||selfLabel,appearanceOverride:state.avatarStyle||null});scene.add(self);const selfWorld=presenceToWorld(state.player.x,state.player.y);if(Math.hypot(selfWorld.x,selfWorld.z)<5.4){selfWorld.x=0;selfWorld.z=-8.2;}self.position.set(selfWorld.x,0,selfWorld.z);environment.updateStreaming?.(self.position);
  ...
  let airdropPreviousCamera='explore';
  function applyAirdropDetail(level){
    const next=['overview','district','full'].includes(level)?level:'full';
    airdropDetail=next;
    return airdropDetail;
  }
  function getAirdropState(){
    const pose=airdropSession&&airdropMode==='plane'?sampleAirdropPlane(airdropSession,Date.now()):null;
    return {
      mode:airdropMode,
      altitude:Math.max(0,Number(airdropMode==='plane'?(pose?.y??airdropAltitude):airdropAltitude)||0),
      detailLevel:airdropMode==='plane'?'overview':airdropDetail,
      targetWorldId:'campus-ds',
      targetName:'Campus DS',
      localX:Number(airdropMode==='plane'?(pose?.x??self.position.x):self.position.x)||0,
      localZ:Number(airdropMode==='plane'?(pose?.z??self.position.z):self.position.z)||0,
      prefetched:true
    };
  }
  function startAirdropSession(input={}){
    const normalized=normalizeAirdropSession(input||{});
    if(!normalized?.id)return false;
    closeSecurityCameras();closeViewpoint();cancelCampusVehicle();rides.cancel({silent:true});train.cancel();
    if(activeInterior){const previous=activeInterior;activeInterior=null;releaseClassInterior(previous);activeInteriorCollisionRoots=[];setWorldMode(false);onInteriorChange?.({inside:false,key:null,label:'Praça Central',runtime:'unmounted'});}
    if(activeToolInterior){const previous=activeToolInterior;activeToolInterior=null;activeToolFloor=0;releaseToolInterior(previous);activeInteriorCollisionRoots=[];setWorldMode(false);onInteriorChange?.({inside:false,key:null,kind:'tool',label:'Campus DS',runtime:'unmounted'});}
    airdropSession=normalized;airdropMode='plane';airdropAltitude=normalized.altitude;airdropDetail='overview';
    const pose=sampleAirdropPlane(normalized,Date.now());airdropHeading=pose.heading||0;
    airdropPlane.visible=true;airdropPlane.position.set(pose.x,pose.y,pose.z);airdropPlane.rotation.y=airdropHeading;
    airdropParachute.visible=false;self.position.set(pose.x,pose.y,pose.z);self.rotation.y=airdropHeading;playerY=pose.y;vertical=0;onGround=false;
    airdropPreviousCamera=cameraController.getMode?.()||'explore';cameraController.setMode?.('aerial');cameraController.setYaw?.(airdropHeading+Math.PI);
    return getAirdropState();
  }
  function jumpFromAirdrop(){
    if(airdropMode!=='plane')return false;
    const pose=airdropSession?sampleAirdropPlane(airdropSession,Date.now()):null;
    if(pose){self.position.set(pose.x,pose.y,pose.z);self.rotation.y=pose.heading||airdropHeading;airdropHeading=pose.heading||airdropHeading;airdropAltitude=pose.y;}
    airdropMode='freefall';airdropPlane.visible=false;airdropParachute.visible=false;playerY=airdropAltitude;vertical=-AIRDROP_CONFIG.freefallSpeed;onGround=false;applyAirdropDetail(airdropDetailLevel(airdropAltitude));
    return getAirdropState();
  }
  function deployAirdropParachute(){
    if(airdropMode!=='freefall')return false;
    airdropMode='parachute';airdropParachute.visible=true;vertical=-AIRDROP_CONFIG.parachuteSpeed;applyAirdropDetail(airdropDetailLevel(airdropAltitude));
    return getAirdropState();
  }
  function cancelAirdropSession({restoreGround=true}={}){
    const wasActive=airdropMode!=='ground'||!!airdropSession;
    airdropSession=null;airdropMode='ground';airdropAltitude=0;airdropDetail='full';airdropHeading=0;
    airdropPlane.visible=false;airdropParachute.visible=false;playerY=0;vertical=0;onGround=true;
    if(restoreGround)self.position.y=0;
    cameraController.setMode?.(airdropPreviousCamera||'explore');
    return wasActive;
  }
  ...
  const cameraController=createCameraController({THREE,camera,canvas,getCollisionRoots:()=>activeInterior||activeToolInterior?activeInteriorCollisionRoots:cameraCollisionRoots,initialYaw:Math.PI,initialPitch:.08,initialDistance:6.8,initialFov:Number(state.graphics?.fov)||null,initialSensitivity:Number(state.graphics?.mouseSensitivity)||1,initialInvertY:!!state.graphics?.invertY});
  ...
  function updateCampusStreaming(nowMs){
    ...
    const forceFull=securityViewState.active||viewpointState.active;environment.updateStreaming?.(self.position,{forceFull});
    lastCampusBudget=applyCampusRenderBudget({quality,profile,player:self.position,buildingRoots:environment.buildingRoots||[],experienceRoots:environment.experienceRoots||[],forceFull});
  }
  ...
  function frame(nowMs){
    ...
    const running=keys.has('ShiftLeft')||keys.has('ShiftRight')||runHeld,targetSpeed=playerMoveSpeed(state,{running,interior:!!activeInterior||!!activeToolInterior}),cameraYaw=cameraController.getYaw(),sy=Math.sin(cameraYaw),cy=Math.cos(cameraYaw),mx=ix*cy+iz*sy,mz=-ix*sy+iz*cy;
    ...
    const activeRideId=rides.currentId(),trainRide=train.tickTrip(nowMs),experienceRide=rides.tick(nowMs),ride=vehicleRide||trainRide||experienceRide,locomotionLocked=airdropMode!=='ground'||manualDriving||networkPassenger||!!ride||!!contextualTransition,motion=locomotion.stepHorizontal(dt,{x:mx,z:mz,running,interior:!!activeInterior||!!activeToolInterior,locked:locomotionLocked});let moving=locomotionLocked?Math.hypot(mx,mz):Math.min(1,motion.speed/Math.max(1,targetSpeed)),groundY=0,support=null;
    if(airdropMode!=='ground'){
      ...
      if(airdropMode==='plane'){
        ...
        if(airdropAltitude<=.05){airdropParachute.visible=false;airdropPlane.visible=false;airdropMode='ground';airdropSession=null;airdropAltitude=0;playerY=0;vertical=0;onGround=true;self.position.y=0;applyAirdropDetail('full');cameraController.setMode?.(airdropPreviousCamera||'explore');}
      }
      applyAirdropDetail(airdropDetailLevel(airdropAltitude));
    }else if(manualDriving){
      const throttle=clamp((up?1:0)-(down?1:0)-moveJoy.y,-1,1),steer=clamp((right?1:0)-(left?1:0)+moveJoy.x,-1,1),preset=activeCampusVehicle.speedPreset||'normal';
      if(activeCampusVehicle.mobilityType==='aerial'){
        const maxKmh=aerialSpeedKmh(activeCampusVehicle,preset),maxAltitude=Math.max(8,Number(activeCampusVehicle.maxAltitude)||30),climbRate=Math.max(3,Number(activeCampusVehicle.climbRate)||6.5),climbInput=clamp((keys.has('Space')||vehicleClimbHeld?1:0)-(keys.has('ShiftLeft')||keys.has('ShiftRight')||vehicleDescendHeld?1:0),-1,1);
        const core=vehicleCore.step(dt,{throttle,steer,climb:climbInput,brake:false},{maxSpeedKmh:maxKmh,maxAltitude,climbRate});vehicleSpeed=core.speed;vehicleHeading=core.heading;vehicleAltitude=core.y;vehicleVerticalSpeed=core.verticalSpeed;self.position.set(core.x,vehicleAltitude,core.z);self.rotation.y=vehicleHeading;playerY=vehicleAltitude;vertical=0;onGround=vehicleAltitude<=.04;moving=Math.min(1,Math.abs(vehicleSpeed)/Math.max(1,maxKmh/3.6));
        const pad=nearestCampusHelipad(self.position.x,self.position.z),landing=vehicleAltitude<=.35&&isCampusHelipadLanding(self.position.x,self.position.z);vehicleTrafficState={limitKmh:maxKmh,zoneName:landing?(pad?.name||'Heliponto'):'Espaço aéreo',signalState:'green',signalLabel:landing?'Pousado':'Voo livre',safety:landing?'Heliponto':core.collision?.label||''};
        ...
        const profileMaxKmh=preset==='tour'?24:preset==='sport'?44:34,road=resolveCampusSpeedLimit(self.position.x,self.position.z),signal=trafficSignalAhead(self.position.x,self.position.z,vehicleHeading,Date.now()),trafficBrake=!!(signal&&signal.state!=='green'&&signal.distance<3.7&&vehicleSpeed>0),maxKmh=Math.min(profileMaxKmh,road.limitKmh),braking=keys.has('Space')||vehicleBrakeHeld||trafficBrake;
        ...
        let core=vehicleCore.step(dt,{throttle,steer,brake:braking},{maxSpeedKmh:maxKmh,forceBrake:trafficBrake});vehicleSpeed=core.speed;vehicleHeading=core.heading;
        if(core.collision){recoverVehicleFromCollision(core.collision,nowMs);vehicleCore.syncExternal({pose:{x:self.position.x,y:0,z:self.position.z,heading:vehicleHeading},speed:vehicleSpeed,verticalSpeed:0,phase:'driving'});core=vehicleCore.snapshot();}
        else{self.position.set(core.x,0,core.z);self.rotation.y=vehicleHeading;rememberSafeVehiclePose(nowMs);}
        self.position.y=0;playerY=0;vertical=0;onGround=true;moving=Math.min(1,Math.abs(vehicleSpeed)/Math.max(1,maxKmh/3.6));playerVehicleShell.visible=true;playerVehicleShell.position.set(self.position.x,.02,self.position.z);playerVehicleShell.rotation.y=vehicleHeading;if(nowMs-lastVehicleTelemetrySent>=320){lastVehicleTelemetrySent=nowMs;const packet=vehicleNetworkPacket({...core,x:self.position.x,y:0,z:self.position.z,heading:vehicleHeading,speedKmh:Math.round(vehicleSpeed*3.6)},{driverId:state.user?.id||null,t:Date.now()});onVehicleTelemetry?.({x:self.position.x,z:self.position.z,heading:vehicleHeading,speedKmh:Math.round(vehicleSpeed*3.6),vehicleCore:packet});}
      }
    }else if(networkPassenger){const session=(vehicleNetworkState.sessions||[]).find(row=>row.driver_id===activeCampusVehicle.driverId),pos=session?occupantPlacement(session,activeCampusVehicle.seatIndex):null;if(pos){self.position.set(pos.x,.05,pos.z);self.rotation.y=pos.heading;playerY=.05;vertical=0;onGround=true;moving=Math.min(1,Math.abs(Number(session.speed_kmh)||0)/44);cameraController.setYaw(pos.heading+Math.PI);vehicleCore.syncExternal({pose:{x:pos.x,y:.05,z:pos.z,heading:pos.heading},speed:(Number(session.speed_kmh)||0)/3.6,phase:'remote'});}else moving=0;
    }else if(ride){self.position.set(ride.x,ride.y,ride.z);playerY=ride.y||0;vertical=0;onGround=true;self.rotation.y=ride.heading||self.rotation.y;cameraController.setYaw((ride.heading||0)+Math.PI);moving=.8;if(activeCampusVehicle)vehicleCore.syncExternal({pose:{x:ride.x,y:ride.y||0,z:ride.z,heading:ride.heading||self.rotation.y},speed:(activeCampusVehicle.speedPreset==='tour'?24:activeCampusVehicle.speedPreset==='sport'?44:34)/3.6,phase:'guided'});}
    else{
      const nx=self.position.x+motion.x,nz=self.position.z+motion.z;if(canStand(nx,self.position.z))self.position.x=nx;if(canStand(self.position.x,nz))self.position.z=nz;
      ...
      support=(!activeInterior&&!activeToolInterior)?campusSurfaceAt(self.position.x,self.position.z,-.04):null;groundY=support?.h||0;if(onGround&&groundY>playerY&&groundY-playerY<=.72){playerY=groundY;vertical=0;}vertical-=playerGravity(state,{worldId:'campus-ds',interior:!!activeInterior||!!activeToolInterior})*dt;playerY+=vertical*dt;if(playerY<=groundY){playerY=groundY;vertical=0;onGround=true;}else onGround=false;
  ...
  function useCampusVehicle(ref,options={}){
    const registered=vehicleRegistry.register(ref||{}),aerial=registered.mobilityType==='aerial';
    if(activeInterior||activeToolInterior||!ref||(!aerial&&!registered.routeId))return false;
