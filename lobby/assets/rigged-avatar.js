...
export function updateRiggedAvatar(avatar,{speed=0,jump=0,dt=.016,emoteKind=null,emoteActive=false,localAction=null,time=0}={}){
  ...
  const pose=()=>{
    ...
    else if(localAction==='exam'||localAction==='exam-paused'){if(hips)hips.position.y-=.72;if(ll)ll.rotation.x-=1.12;if(rl)rl.rotation.x-=1.12;if(torso)torso.rotation.x+=.1;if(la){la.rotation.x-=.92;la.rotation.z+=.14;}if(ra){ra.rotation.x-=.92;ra.rotation.z-=.14;if(localAction==='exam'){const write=Math.sin(time*9)*.12;ra.rotation.x+=write;ra.rotation.z-=write*.45;}}}
    else if(localAction==='program'){if(hips)hips.position.y-=.68;if(ll)ll.rotation.x-=1.08;if(rl)rl.rotation.x-=1.08;if(torso)torso.rotation.x+=.08;const type=Math.sin(time*12)*.13;if(la){la.rotation.x-=1.02-type;la.rotation.z+=.12;}if(ra){ra.rotation.x-=1.02+type;ra.rotation.z-=.12;}}
  };
