...
export async function createPluginWorldLiteHost(context={},config={}){
  ...
  function frame(now){
    ...
    const move=input.sample(),speed=move.running?(config.runSpeed||20):(config.walkSpeed||12),desired={x:player.x+move.x*speed*dt,y:player.y,z:player.z+move.z*speed*dt},next=resolve({...player},desired)||player;Object.assign(player,clonePosition(next,player));
...
export async function createPluginWorld3DHost(context={},config={}){
  ...
  const profile=detectPerformanceProfile(),qualityStart=chooseInitialQuality(profile,null),renderer=new THREE.WebGLRenderer({canvas:context.canvas,antialias:qualityStart!=='low'&&!profile.mobile,alpha:false,powerPreference:profile.mobile?'low-power':'high-performance'});renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.0;
  ...
  const others=new Map(),cameraController=createCameraController({THREE,camera,canvas:context.canvas,getCollisionRoots:()=>config.cameraCollisionRoots?.(plugin)||[],initialYaw:Math.PI,initialPitch:.38,initialDistance:8.2,initialFov:Number(context.state?.graphics?.fov)||65});
  ...
  function syncOthers(time,dt){const seen=new Set();for(const o of context.state?.others||[]){if(o.area!==config.area)continue;seen.add(o.student_id);let e=others.get(o.student_id);if(!e){const staff=['teacher','admin','super_admin'].includes(o.participant_role),avatar=avatarSystem.createAvatar({accent:staff?'#ffd166':'#72e6ff',staff,label:o.display_name||'Visitante',seed:o.student_id});scene.add(avatar);e={avatar,target:new THREE.Vector3()};others.set(o.student_id,e);}const wp=config.presenceToWorld?.(o.x,o.y)||mapPresenceToWorld(config.bounds,o.x,o.y);e.target.set(wp.x,0,wp.z);const before=e.avatar.position.clone();e.avatar.position.lerp(e.target,1-Math.exp(-6*dt));const dx=e.avatar.position.x-before.x,dz=e.avatar.position.z-before.z,sp=Math.hypot(dx,dz)/Math.max(.001,dt);if(sp>.03)e.avatar.rotation.y=Math.atan2(dx,dz);avatarSystem.animate(e.avatar,{speed:sp,time,dt});avatarSystem.applyLOD(e.avatar,self.position.distanceTo(e.avatar.position),{staff});avatarSystem.updateEmote(e.avatar,o.emote,o.emote_until);}for(const[id,e]of others)if(!seen.has(id)){scene.remove(e.avatar);avatarSystem.disposeAvatar(e.avatar);others.delete(id);}}
  function resize(){if(!sizeDirty)return;sizeDirty=false;const r=context.canvas.getBoundingClientRect(),w=Math.max(1,Math.floor(r.width)),h=Math.max(1,Math.floor(r.height));renderer.setPixelRatio(Math.min(devicePixelRatio||1,quality==='low'?1:quality==='medium'?1.25:quality==='high'?1.55:1.8));renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix();}
  ...
  function frame(now){
    ...
    for(const fn of updateHooks)try{fn(dt);}catch(error){console.warn(`[${config.worldId}] world update hook`,error);}
    cameraController.update({playerPosition:self.position,moving:move.l,running:move.running,time:now/1000,dt});self.visible=!cameraController.isFirstPerson?.();emitPlayer(context,config,player,plugin,move.l,move.running,{onGround});renderer.render(scene,camera);if(!firstFrame){firstFrame=true;context.onFirstFrame?.();}
  }
