...
export async function createPluginWorldLiteHost(context={},config={}){
  const player=startPosition(context,config),hostState=makeHostState(context.state,player),updateHooks=new Set(),locomotion=createPlayerLocomotion({worldId:config.worldId});let stopped=false,raf=0,last=performance.now(),fps=60,frames=0,sampleStart=last;
  ...
  function frame(now){
    ...
    const move=input.sample(),motion=locomotion.stepHorizontal(dt,{x:move.x,z:move.z,running:move.running,interior:!!plugin?.getActiveInterior?.()}),desired={x:player.x+motion.x,y:player.y,z:player.z+motion.z},next=resolve({...player},desired)||player;Object.assign(player,clonePosition(next,player));
  ...
  const api={
    ...
    teleportTo(target){const destination=typeof target==='string'?plugin?.getDestinations?.()?.find?.(d=>d.id===target):target;if(!destination)return false;Object.assign(player,clonePosition(destination,player));locomotion.haltHorizontal();plugin?.teleportTo?.(destination);return true;},
...
export async function createPluginWorld3DHost(context={},config={}){
  ...
  const others=new Map(),cameraController=createCameraController({THREE,camera,canvas:context.canvas,getCollisionRoots:()=>config.cameraCollisionRoots?.(plugin)||[],initialYaw:Math.PI,initialPitch:.08,initialDistance:8.2,initialFov:Number(context.state?.graphics?.fov)||65,initialSensitivity:Number(context.state?.graphics?.mouseSensitivity)||1,initialInvertY:!!context.state?.graphics?.invertY}),locomotion=createPlayerLocomotion({worldId:config.worldId});
  ...
  const resolve=(from,desired)=>config.resolveMovement?.(plugin,from,desired)||plugin?.resolveMovement?.(from,desired,{activeInteriorId:plugin?.getActiveInterior?.()||null})||desired;
  const input=inputBindings({onInteract:()=>context.onInteract?.(),onJump:()=>{if(onGround){vertical=playerJumpImpulse(context.state,{worldId:config.worldId});onGround=false;}},onRun:v=>plugin?.setRun?.(v)});
  ...
  function frame(now){
    ...
    const move=input.sample(),yaw=cameraController.getYaw(),sy=Math.sin(yaw),cy=Math.cos(yaw),mx=move.x*cy+move.z*sy,mz=-move.x*sy+move.z*cy,motion=locomotion.stepHorizontal(dt,{x:mx,z:mz,running:move.running,interior:!!plugin?.getActiveInterior?.()}),desired={x:player.x+motion.x,y:player.y,z:player.z+motion.z},next=resolve({...player},desired)||player;Object.assign(player,clonePosition(next,player));self.position.x=player.x;self.position.z=player.z;
    vertical-=playerGravity(context.state,{worldId:config.worldId})*dt;playerY+=vertical*dt;if(playerY<=0){playerY=0;vertical=0;onGround=true;}self.position.y=(Number(player.y)||0)+playerY;if(move.l>.03){const desiredRot=Math.atan2(mx,mz),delta=(desiredRot-self.rotation.y+Math.PI*3)%(Math.PI*2)-Math.PI;self.rotation.y+=delta*Math.min(1,dt*9);}avatarSystem.animate(self,{speed:motion.speed,jump:playerY,time:now/1000,vertical,dt});syncOthers(now/1000,dt);
    for(const fn of updateHooks)try{fn(dt);}catch(error){console.warn(`[${config.worldId}] world update hook`,error);}
