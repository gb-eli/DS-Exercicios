import { supabase, NETWORK_TIMEOUT_MS } from './supabase.js?v=14.10.8.51';
import { SCHOOL_EMAIL_DOMAIN, ACTIVITY_URL, LOBBY_VERSION } from './config.js?v=14.10.8.51';
import { createLobby3D } from './lobby3d.js?v=14.10.8.51';
import { createLobbyLite } from './lobby-lite.js?v=14.10.8.51';
...
function handleWorldObject(obj){
  ...
  if(['pool','playground','slide','coaster','tower'].includes(obj.type)){
    const messages={pool:'Piscina Neon • área de convivência e cenário animado.',playground:'Parquinho DS • área recreativa do Lobby.',slide:'Escorregador Turbo • explore as escadas e a rampa.',coaster:'Trilho Panorâmico • carrinho e trilhos animados do Campus.',tower:'Torre de Escadas • área vertical e mirante do Lobby.'};
    state.runtime?.setLocalAction?.(obj.type==='playground'?'cheer':null);toast(messages[obj.type]||obj.description||obj.name);return true;
  }
  return false;
}
function interact(){
...
function updateModeUI(mode=state.runtimeMode){
  ...
  if(waiting)waiting.textContent=mode==='3d'?'Modo 3D ativo. Explore o Campus ou volte ao 2D a qualquer momento.':'Explore o mapa 2D enquanto aguarda a atividade. O modo 3D é opcional.';
...
function runtimeHooks(){return{canvas:$('game3d'),zones,state,isStaff,className,onInteract:interact,onQualityChange:q=>{globalThis.__agvLobbyDiag?.update?.({runtime:{quality:q}});$('quality-button').textContent=state.runtimeMode==='lite'?'Leve':q==='ultra'?'Ultra':q==='high'?'Alto':q==='low'?'Eco':'Médio'},onPerf:perfUpdate,onError:m=>toast(m),onContextLost:()=>{if(state.runtimeMode==='3d'&&!state.bootingRuntime)startLite('webgl_context_lost').catch(()=>{})},onChallengeEvent:event=>{clearTimeout(challengeStatusTimer);if(event?.message)toast(event.message);const waiting=$('waiting-strip-text');if(waiting&&event?.message)waiting.textContent=event.message;if(event?.type==='complete'||event?.type==='cancel')challengeStatusTimer=setTimeout(()=>updateModeUI(state.runtimeMode),4200);},onAreaChange:showZoneBanner,onInteriorChange:e=>{state.interior=e.inside?e.key:null;showZoneBanner(e.inside?e.key:'central');toast(e.inside?`Laboratório ${String(e.label||e.key).toUpperCase()} • ambiente interno`:'Praça Central • ambiente externo');},onPlayerState:p=>{state.player.x=p.x;state.player.y=p.y;state.player.area=p.area;state.interior=p.interior||null;state.nearPortal=p.nearPortal;state.nearStudent=p.nearStudent;state.nearSeat=p.nearSeat||null;state.nearWorldObject=p.nearWorldObject||null;state.seated=!!p.seated;$('area-label').textContent=p.interior?`Laboratório ${String(p.interior).toUpperCase()}`:areaText(p.area);renderInteraction();presence();}}}
...
async function boot(){
  ...
  // Regra v14.10.8.51: todo acesso começa no 2D. O aluno decide quando carregar o 3D.
