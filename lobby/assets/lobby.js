...
import { SCHOOL_EMAIL_DOMAIN, ACTIVITY_URL, LOBBY_VERSION } from './config.js?v=14.10.8.70';
import { createLobbyState } from './core/lobby-state.js?v=14.10.8.70-stage39-traffic';
import { createWorldManager } from './core/world-manager.js?v=14.10.8.66-stage34-f63a';
import { CAMPUS_WORLD_ADAPTER, VALE_WORLD_ADAPTER } from './core/world-adapter.js?v=14.10.8.70-stage39-traffic';
...
import { CAMPUS_EXPERIENCES, CAMPUS_TRAIN_STATIONS } from './world/campus-experiences.js?v=14.10.8.66-stage28';
import { CAMPUS_TRAFFIC_ROUTES, CAMPUS_DRIVABLE_VEHICLES } from './world/campus-mobility-systems.js?v=14.10.8.67-stage36';
...
import { CAMPUS_INTERIOR_MAP } from './world/campus-interiors.js?v=14.10.8.66-stage34';
...
const currentWorldAdapter=()=>state.scene==='vale'?VALE_WORLD_ADAPTER:CAMPUS_WORLD_ADAPTER;
...
function vehicleKindLabel(kind){return kind==='bus'?'ônibus':kind==='van'?'van':kind==='bike'?'bike':kind==='drone'?'drone':'carro';}
function describeVehicle(ref){const kind=vehicleKindLabel(ref?.kind);const seats=Number(ref?.seatCapacity||ref?.capacity||((ref?.kind==='bus')?8:(ref?.kind==='van')?4:2));return{kind,seats};}
function syncVehicleModal(){
  const ref=pendingVehicleRef,meta=describeVehicle(ref),speed=VEHICLE_SPEED_PROFILES[selectedVehicleSpeed]||VEHICLE_SPEED_PROFILES.normal;
  if($('vehicle-title'))$('vehicle-title').textContent=ref?.name||'Veículo do Campus';
  if($('vehicle-subtitle'))$('vehicle-subtitle').textContent=ref?.description||`Passeio guiado pelo Campus DS em ${meta.kind}.`;
  if($('vehicle-meta'))$('vehicle-meta').textContent=`Tipo: ${meta.kind} • Lugares: ${meta.seats} • Velocidade alvo: ${speed.kmh} km/h`;
  document.querySelectorAll('[data-vehicle-seat]').forEach(button=>button.classList.toggle('active',button.dataset.vehicleSeat===selectedVehicleSeat));
...
function openVehicleModal(ref){
  if(!ref)return false;
  pendingVehicleRef=ref;
  selectedVehicleSeat='driver';
  selectedVehicleSpeed=ref.kind==='bus'?'tour':'normal';
  syncVehicleModal();
  openModal('vehicle-modal');
  return true;
}
async function startVehicleFromModal(){
  if(!pendingVehicleRef)return false;
  const ref=pendingVehicleRef,speed=VEHICLE_SPEED_PROFILES[selectedVehicleSpeed]||VEHICLE_SPEED_PROFILES.normal,button=$('vehicle-start');if(button)button.disabled=true;
  try{
    ...
    if(state.vehicleMembership)await leaveNetworkVehicle({silent:true});const ok=state.runtime?.useCampusVehicle?.(ref,{seatMode:'passenger',speedPreset:selectedVehicleSpeed});if(ok){closeModal('vehicle-modal');toast(`${ref.name} • carona automática • ${speed.label.toLowerCase()} (${speed.kmh} km/h) • E sair`);return true;}toast('Este veículo não está disponível agora.');return false;
  }finally{if(button)button.disabled=false;}
}
function renderValeMinimap(force=false){
  const canvas=$('vale-minimap');if(!canvas)return;const visible=state.runtimeMode==='3d'&&!state.interior&&(state.scene==='vale'||state.scene==='campus');canvas.classList.toggle('hidden',!visible);if(!visible)return;const now=performance.now();if(!force&&now-valeMiniLast<120)return;valeMiniLast=now;const rect=canvas.getBoundingClientRect(),w=Math.max(120,Math.floor(rect.width||160)),h=Math.max(120,Math.floor(rect.height||160)),dpr=Math.min(devicePixelRatio||1,1.5);if(canvas.width!==Math.round(w*dpr)||canvas.height!==Math.round(h*dpr)){canvas.width=Math.round(w*dpr);canvas.height=Math.round(h*dpr);}const c=canvas.getContext('2d');if(!c)return;c.setTransform(dpr,0,0,dpr,0,0);c.clearRect(0,0,w,h);c.fillStyle='rgba(3,12,18,.88)';c.fillRect(0,0,w,h);c.strokeStyle='rgba(114,230,255,.28)';c.lineWidth=1;c.strokeRect(.5,.5,w-1,h-1);const pad=10;
  if(state.scene==='campus'){
    ...
    for(const vehicle of CAMPUS_DRIVABLE_VEHICLES||[]){const q=map(vehicle.x,vehicle.z);minimapDot(c,q.x,q.y,1.8,'#ffb86b');}for(const session of state.vehicleSessions||[]){const q=map(session.x,session.z);minimapDot(c,q.x,q.y,2.5,session.driver_id===state.user?.id?'#ffffff':'#7ce7ff');}
...
function handleWorldObject(obj){
  ...
  if(obj.type==='tool-interior-exit'){if(window.AGVAvatarContext?.isLocked?.(state.avatarContext)){toast('Finalize ou saia da atividade antes de deixar este ambiente.');return true;}const ok=state.runtime?.exitToolInterior?.();if(ok)toast('Voltando para as ruas do Campus…');return !!ok}
  ...
  if(obj.type==='campus-vehicle'){openVehicleModal(obj);return true}
...
function interact(){
  const activeVehicle=state.runtime?.getCampusVehicle?.();if(activeVehicle){if(state.vehicleMembership){leaveNetworkVehicle().catch(()=>{});return;}state.runtime?.cancelCampusVehicle?.();toast(`${activeVehicle.name||'Veículo'} • passeio encerrado.`);return;}
...
async function startLite(reason='fallback'){
  ...
  const restoreInterior=state.scene==='campus'?state.interior:null,restoreFloor=state.runtime?.getToolInterior?.()?.floor||0;setModeTransition(true,'Abrindo mapa 2D');worldManager.stop('switch_to_lite');freshCanvas();state.runtimeMode='lite';showLiteBadge(true);setLoading(state.scene==='vale'?'Gerando distritos, empresas, ruas e atrações do Vale…':'Preparando o mapa interativo com presença em tempo real…',state.scene==='vale'?'Mapa 2D do Vale do Silício AGV':'Mapa 2D do Campus');
  await worldManager.start({adapter:currentWorldAdapter(),mode:'lite',context:{...runtimeHooks(),signal:null}});state.runtime?.setCinemaMedia?.(state.cinemaMedia||EMPTY_CINEMA_MEDIA);restoreToolInterior(restoreInterior,restoreFloor);if(state.scene==='campus')applyAvatarContext(state.avatarContext||window.AGVAvatarContext?.get?.());state.runtime?.setWorldTimeMode?.(state.graphics.worldTimeMode);state.runtime?.setFPSCap?.(state.graphics.fpsCap);$('quality-button').textContent='Leve';$('camera-button').textContent='Mapa';$('perf-badge')?.classList.add('hidden');updateModeUI('lite');
...
async function start3D({allowFallback=true}={}){
  ...
  if(state.bootingRuntime)return state.runtime;const restoreInterior=state.scene==='campus'?state.interior:null,restoreFloor=state.runtime?.getToolInterior?.()?.floor||0;state.bootingRuntime=true;setModeTransition(true,'Entrando no ambiente 3D');worldManager.stop('switch_to_3d');freshCanvas();state.runtimeMode='3d';showLiteBadge(false);setLoading(state.scene==='vale'?'Gerando cidade tecnológica, prédios, NPCs e mobilidade…':'Montando praça, edifícios, personagens e presença online…',state.scene==='vale'?'Entrando no Vale do Silício AGV 3D':'Entrando no Campus DS 3D');
