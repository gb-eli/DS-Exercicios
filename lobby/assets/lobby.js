import { supabase, NETWORK_TIMEOUT_MS } from './supabase.js?v=14.10.8.50';
import { SCHOOL_EMAIL_DOMAIN, ACTIVITY_URL, LOBBY_VERSION } from './config.js?v=14.10.8.50';
import { createLobby3D } from './lobby3d.js?v=14.10.8.50';
import { createLobbyLite } from './lobby-lite.js?v=14.10.8.50';
...
function cycleQuality(){if(state.runtimeMode==='lite'){retry3D().catch(()=>{});return}const current=state.runtime?.getQuality?.()||'medium',order=['low','medium','high','ultra'],next=order[(order.indexOf(current)+1)%order.length];try{localStorage.setItem('agv:lobby:quality',next)}catch(_){}state.runtime?.setQuality?.(next)}
...
console.info(`AGV Lobby DS ${LOBBY_VERSION} • Fase F Produção • rede ${NETWORK_TIMEOUT_MS}ms • presença protegida por Edge Function`);
