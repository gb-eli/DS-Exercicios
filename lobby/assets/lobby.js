...
import { createLobbyLite } from './lobby-lite.js?v=14.10.8.65-stage34';
...
const state={
  ...
  graphics:{fov:65,fpsCap:60,worldTimeMode:'cycle',showPerf:false},worldTimeControl:null,lastWorldTimeToken:null,worldTimeAdminControl:null,worldWeatherControl:null,lastWorldWeatherToken:null,worldWeatherAdminControl:null,avatarContext:null,avatarStyle:{...AVATAR_STYLE_PRESETS.casual}
};
...
async function start3D({allowFallback=true}={}){
  ...
  try{
    ...
    state.runtime=await Promise.race([create,timeout]);clearTimeout(timer);restoreToolInterior(restoreInterior,restoreFloor);if(state.scene==='campus')applyAvatarContext(state.avatarContext||window.AGVAvatarContext?.get?.());setLoading('Sincronizando câmera 360° e personagens…',state.scene==='vale'?'Vale quase pronto':'Campus quase pronto');
