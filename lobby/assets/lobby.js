...
import { createLobbyState } from './core/lobby-state.js?v=14.10.8.65-stage34-f63a';
import { createWorldManager } from './core/world-manager.js?v=14.10.8.65-stage34-f63a';
import { CAMPUS_WORLD_ADAPTER, VALE_WORLD_ADAPTER } from './core/world-adapter.js?v=14.10.8.65-stage34-f63a';
...
import { CAMPUS_EXPERIENCES } from './world/campus-experiences.js?v=14.10.8.65-stage28';
import { createProximityChat } from './social/proximity-chat.js?v=14.10.8.65';
import { AVATAR_STYLE_PRESETS } from './characters/avatar-system.js?v=14.10.8.65-stage34';
import { CAMPUS_DESTINATION_MAP } from './world/campus-destinations.js?v=14.10.8.65';
import { CAMPUS_INTERIOR_MAP } from './world/campus-interiors.js?v=14.10.8.65-stage34';
import { normalizeWorldWeatherControl, resolveWorldWeather } from './world/weather-system.js?v=14.10.8.65-stage32';
...
async function start3D({allowFallback=true}={}){
  ...
  try{
    ...
    await Promise.race([create,timeout]);clearTimeout(timer);restoreToolInterior(restoreInterior,restoreFloor);if(state.scene==='campus')applyAvatarContext(state.avatarContext||window.AGVAvatarContext?.get?.());setLoading('Sincronizando câmera 360° e personagens…',state.scene==='vale'?'Vale quase pronto':'Campus quase pronto');
...
$('train-close').onclick=()=>closeModal('train-modal');$('train-panoramic').onclick=()=>{const ok=state.runtime?.startExperience?.('coaster');if(ok){closeModal('train-modal');toast('🎢 Montanha-russa panorâmica iniciada • volta completa pelo Campus.');}else toast('O passeio panorâmico não está disponível agora.');};$('chat-close').onclick=()=>closeModal('chat-modal');$('chat-form').onsubmit=e=>{e.preventDefault();sendProximityChat();};$('world-settings-button').onclick=openWorldSettings;$('world-settings-close').onclick=()=>closeModal('world-settings-modal');$('world-settings-apply').onclick=()=>applyWorldSettings().catch(error=>{console.error(error);toast('Não foi possível aplicar as configurações.');});$('world-settings-reset').onclick=()=>{state.graphics={fov:65,fpsCap:60,worldTimeMode:'cycle',showPerf:false};state.avatarStyle={...AVATAR_STYLE_PRESETS.casual};syncWorldSettingsForm();};$('staff-world-time-mode').onchange=()=>$('staff-world-time-hour-row').classList.toggle('hidden',$('staff-world-time-mode').value!=='fixed');$('staff-world-time-apply').onclick=()=>broadcastWorldTimeControl();$('staff-world-weather-apply').onclick=()=>broadcastWorldWeatherControl();$('fov-range').oninput=()=>{$('fov-value').textContent=`${$('fov-range').value}°`;};document.querySelectorAll('[data-camera-mode]').forEach(button=>button.onclick=()=>{if(state.runtimeMode!=='3d')return toast('Entre no modo 3D para trocar a câmera.');const mode=state.runtime?.setCameraMode?.(button.dataset.cameraMode);if(mode){$('camera-button').textContent={explore:'3ª pessoa',firstperson:'1ª pessoa',wide:'Ampla',campus:'Campus'}[mode]||'Câmera';toast(`Câmera: ${button.textContent}`);}});document.querySelectorAll('[data-avatar-preset]').forEach(button=>button.onclick=()=>applyAvatarPreset(button.dataset.avatarPreset));$('staff-chat-target')?.addEventListener('click',()=>{const id=$('staff-student-modal').dataset.student,person=state.others.find(o=>o.student_id===id);if(person){closeModal('staff-student-modal');openChatWith(person);}});
