...
function activityListRow(exercise,{progress=null,locked=false,reason='',completed=false}={}){
  ...
  const title=document.createElement('strong');title.textContent=exercise.title;
  ...
  if(locked)meta.textContent=`${subject} • ${reason||'Aguardando liberação'}`;
    ...
    meta.textContent=progress?.status==='in_progress'?`${subject} • ${pct}% concluído`:`${subject} • disponível`;
  }
...
async function renderDashboard() {
  ...
  if (nextExercise) {
    ...
    $('resume-title').textContent = `Exercício ${String(nextExercise.exercise_number).padStart(2, '0')} — ${nextExercise.title}`;
    $('resume-description').textContent = continuing?`Você já fez ${pct}%. Continue sem perder seu trabalho.`:'Esta atividade está liberada e pronta para começar.';
...
$('legacy-import-btn')?.addEventListener('click',()=>{
  ...
  select.innerHTML=eligible.map(ex=>`<option value="${ex.id}">Ex. ${String(ex.exercise_number).padStart(2,'0')} — ${escapeHtml(ex.title)}</option>`).join('');
