...
async function loadHistory(){
  ...
  (data||[]).forEach(row=>{
    ...
    btn.onclick=()=>{ $('code-editor').value=row.content||''; renderHighlight(); scheduleSave(); $('history-dialog').close(); };
...
async function completeExercise(){
  ...
  const missing=expected.filter(name=>!String(fileContent(name)).trim());
  if(missing.length){
    setSaveState(`Preencha todos os arquivos antes de concluir: ${missing.join(', ')}`,'error');
    return;
  }
